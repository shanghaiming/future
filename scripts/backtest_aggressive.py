...
class AggressiveBacktest:
    ...
    def run(self, data, start_date, end_date):
        ...
        for date in dates:
            ...
            # 检查爆仓
            if total < self.initial_capital * 0.3:
                print(f"\n!!! 爆仓! 权益: {total:,.0f}")
                break
    ...
    def _open(self, symbol, direction, price, date, atr):
        """开仓 - 使用杠杆"""
        # 每品种投入20%资金 * 5倍杠杆 = 100%名义仓位
        notional = self.capital * 0.2 * self.leverage
        ...
        self.positions[symbol] = {
            ...
            'entry_price': price,
            'atr': atr,
            'stop': price - atr * 3 * direction,  # 3倍ATR止损
            'add_ons': 0,
            'total_size': size
        }
    ...
    def _add(self, symbol, date, price, atr):
        """加仓"""
        pos = self.positions[symbol]
        add_size = pos['total_size'] * 0.5  # 加半仓
        ...
        pos['add_ons'] += 1
        pos['stop'] = price - atr * 2 * pos['direction']  # 移动止损
...
def main():
    ...
    bt = AggressiveBacktest(initial_capital=500000)
    ...
    with open(os.path.join(output_dir, 'backtest_aggressive.json'), 'w') as f:
