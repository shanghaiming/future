...
期货高胜率策略 v13
目标: 年化500%+ 胜率>50% 持仓<=3
策略: 趋势回调 + RSI过滤 + 成交量确认 + 高仓位 + 动态止盈止损
"""
...
class BacktestV13:
    ...
    def generate_signals(self, data, date):
        ...
        for symbol, df in data.items():
            ...
            trend_up = latest['trend_score'] >= 3
            trend_down = latest['trend_score'] <= 1
            ...
            # 成交量确认: 当前放量或昨天缩量今天放量
            vol_ok = latest['vol_ratio'] > 0.8
            
            # 波动率过滤
            hv_ok = 0.05 < latest['hv_20'] < 0.8
            atr_ok = latest['atr_pct'] > 0.003
            
            score = 0
            
            if trend_up and pullback_long and rsi_long and vol_ok and hv_ok and atr_ok:
                score = latest['trend_score'] + (35 - latest['rsi']) / 5 + latest['vol_ratio']
                signals.append({
                    'symbol': symbol,
                    'direction': 1,
                    'score': score,
                    'reason': f'回调做多 趋势{latest["trend_score"]} RSI{latest["rsi"]:.0f}'
                })
            
            elif trend_down and pullback_short and rsi_short and vol_ok and hv_ok and atr_ok:
                score = (4 - latest['trend_score']) + (latest['rsi'] - 65) / 5 + latest['vol_ratio']
                signals.append({
                    'symbol': symbol,
                    'direction': -1,
                    'score': score,
                    'reason': f'反弹做空 趋势{latest["trend_score"]} RSI{latest["rsi"]:.0f}'
                })
    ...
    def _check_stops(self, symbol, date, price):
        ...
        if pos['direction'] == 1:
            ...
            if price > pos['entry_price'] * 1.02:
                trailing = pos['highest_price'] * 0.985
                if price < trailing:
            ...
            if price < pos['entry_price'] * 0.98:
                trailing = pos['lowest_price'] * 1.015
                if price > trailing:
                    self._close(symbol, date, price, '移动止盈')
                    return
...
def main():
    ...
    bt = BacktestV13(initial_capital=500000)
    print("加载期货数据...")
    ...
    with open(os.path.join(output_dir, 'backtest_v13.json'), 'w') as f:
        json.dump({
