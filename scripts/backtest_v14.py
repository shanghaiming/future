...
期货高胜率策略 v14
目标: 年化500%+ 胜率>50% 持仓<=3
策略: 趋势>=2 + 价格<MA5做多 / 趋势<=2 + 价格>MA5做空 + 动态止盈止损
"""
...
class BacktestV14:
        ...
        for symbol, df in data.items():
            ...
            if trend_up and pullback_long and hv_ok and atr_ok:
                ...
                signals.append({
                    ...
                    'direction': 1,
                    'score': score,
                    'reason': f'趋势回调做多 趋势分={latest["trend_score"]}'
                })
                ...
                signals.append({
                    ...
                    'direction': -1,
                    'score': score,
                    'reason': f'趋势反弹做空 趋势分={latest["trend_score"]}'
                })
        ...
        for date in dates:
            ...
            for symbol in list(self.positions.keys()):
                ...
                if len(day_data) == 0:
                    continue
                price = day_data.iloc[0]['close']
                ma5 = day_data.iloc[0]['ma_5']
                self._check_stops(symbol, date, price, ma5)
    ...
    def _check_stops(self, symbol, date, price, ma5):
        pos = self.positions[symbol]
        ...
        if pos['direction'] == 1:
            ...
            # 暂时注释掉，只用止损和止盈
            # if price > ma5:
            #     self._close(symbol, date, price, '趋势恢复')
            #     return
            
            # 目标止盈: 3%
            if price > pos['entry_price'] * 1.03:
                self._close(symbol, date, price, '目标止盈3%')
                return
            
            # 移动止盈: 盈利2%后启动，回撤1.5%出场
...
def main():
    ...
    bt = BacktestV14(initial_capital=500000)
    print("加载期货数据...")
    ...
    with open(os.path.join(output_dir, 'backtest_v14.json'), 'w') as f:
        json.dump({
