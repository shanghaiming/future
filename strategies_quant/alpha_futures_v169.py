"""
Alpha Futures V169 — Adaptive Vol Threshold + Vol Filter Optimization
==============================================================================
V167 discovered the best risk-adjusted configs:
  - atr_norm<12% + no equity curve = +249%/-16% WF (R/M=15.16)
  - atr_norm<12% + WR-adaptive     = +253%/-17% WF (R/M=14.85)
  - atr_norm<10% + streak sizing   = +231%/-15% WF (R/M=15.25)

V169 pushes the vol filter approach further:
1. Adaptive vol threshold: dynamic atr_norm_max based on MKT_VOL regime
2. Fine-tune atr_norm around 12% (11, 12, 13)
3. ATR + MKT_VOL double filter combos
4. max_corr sweep with vol filter
5. WR-adaptive vs no WR confirmation
6. top_n=4 with vol filter

All use max_corr=0.7 default, aggro100 DD tiers, Kitchen Sink sizing.
Same signals, same DD tiers, same formula as V167.
"""
...
def main():
    ...
    # ===================== SECTION 0: V167 BASELINE REPRODUCTION =====================
    print("\n" + "=" * 130)
    print("  SECTION 0: V167 BASELINE REPRODUCTION")
    print("  atr_norm<12%, max_corr=0.7, no wr_mult, aggro100 DD, top_n=3")
    print("=" * 130)

    r_base12 = backtest(atr_norm_max=12.0, max_corr=0.7,
                        dd_tiers=DD_AGGR100, eq_method='none',
                        regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                        hold=1, top_n=3, vol_mode='fixed')
    pr(r_base12, "V167 BASELINE: atr<12%, corr=0.7, no wr, aggro100, top_n=3")
    all_results.append({**r_base12, 'label': 'V167_base_atr12_nowr', 'section': 0,
                        'atr_norm_max': 12.0, 'eq_method': 'none', 'max_corr': 0.7,
                        'top_n': 3, 'vol_mode': 'fixed', 'mkt_vol_pctile': None})

    r_base12_wr = backtest(atr_norm_max=12.0, max_corr=0.7,
                           dd_tiers=DD_AGGR100, eq_method='wr',
                           regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                           hold=1, top_n=3, vol_mode='fixed')
    pr(r_base12_wr, "V167 BASELINE: atr<12%, corr=0.7, WR-adaptive, aggro100, top_n=3")
    all_results.append({**r_base12_wr, 'label': 'V167_base_atr12_wr', 'section': 0,
                        'atr_norm_max': 12.0, 'eq_method': 'wr', 'max_corr': 0.7,
                        'top_n': 3, 'vol_mode': 'fixed', 'mkt_vol_pctile': None})

    r_base10 = backtest(atr_norm_max=10.0, max_corr=0.7,
                        dd_tiers=DD_AGGR100, eq_method='none',
                        regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                        hold=1, top_n=3, vol_mode='fixed')
    pr(r_base10, "V167 BASELINE: atr<10%, corr=0.7, no wr, aggro100, top_n=3")
    all_results.append({**r_base10, 'label': 'V167_base_atr10_nowr', 'section': 0,
                        'atr_norm_max': 10.0, 'eq_method': 'none', 'max_corr': 0.7,
                        'top_n': 3, 'vol_mode': 'fixed', 'mkt_vol_pctile': None})

    r_base10_streak = backtest(atr_norm_max=10.0, max_corr=0.7,
                               dd_tiers=DD_AGGR100, eq_method='streak',
                               regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                               hold=1, top_n=3, vol_mode='fixed')
    pr(r_base10_streak, "V167 BASELINE: atr<10%, corr=0.7, streak, aggro100, top_n=3")
    all_results.append({**r_base10_streak, 'label': 'V167_base_atr10_streak', 'section': 0,
                        'atr_norm_max': 10.0, 'eq_method': 'streak', 'max_corr': 0.7,
                        'top_n': 3, 'vol_mode': 'fixed', 'mkt_vol_pctile': None})

    # ===================== SECTION 1: ADAPTIVE VOL THRESHOLD =====================
    print("\n" + "=" * 130)
    print("  SECTION 1: ADAPTIVE VOL THRESHOLD")
    print("  Dynamic atr_norm_max based on MKT_VOL regime:")
    print("    Low vol (MKT_VOL < P50):    atr_max = 15%")
    print("    Normal (P50 < MKT_VOL < P75): atr_max = 12%")
    print("    High vol (MKT_VOL > P75):     atr_max = 8%")
    print("=" * 130)

    for eq_m in ['none', 'wr', 'streak']:
        label = f"ADAPTIVE vol, eq={eq_m:6s}"
        r = backtest(max_corr=0.7, dd_tiers=DD_AGGR100, eq_method=eq_m,
                     regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                     hold=1, top_n=3, vol_mode='adaptive')
        pr(r, label)
        all_results.append({**r, 'label': f'adaptive_{eq_m}', 'section': 1,
                            'atr_norm_max': 'adaptive', 'eq_method': eq_m,
                            'max_corr': 0.7, 'top_n': 3, 'vol_mode': 'adaptive',
                            'mkt_vol_pctile': None})

    # ===================== SECTION 2: FINE-TUNE atr_norm AROUND 12% =====================
    print("\n" + "=" * 130)
    print("  SECTION 2: FINE-TUNE atr_norm AROUND 12%")
    print("  Testing 11%, 12%, 13% with max_corr=0.7, no wr_mult")
    print("=" * 130)

    for an_max in [9.0, 10.0, 11.0, 12.0, 13.0, 14.0]:
        for eq_m in ['none', 'wr']:
            label = f"atr<{an_max:.0f}%, corr=0.7, eq={eq_m:4s}"
            r = backtest(atr_norm_max=an_max, max_corr=0.7,
                         dd_tiers=DD_AGGR100, eq_method=eq_m,
                         regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                         hold=1, top_n=3, vol_mode='fixed')
            pr(r, label)
            all_results.append({**r, 'label': f'atr{an_max:.0f}_{eq_m}', 'section': 2,
                                'atr_norm_max': an_max, 'eq_method': eq_m,
                                'max_corr': 0.7, 'top_n': 3, 'vol_mode': 'fixed',
                                'mkt_vol_pctile': None})

    # ===================== SECTION 3: ATR + MKT_VOL DOUBLE FILTER =====================
    print("\n" + "=" * 130)
    print("  SECTION 3: ATR + MKT_VOL DOUBLE FILTER")
    print("  Combining individual atr_norm with market-wide vol percentile gate")
    print("=" * 130)

    double_configs = [
        # (atr_norm_max, mkt_vol_pctile, description)
        (12.0, 75, "atr<12% AND MKT_VOL<P75"),
        (12.0, 90, "atr<12% AND MKT_VOL<P90"),
        (15.0, 75, "atr<15% AND MKT_VOL<P75"),
        (10.0, 75, "atr<10% AND MKT_VOL<P75"),
        (10.0, 90, "atr<10% AND MKT_VOL<P90"),
        (12.0, 50, "atr<12% AND MKT_VOL<P50"),
    ]

    for an_max, pctile, desc in double_configs:
        for eq_m in ['none', 'wr']:
            label = f"{desc}, eq={eq_m:4s}"
            r = backtest(atr_norm_max=an_max, max_corr=0.7,
                         dd_tiers=DD_AGGR100, eq_method=eq_m,
                         regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                         hold=1, top_n=3, vol_mode='double',
                         mkt_vol_pctile=pctile)
            pr(r, label)
            all_results.append({**r, 'label': f'double_atr{an_max:.0f}_pct{pctile}_{eq_m}',
                                'section': 3, 'atr_norm_max': an_max, 'eq_method': eq_m,
                                'max_corr': 0.7, 'top_n': 3, 'vol_mode': 'double',
                                'mkt_vol_pctile': pctile})

    # ===================== SECTION 4: max_corr SWEEP WITH VOL FILTER =====================
    print("\n" + "=" * 130)
    print("  SECTION 4: max_corr SWEEP WITH VOL FILTER")
    print("  Testing max_corr=[0.5, 0.6, 0.7, 0.8, 1.0] x atr_norm=[10, 12]")
    print("=" * 130)

    for mc in [0.5, 0.6, 0.7, 0.8, 1.0]:
        for an_max in [10.0, 12.0]:
            for eq_m in ['none', 'wr']:
                label = f"corr={mc:.1f}, atr<{an_max:.0f}%, eq={eq_m:4s}"
                r = backtest(atr_norm_max=an_max, max_corr=mc,
                             dd_tiers=DD_AGGR100, eq_method=eq_m,
                             regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                             hold=1, top_n=3, vol_mode='fixed')
                pr(r, label)
                all_results.append({**r, 'label': f'corr{mc:.1f}_atr{an_max:.0f}_{eq_m}',
                                    'section': 4, 'atr_norm_max': an_max, 'eq_method': eq_m,
                                    'max_corr': mc, 'top_n': 3, 'vol_mode': 'fixed',
                                    'mkt_vol_pctile': None})

    # ===================== SECTION 5: WR-ADAPTIVE vs NO WR CONFIRMATION =====================
    print("\n" + "=" * 130)
    print("  SECTION 5: WR-ADAPTIVE vs NO WR CONFIRMATION")
    print("  Side-by-side comparison across vol filter settings")
    print("=" * 130)
    ...
    for an_max, mc, vm, pctile in wr_configs:
        r_nowr = backtest(atr_norm_max=an_max, max_corr=mc,
                          dd_tiers=DD_AGGR100, eq_method='none',
                          regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                          hold=1, top_n=3, vol_mode=vm, mkt_vol_pctile=pctile)
        r_wr = backtest(atr_norm_max=an_max, max_corr=mc,
                        dd_tiers=DD_AGGR100, eq_method='wr',
                        regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                        hold=1, top_n=3, vol_mode=vm, mkt_vol_pctile=pctile)
        label_base = f"atr<{an_max:.0f}%, corr={mc:.1f}"
        pr(r_nowr, f"NO_WR  {label_base}")
        pr(r_wr,   f"WR     {label_base}")

        delta_ann = r_wr['ann'] - r_nowr['ann']
        delta_mdd = r_wr['mdd'] - r_nowr['mdd']
        rm_nowr = abs(r_nowr['ann'] / r_nowr['mdd']) if r_nowr['mdd'] != 0 else 0
        rm_wr = abs(r_wr['ann'] / r_wr['mdd']) if r_wr['mdd'] != 0 else 0
        print(f"    --> Delta: Ann={delta_ann:+.1f}%, MDD={delta_mdd:+.1f}%, R/M: {rm_nowr:.2f} vs {rm_wr:.2f}")

    # ===================== SECTION 6: top_n=4 WITH VOL FILTER =====================
    print("\n" + "=" * 130)
    print("  SECTION 6: top_n=4 WITH VOL FILTER")
    print("  More positions when vol filter selects higher quality entries")
    print("=" * 130)
    ...
    for an_max in [10.0, 12.0, 14.0]:
        for mc in [0.7, 0.8]:
            for eq_m in ['none', 'wr']:
                for tn in [3, 4]:
                    label = f"top_n={tn}, atr<{an_max:.0f}%, corr={mc:.1f}, eq={eq_m:4s}"
                    r = backtest(atr_norm_max=an_max, max_corr=mc,
                                 dd_tiers=DD_AGGR100, eq_method=eq_m,
                                 regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                                 hold=1, top_n=tn, vol_mode='fixed')
                    pr(r, label)
                    all_results.append({**r, 'label': f'top{tn}_atr{an_max:.0f}_c{mc:.1f}_{eq_m}',
                                        'section': 6, 'atr_norm_max': an_max, 'eq_method': eq_m,
                                        'max_corr': mc, 'top_n': tn, 'vol_mode': 'fixed',
                                        'mkt_vol_pctile': None})

    # ===================== SECTION 7: RANKED RESULTS =====================
    print("\n" + "=" * 130)
    print("  SECTION 7: ALL CONFIGS RANKED BY ANNUAL RETURN (full period)")
    print("=" * 130)
    ...
    print("\n" + "=" * 130)
    print("  SECTION 7b: ALL CONFIGS RANKED BY R/M RATIO (risk-adjusted)")
    print("=" * 130)
    ...
    # ===================== SECTION 8: WALK-FORWARD TOP CONFIGS =====================
    print("\n" + "=" * 130)
    ...
    for r in wf_candidates[:20]:
        lbl = r['label']
        wf_kwargs = {
            'atr_norm_max': r['atr_norm_max'] if r['atr_norm_max'] != 'adaptive' else 12.0,
            'max_corr': r['max_corr'],
            'dd_tiers': DD_AGGR100,
            ...
            'vol_mode': r['vol_mode'],
            'mkt_vol_pctile': r.get('mkt_vol_pctile'),
        }
        # Override for adaptive mode
        if r['vol_mode'] == 'adaptive':
            wf_kwargs['atr_norm_max'] = 12.0  # doesn't matter for adaptive, but needs a value
    ...
    # ===================== SECTION 11: BEST PER CATEGORY =====================
    print("\n" + "=" * 130)
    print("  SECTION 11: BEST PER CATEGORY")
    print("=" * 130)

    categories = {
        'Adaptive Vol': lambda x: x['vol_mode'] == 'adaptive',
        'Fixed Vol': lambda x: x['vol_mode'] == 'fixed',
        'Double Filter': lambda x: x['vol_mode'] == 'double',
        'WR-adaptive': lambda x: x['eq_method'] == 'wr',
        'No WR': lambda x: x['eq_method'] == 'none',
        'Streak': lambda x: x['eq_method'] == 'streak',
        'top_n=4': lambda x: x['top_n'] == 4,
        'top_n=3': lambda x: x['top_n'] == 3,
        'corr<=0.6': lambda x: x['max_corr'] <= 0.6,
        'corr=0.7': lambda x: x['max_corr'] == 0.7,
        'corr>=0.8': lambda x: x['max_corr'] >= 0.8,
    }
    ...
    # ===================== SECTION 12: DELTA vs V167 BASELINE =====================
    print("\n" + "=" * 130)
    print("  SECTION 12: IMPROVEMENT vs V167 BASELINE (atr<12%, no wr, corr=0.7, top_n=3)")
    print("=" * 130)

    base_lbl = 'V167_base_atr12_nowr'
    if base_lbl in wf_all:
        ...
        b_wmdd = min(r['mdd'] for r in b_wf.values())
        b_rm = abs(b_avg / b_wmdd) if b_wmdd != 0 else 0
        print_wf(b_wf, "V167 BASELINE (computed)")

    print(f"\n  V167 BASELINE: WF AVG={b_avg:+.0f}% | WorstMDD={b_wmdd:.0f}% | R/M={b_rm:.2f}")
    ...
    print(f"\n  Configs by R/M improvement over V167 baseline:")
    ...
    print(f"\n  V167 Baseline (atr<12%, corr=0.7, no wr, top_n=3):")
    ...
    if wf_ra:
        ...
        print(f"    R/M improvement over V167: {delta_rm_best:+.2f}")
        ...
        for sec in sorted(sections_tested):
            ...
            s = sec_items[0]
            sec_names = {0: "V167 Baseline", 1: "Adaptive Vol", 2: "Fine-tune ATR",
                         3: "Double Filter", 4: "Corr Sweep", 6: "top_n=4"}
