"""
V74: V61 + Adaptive Exit Timing
================================
V61 is ALL-TIME BEST: Sharpe 6.71, MDD 5.5%, ann +15.7%, 78 trades.
V74 tests whether adaptive exits improve results over V61's fixed hold period.

Instead of always exiting at a fixed hold period, V74 uses signal-based exit:
  - Exit early (at next open) if composite rank drops below exit_early_rank
    (signal reversed significantly -> cut losers fast)
  - Hold extended if composite rank stays above hold_extended_rank
    (strong signal persists -> ride winners longer)
  - Default: exit after max_hold_days (safety cap)

Parameter sweep:
  - exit_early_rank: 0.20, 0.30, 0.40
  - hold_extended_rank: 0.65, 0.70, 0.75
  - max_hold_days: 2, 3
  - max_per_sector: 1, 2

Walk-forward 2019-2026. Report full 10yr results for best.
...
# Short-term (5d) factor weights -- same as V61/V47
ST_WEIGHTS = {
    "rank_ret5d": 0.25,
    "rank_oi5d": 0.20,
    "rank_rsi": 0.15,
...
def compute_raw_factors(
    ...
    t0 = time.time()
    print("[V74] Computing raw factors (5d + 20d)...", flush=True)
...
def compute_cross_sectional_ranks(
    ...
    t0 = time.time()
    print("[V74] Computing cross-sectional ranks...", flush=True)
...
def build_multi_tf_composite(
    ...
    t0 = time.time()
    print(f"[V74] Building multi-TF composite (st_w={st_weight:.2f})...",
          flush=True)
...
def compute_all_signals(
    ...
    """Full signal pipeline for V74 (same as V61)."""
...
def backtest_v74(
    ...
    # V74 NEW: adaptive exit parameters
    exit_early_rank: float = 0.30,
    hold_extended_rank: float = 0.70,
    max_hold_days: int = 2,
    pyramid_day: int = 1,
    start_di: int = 60,
    end_di: Optional[int] = None,
) -> Tuple[List[dict], float, float]:
    """Backtest V74: multi-TF composite + dynamic threshold + sector limits
    + ADAPTIVE EXIT TIMING.

    Exit logic (evaluated at each day's close):
      1. ATR stop-loss hit -> exit at close
      2. Composite rank < exit_early_rank -> signal reversed, exit at next open
      3. Composite rank >= hold_extended_rank AND hold < max_hold_days
         -> signal persists, hold another day
      4. hold >= max_hold_days -> max hold reached, exit at close
      5. Otherwise -> exit at close (default 1-day hold)
    """
    ...
    for di in range(max(start_di, 1), end_di):
        ...
        for si, pos_list in pos_by_si.items():
            ...
            # Get current composite rank for this symbol
            current_rank = composite[si, di]
            rank_available = not np.isnan(current_rank)

            if stopped:
                # ATR stop hit -> exit at close
                for edi, ep, sp, alloc, is_pyr in pos_list:
                    pnl = (c - ep) / ep - COMM
            ...
            elif rank_available and current_rank < exit_early_rank:
                # Signal reversed significantly -> exit early at next open
                # But we need next open; if not available, exit at close
                if di + 1 < ND and not np.isnan(O[si, di + 1]):
                    exit_price = O[si, di + 1]
                else:
                    exit_price = c
                for edi, ep, sp, alloc, is_pyr in pos_list:
                    pnl = (exit_price - ep) / ep - COMM
                    profit = equity * alloc * pnl
                    daily_pnl += profit
                    is_win = pnl > 0
                    trades.append({
            ...
            elif rank_available and current_rank >= hold_extended_rank and hold < max_hold_days:
                # Strong signal persists -> hold extended
                for edi, ep, sp, alloc, is_pyr in pos_list:
                    new_positions.append(
                        (si, edi, ep, sp, alloc, is_pyr))
            ...
            elif rank_available and current_rank < hold_extended_rank and hold >= 1:
                # Signal weakened (not reversed but not strong enough to extend)
                # -> exit at close (default 1-day hold behavior)
                for edi, ep, sp, alloc, is_pyr in pos_list:
                    pnl = (c - ep) / ep - COMM
                    profit = equity * alloc * pnl
                    daily_pnl += profit
                ...
                # No rank available or first day of hold -> keep position
                for edi, ep, sp, alloc, is_pyr in pos_list:
                    new_positions.append(
                        (si, edi, ep, sp, alloc, is_pyr))
...
def analyze(trades: List[dict], equity: float, max_dd: float,
    ...
    n_weak = sum(1 for t in trades if t["reason"] == "signal_weak")
    ...
    print(
        ...
        f"stop:{n_stop} early:{n_early} weak:{n_weak} maxhold:{n_hold}) "
...
def walk_forward(
    ...
    print(
        ...
        f"eer={exit_early_rank:.2f} her={hold_extended_rank:.2f} "
        f"mhd={max_hold_days})"
    )
    print(f"{'=' * 70}")
    ...
    for test_year in range(2019, years[-1] + 1):
        ...
        trades, _, _ = backtest_v74(
            ...
            min_confidence=3,
            use_ker_gate=True,
            exit_early_rank=exit_early_rank,
            hold_extended_rank=hold_extended_rank,
    ...
    if all_trades:
        ...
        exit_totals: Dict[str, int] = defaultdict(int)
        for t in all_trades:
            exit_totals[t["reason"]] += 1
        exit_str = " ".join(
            f"{k}:{v}" for k, v in sorted(exit_totals.items()))
        ...
        print(f"  WF EXIT REASONS: {exit_str}")
...
def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("  V74: V61 + ADAPTIVE EXIT TIMING")
    print("  V61 ALL-TIME BEST: Sharpe 6.71, MDD 5.5%, ann +15.7%, 78 trades")
    print("  Adaptive exits: early cut (rank < threshold) + extended hold (rank strong)")
    print("=" * 70)
    ...
    # === 2. Parameter sweep with adaptive exit params ===
    print("\n" + "=" * 70)
    print("  PARAMETER SWEEP (2019-2026) -- ADAPTIVE EXIT")
    print("=" * 70)

    results: List[dict] = []

    # V74 adaptive exit parameter sweep
    exit_early_ranks = [0.20, 0.30, 0.40]
    hold_extended_ranks = [0.65, 0.70, 0.75]
    max_hold_days_list = [2, 3]
    max_per_sector_list = [1, 2]

    # V61 base params (reduced sweep to keep runtime manageable)
    st_weights_list = [0.55, 0.60, 0.65]
    win_thresholds = [0.55, 0.60]
    normal_thresholds = [0.80, 0.82, 0.85]
    lose_thresholds = [0.88, 0.90]
    win_rate_windows = [10, 15]
    top_n_winning_list = [2, 3]

    total_configs = (
        len(st_weights_list)
        * len(win_thresholds)
        * len(normal_thresholds)
        * len(lose_thresholds)
        * len(win_rate_windows)
        * len(max_per_sector_list)
        * len(top_n_winning_list)
        * len(exit_early_ranks)
        * len(hold_extended_ranks)
        * len(max_hold_days_list)
    )
    print(f"  Total configs to evaluate: ~{total_configs}")

    config_count = 0
    for st_w in st_weights_list:
        sigs = signal_cache[st_w]
        for eer in exit_early_ranks:
            for her in hold_extended_ranks:
                # her must be > eer for the logic to make sense
                if her <= eer:
                    continue
                for mhd in max_hold_days_list:
                    for mps in max_per_sector_list:
                        for tnw in top_n_winning_list:
                            for wt in win_thresholds:
                                for nt in normal_thresholds:
                                    for lt in lose_thresholds:
                                        if lt <= nt:
                                            continue
                                        for ww in win_rate_windows:
                                            config_count += 1
                                            trades, eq, dd = backtest_v74(
                                                C, O, H, L, NS, ND,
                                                dates, syms, sigs,
                                                sector_lookup=sector_lookup,
                                                st_weight=st_w,
    ...
    for r in results[:40]:
        print(
            ...
            f"{r['tnw']:>4} "
            f"{r['eer']:>4.2f} {r['her']:>4.2f} {r['mhd']:>3} "
            f"{r['n']:>5} {r['wr']:>5.1f} {r['ann']:>+8.1f} "
            f"{r['dd']:>6.1f} {r['sharpe']:>6.2f} "
            f"{r['avg_yr']:>7.1f}"
        )

    # Print exit reason distribution for top configs
    if results:
        print("\n  TOP 10 EXIT REASON DISTRIBUTIONS:")
        for i, r in enumerate(results[:10]):
            exit_str = " ".join(
                f"{k}:{v}" for k, v in sorted(r["exits"].items()))
            print(
                f"  #{i+1} Sh={r['sharpe']:.2f}: {exit_str}"
            )
    ...
    for r in unique_top:
        ...
        trades, eq, dd = backtest_v74(
            ...
            exit_early_rank=r["eer"],
            hold_extended_rank=r["her"],
            max_hold_days=r["mhd"],
            start_di=60,
        )
        label = (
            ...
            f"eer={r['eer']:.2f} her={r['her']:.2f} mhd={r['mhd']}"
        )
        print(f"\n  FULL {label}")
    ...
    if results:
        ...
        print(
            ...
            f"eer={best['eer']:.2f} her={best['her']:.2f} "
            f"mhd={best['mhd']}"
        )
        print("=" * 70)
        walk_forward(
            ...
            exit_early_rank=best["eer"],
            hold_extended_rank=best["her"],
            max_hold_days=best["mhd"],
        )

        # === 5. V74 vs V61 comparison ===
        print("\n" + "=" * 70)
        print("  COMPARISON: V74 (adaptive exit) vs V61 baseline")
        print("  (2019-2026 OOS)")
        print("=" * 70)

        # V74 best (adaptive exit)
        trades_v74, eq_v74, dd_v74 = backtest_v74(
            C, O, H, L, NS, ND, dates, syms,
            signal_cache[best["st_w"]],
            sector_lookup=sector_lookup,
            st_weight=best["st_w"],
        ...
        # V61 baseline: same params but with V61's fixed hold_days=5
        # Simulate by using max_hold_days=5 and extreme thresholds
        # so adaptive exit never triggers (early rank=0, extended rank=1.0)
        trades_v61, eq_v61, dd_v61 = backtest_v74(
            C, O, H, L, NS, ND, dates, syms,
            signal_cache[best["st_w"]],
            sector_lookup=sector_lookup,
            st_weight=best["st_w"],
            ...
            top_n_winning=best["tnw"],
            max_per_sector=best["mps"],
            exit_early_rank=0.0,  # never trigger early exit
            hold_extended_rank=1.01,  # never extend
            max_hold_days=5,  # V61's fixed hold period
            start_di=bt_2019,
        )

        print(f"\n  V74 ADAPTIVE EXIT:")
        analyze(trades_v74, eq_v74, dd_v74, "V74-adaptive")
        print(f"\n  V61 BASELINE (fixed 5d hold):")
        analyze(trades_v61, eq_v61, dd_v61, "V61-baseline")

        if trades_v74 and trades_v61:
            print(
                f"\n  V74 vs V61: "
                f"eq_delta={eq_v74 - eq_v61:+,.0f} "
                f"dd_delta={dd_v74 - dd_v61:+.1f}%"
            )

    print(f"\n[V74] Done. {time.time() - t0:.1f}s")
