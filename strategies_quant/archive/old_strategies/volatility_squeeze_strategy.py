"""
波动率压缩振荡器策略 (Volatility Squeeze Oscillator Strategy)
============================================================
ATR压缩比率 + 累积Delta代理 + 量能RSI多层确认。

来源: TradingView "Volatility Squeeze Oscillator"

核心逻辑:
  1. ATR压缩比率检测低波动
  2. 累积Delta方向(净买卖量代理)
  3. Volume RSI确认需求
  4. 波动率扩张入场
  5. ATR追踪止损

技术指标: ATR, Cumulative Delta Proxy, Volume RSI
"""
import numpy as np
import pandas as pd
from core.base_strategy import BaseStrategy


class VolatilitySqueezeStrategy(BaseStrategy):
    """波动率压缩振荡器策略 — ATR压缩 + Delta方向 + 量能RSI"""

    strategy_description = "波动率压缩: ATR压缩比率 + Delta代理 + Volume RSI"
    strategy_category = "momentum"
    strategy_params_schema = {
        "atr_period": {"type": "int", "default": 14, "label": "ATR周期"},
        "squeeze_period": {"type": "int", "default": 20, "label": "压缩判定周期"},
        "vol_rsi_period": {"type": "int", "default": 14, "label": "Volume RSI周期"},
        "hold_min": {"type": "int", "default": 3, "label": "最少持仓天数"},
        "trail_atr_mult": {"type": "float", "default": 2.5, "label": "追踪止损ATR倍数"},
    }

    def __init__(self, data, params):
        super().__init__(data, params)
        self.atr_period = params.get('atr_period', 14)
        self.squeeze_period = params.get('squeeze_period', 20)
        self.vol_rsi_period = params.get('vol_rsi_period', 14)
        self.hold_min = params.get('hold_min', 3)
        self.trail_atr_mult = params.get('trail_atr_mult', 2.5)

    def get_default_params(self):
        return {
            'atr_period': 14, 'squeeze_period': 20, 'vol_rsi_period': 14,
            'hold_min': 3, 'trail_atr_mult': 2.5,
        }

    def generate_signals(self):
        data = self.data.copy()
        if 'symbol' not in data.columns:
            data['symbol'] = 'DEFAULT'

        unique_times = sorted(data.index.unique())
        self.signals = []
        current_holding = None
        buy_time = None
        position_dir = 0
        high_water = 0.0
        low_water = float('inf')

        for current_time in unique_times:
            current_bars = data.loc[current_time]
            if isinstance(current_bars, pd.Series):
                current_bars = pd.DataFrame([current_bars])

            if current_holding is None:
                best_score = 0
                best_sym = None
                best_dir = 0

                for _, bar in current_bars.iterrows():
                    sym = bar['symbol']
                    hist = data[(data['symbol'] == sym) & (data.index < current_time)]
                    result = self._evaluate(hist)
                    if result is None:
                        continue
                    score, direction, _ = result
                    if abs(score) > abs(best_score):
                        best_score = score
                        best_sym = sym
                        best_dir = direction

                if best_sym and abs(best_score) >= 3:
                    if best_dir == 1:
                        self._record_signal(current_time, 'buy', best_sym)
                        position_dir = 1
                    else:
                        self._record_signal(current_time, 'sell', best_sym)
                        position_dir = -1
                    current_holding = best_sym
                    buy_time = current_time
                    high_water = 0.0
                    low_water = float('inf')

            else:
                days_held = len([t for t in unique_times if buy_time < t <= current_time])
                bar_data = current_bars[current_bars['symbol'] == current_holding]
                if len(bar_data) == 0:
                    continue
                current_price = float(bar_data.iloc[0]['close'])

                if position_dir == 1:
                    high_water = max(high_water, current_price) if high_water > 0 else current_price
                else:
                    low_water = min(low_water, current_price) if low_water < float('inf') else current_price

                if days_held >= self.hold_min:
                    hist = data[(data['symbol'] == current_holding) & (data.index < current_time)]
                    atr_val = self._calc_atr(hist)
                    should_exit = False

                    if atr_val > 0:
                        if position_dir == 1 and high_water > 0:
                            if current_price < high_water - self.trail_atr_mult * atr_val:
                                should_exit = True
                        elif position_dir == -1 and low_water < float('inf'):
                            if current_price > low_water + self.trail_atr_mult * atr_val:
                                should_exit = True

                    if days_held >= 60:
                        should_exit = True

                    if not should_exit:
                        result = self._evaluate(hist)
                        if result is not None:
                            score, direction, _ = result
                            if position_dir == 1 and direction == -1 and score < -3:
                                should_exit = True
                            elif position_dir == -1 and direction == 1 and score > 3:
                                should_exit = True

                    if should_exit:
                        if position_dir == 1:
                            self._record_signal(current_time, 'sell', current_holding)
                        else:
                            self._record_signal(current_time, 'buy', current_holding)
                        current_holding = None
                        buy_time = None
                        position_dir = 0
                        high_water = 0.0
                        low_water = float('inf')

        print(f"VolatilitySqueeze: 生成 {len(self.signals)} 个信号")
        return self.signals

    def _calc_atr_series(self, data):
        """Calculate ATR series"""
        high = data['high'].values
        low = data['low'].values
        close = data['close'].values
        n = len(close)
        if n < self.atr_period + 1:
            return np.array([])
        tr = np.maximum(high[1:] - low[1:],
                        np.maximum(np.abs(high[1:] - close[:-1]), np.abs(low[1:] - close[:-1])))
        result = np.zeros(n)
        for i in range(self.atr_period, n):
            result[i] = np.mean(tr[i - self.atr_period:i])
        return result

    def _calc_cumulative_delta(self, data):
        """Approximate cumulative volume delta"""
        close = data['close'].values
        vol_col = 'vol' if 'vol' in data.columns else ('volume' if 'volume' in data.columns else None)
        if vol_col is None:
            return 0
        vol = data[vol_col].values
        n = len(vol)
        if n < 2:
            return 0
        # Approximate delta: up bars = buy, down bars = sell
        delta = np.where(close[1:] > close[:-1], vol[1:], -vol[1:])
        # Use recent sum
        lookback = min(self.squeeze_period, len(delta))
        return np.sum(delta[-lookback:])

    def _calc_volume_rsi(self, data):
        """Volume-weighted RSI"""
        close = data['close'].values
        vol_col = 'vol' if 'vol' in data.columns else ('volume' if 'volume' in data.columns else None)
        if vol_col is None:
            return 50
        vol = data[vol_col].values
        n = len(vol)
        if n < self.vol_rsi_period + 1:
            return 50
        up_vol = np.where(close[1:] > close[:-1], vol[1:], 0)
        down_vol = np.where(close[1:] < close[:-1], vol[1:], 0)
        avg_up = np.mean(up_vol[-self.vol_rsi_period:])
        avg_down = np.mean(down_vol[-self.vol_rsi_period:])
        if avg_down == 0:
            return 100
        rs = avg_up / avg_down
        return 100 - 100 / (1 + rs)

    def _evaluate(self, data):
        if len(data) < self.squeeze_period + self.atr_period + 10:
            return None

        close = data['close'].values
        n = len(close)
        score = 0

        # 1. ATR compression ratio
        atr_series = self._calc_atr_series(data)
        if len(atr_series) > self.squeeze_period:
            recent_atr = atr_series[-1]
            hist_atr = np.mean(atr_series[-self.squeeze_period:])
            if hist_atr > 0:
                compression = recent_atr / hist_atr
                if compression < 0.7:
                    # Squeeze detected — expansion coming
                    score += 2  # Direction to be confirmed

        # 2. Cumulative Delta direction
        cum_delta = self._calc_cumulative_delta(data)
        vol_col = 'vol' if 'vol' in data.columns else ('volume' if 'volume' in data.columns else None)
        if vol_col is not None:
            vol = data[vol_col].values
            recent_vol = np.sum(vol[-self.squeeze_period:])
            if recent_vol > 0:
                norm_delta = cum_delta / recent_vol
                if norm_delta > 0.1:
                    score += 3
                elif norm_delta > 0.03:
                    score += 1
                elif norm_delta < -0.1:
                    score -= 3
                elif norm_delta < -0.03:
                    score -= 1

        # 3. Volume RSI confirmation
        vol_rsi = self._calc_volume_rsi(data)
        if vol_rsi > 60 and score > 0:
            score += 2
        elif vol_rsi < 40 and score < 0:
            score += 2
        elif vol_rsi < 40 and score > 0:
            score -= 1
        elif vol_rsi > 60 and score < 0:
            score -= 1

        # 4. Price momentum confirmation
        if n >= 10:
            ret = (close[-1] / close[-10] - 1) * 100
            if score > 0 and ret > 1:
                score += 1
            elif score < 0 and ret < -1:
                score += 1

        direction = 1 if score > 0 else -1
        atr = self._calc_atr(data)
        return score, direction, atr

    def _calc_atr(self, data):
        if len(data) < self.atr_period + 1:
            return 0
        high = data['high'].values
        low = data['low'].values
        close = data['close'].values
        tr = np.maximum(high[1:] - low[1:],
                        np.maximum(np.abs(high[1:] - close[:-1]), np.abs(low[1:] - close[:-1])))
        return np.mean(tr[-self.atr_period:])

    def screen(self):
        data = self.data.copy()
        if len(data) < 60:
            return {'action': 'hold', 'reason': '数据不足', 'price': float(data['close'].iloc[-1])}

        result = self._evaluate(data)
        price = float(data['close'].iloc[-1])

        if result is None:
            return {'action': 'hold', 'reason': '评估失败', 'price': price}

        score, direction, _ = result
        if abs(score) >= 3:
            return {
                'action': 'buy' if direction == 1 else 'sell',
                'reason': f"score={score} (vol_squeeze)",
                'price': price,
            }
        return {'action': 'hold', 'reason': f'score={score}', 'price': price}
