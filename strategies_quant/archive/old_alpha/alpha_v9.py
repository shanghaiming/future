...
def compute_v9_factors(NS, ND, C, O, H, L, V):
    ...
    # === 1. HURST: Single-scale R/S Hurst exponent (optimized) ===
    # H > 0.5 = trending (persistent), H < 0.5 = mean-reverting
    # Precompute log returns once, then use single-window R/S for speed
    HURST = np.full((NS, ND), np.nan)
    hurst_window = 100
    # Precompute log returns for all stocks
    ...
    for si in range(NS):
        ...
        for di in range(hurst_window + 2, ND):
            d = di - 1
            window = rets[d - hurst_window + 1:d + 1]  # 100 returns up to d
            valid = window[~np.isnan(window)]
            n = len(valid)
            if n < 50:
                continue
            # Single-scale R/S on the full window
            mean_r = np.mean(valid)
            cumdev = np.cumsum(valid - mean_r)
            R = np.max(cumdev) - np.min(cumdev)
            S = np.std(valid, ddof=1)
            if S > 0 and n > 1:
                HURST[si, di] = float(np.clip(np.log(R / S) / np.log(n), 0.0, 1.0))
    ...
    # === 3. BB_SQUEEZE: Bollinger Band width / Keltner Channel width (vectorized) ===
    ...
    # Precompute True Range (vectorized over all stocks)
    C_shifted = np.roll(C, 1, axis=1)
    C_shifted[:, 0] = np.nan
    with np.errstate(invalid='ignore'):
        TR = np.maximum(H - L, np.maximum(np.abs(H - C_shifted), np.abs(L - C_shifted)))
    TR = np.where(np.isnan(C_shifted), np.nan, TR)
    ...
    for si in range(NS):
        ...
        for di in range(bb_period + atr_period + 1, ND):
            d = di - 1
            c_win = closes[d - bb_period + 1:d + 1]
            if np.any(np.isnan(c_win)):
                continue
            bb_std = np.std(c_win, ddof=1)
            if bb_std <= 0:
                continue
            bb_width = 2.0 * bb_mult * bb_std

            atr_win = tr_vals[d - atr_period + 1:d + 1]
            valid_atr = atr_win[~np.isnan(atr_win)]
            if len(valid_atr) < atr_period // 2:
                continue
            atr = np.mean(valid_atr)
            kc_width = 2.0 * kc_mult * atr

            if kc_width > 0:
                BB_SQUEEZE[si, di] = bb_width / kc_width
