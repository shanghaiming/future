...
# ============================================================
def load_all_data(max_stocks=500, min_days=300, start='2016-01-01', end=None, load_oi=False):
    """Load OHLCV data for top stocks by volume.
    If load_oi=True, also load OI (open interest) from CSV files.
    """
    ...
    for sym in list_available_symbols('daily'):
        try:
            ...
            if df is not None and len(df) >= min_days:
                cols = [c for c in ['open','high','low','close','vol','volume','amount','oi'] if c in df.columns]
    ...
    print(f"  {NS} stocks, {ND} days ({time.time()-t0:.1f}s) OI={'yes' if has_oi else 'no'}", flush=True)
    if load_oi:
        return NS, ND, dates, C, O, H, L, V, OI, syms, sym_set
