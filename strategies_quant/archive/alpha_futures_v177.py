"""
Alpha Futures V177 — Dual-Side (Long+Short) V121 Strategy
==============================================================================
V169 champion: +253%/-15% WF, R/M=16.91 (best risk-adjusted). Long-only.

V177 tests BIDIRECTIONAL trading — adding short signals as mirror of V121:
  LONG signal:  ROC(5)>1% + Z>1.5 + ROC improving (same as V121)
  SHORT signal: ROC(5)<-1% + Z<-1.5 + ROC declining (inverse)

Key hypothesis: In downtrends, shorting provides extra returns + natural hedge.
Previous versions only tested long, leaving half the opportunity on the table.

Test matrix:
  short_mode: [long_only_baseline, short_mirror, short_aggressive,
               short_selective, short_alternating]
  atr_norm_max: [10, 12]
  max_corr: [0.5, 0.7]
  top_n: [3, 4]
...
Key insight: OI - all modes hurt R/M. V177 moves to a completely new direction.
"""
...
def main():
    ...
    # SHORT: Mirror of V121 for short selling
    def sig_v121_short(di, edi):
        ...
        for s in range(NS):
            ...
            if np.isnan(roc) or np.isnan(zs) or roc >= -1.0 or zs >= -1.5: continue
            rp = ROC5[s, di-1] if di > 0 else np.nan
            if not np.isnan(rp) and roc >= rp: continue  # ROC declining (getting more negative)
            ep = O[s, edi]
            if np.isnan(ep) or ep <= 0: continue
        ...
        for di in range(start_di, end_di - 1):
            ...
            for p in positions:
                if di - p['entry_di'] >= p['hold_days']:
                    ...
                    else:  # short: buy back at exit price
                        margin = p['entry_price'] * m * abs(p['lots'])
            ...
            if best_v121 and best_union:
                ...
                entries.append((best_v121[0], best_v121[1], best_v121[2], 'v121', pos_size, 1))
            elif best_union:
                entries.append((best_union[0], best_union[1], best_union[2], 'union', pos_size, 1))

            # SHORT SIGNALS (when short_mode enabled)
            if short_mode != 'long_only' and len(positions) < top_n:
                ...
                if best_short and (short_mode in ('short_mirror', 'short_aggressive', 'short_alternating')):
                    # In alternating mode, only short when no long entry
                    if short_mode == 'short_alternating' and entries:
                        pass  # skip short if we have long entries
                    elif short_mode == 'short_selective':
                        # Only short when ROC5 < -3% (stronger signal)
                        if ROC5[best_short[1], di] < -3.0:
            ...
            # Also add dir=1 to existing long entries
            entries_with_dir = []
            for e in entries:
                if len(e) == 5:
                    entries_with_dir.append((*e, 1))
                else:
                    entries_with_dir.append(e)

            cash_snapshot = cash
            n_planned = len(entries_with_dir)
            for sc, s, pr, sig_str, pct, d in entries_with_dir:
                ...
                if d == 1:  # long
                    ...
                    cash -= ci
                else:  # short: lock up margin (entry price * mult)
                    ct = max(1, int(cap / (pr * m * (1 + COMM))))
                    ci = pr * m * ct * (1 + COMM)
                    if ci > cash:
                        ct = int(cash * 0.9 / (pr * m * (1 + COMM)))
    ...
    # ===================== V177: DUAL-SIDE (LONG+SHORT) TESTS =====================

    all_results = []

    # --- BASELINE: Long-only V169 ---
    print("\n" + "=" * 130)
    print("  BASELINE: Long-only (V169 champion)")
    print("=" * 130)

    r_base = backtest(atr_norm_max=12.0, max_corr=0.5,
                      dd_tiers=DD_AGGR100, eq_method='none',
                      regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                      hold=1, top_n=3, vol_mode='fixed', short_mode='long_only')
    pr(r_base, "BASELINE: Long-only atr<12%, corr=0.5, top_n=3")
    all_results.append({**r_base, 'label': 'baseline_long_only', 'short_mode': 'long_only',
                        'atr_norm_max': 12.0, 'max_corr': 0.5, 'top_n': 3})

    # --- SECTION 1: Short modes with V169 config ---
    print("\n" + "=" * 130)
    print("  SECTION 1: Short Modes (atr<12%, corr=0.5, top_n=3)")
    print("=" * 130)

    for sm in ['short_mirror', 'short_aggressive', 'short_selective', 'short_alternating']:
        r = backtest(atr_norm_max=12.0, max_corr=0.5,
                     dd_tiers=DD_AGGR100, eq_method='none',
                     regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                     hold=1, top_n=3, vol_mode='fixed', short_mode=sm)
        pr(r, f"SHORT={sm} (atr<12%, c=0.5)")
        all_results.append({**r, 'label': f'atr12_c05_{sm}', 'short_mode': sm,
                            'atr_norm_max': 12.0, 'max_corr': 0.5, 'top_n': 3})

    # --- SECTION 2: Short with corr=0.7 ---
    print("\n" + "=" * 130)
    print("  SECTION 2: Short (atr<12%, corr=0.7, top_n=3)")
    print("=" * 130)
    ...
    for sm in ['short_mirror', 'short_aggressive', 'short_selective', 'short_alternating']:
        r = backtest(atr_norm_max=12.0, max_corr=0.7,
                     ...
                     regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                     hold=1, top_n=3, vol_mode='fixed', short_mode=sm)
        pr(r, f"SHORT={sm} (atr<12%, c=0.7)")
        all_results.append({**r, 'label': f'atr12_c07_{sm}', 'short_mode': sm,
                            'atr_norm_max': 12.0, 'max_corr': 0.7, 'top_n': 3})

    # --- SECTION 3: Short with atr<10% ---
    print("\n" + "=" * 130)
    print("  SECTION 3: Short (atr<10%, corr=0.5, top_n=3)")
    print("=" * 130)

    for sm in ['short_mirror', 'short_aggressive']:
        r = backtest(atr_norm_max=10.0, max_corr=0.5,
                     dd_tiers=DD_AGGR100, eq_method='none',
                     regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                     hold=1, top_n=3, vol_mode='fixed', short_mode=sm)
        pr(r, f"SHORT={sm} (atr<10%, c=0.5)")
        all_results.append({**r, 'label': f'atr10_c05_{sm}', 'short_mode': sm,
                            'atr_norm_max': 10.0, 'max_corr': 0.5, 'top_n': 3})

    # --- SECTION 4: Short with top_n=4 ---
    print("\n" + "=" * 130)
    print("  SECTION 4: Short (atr<12%, corr=0.7, top_n=4)")
    print("=" * 130)
    ...
    for sm in ['short_mirror', 'short_aggressive']:
        r = backtest(atr_norm_max=12.0, max_corr=0.7,
                     dd_tiers=DD_AGGR100, eq_method='none',
                     regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                     hold=1, top_n=4, vol_mode='fixed', short_mode=sm)
        pr(r, f"SHORT={sm} (atr<12%, c=0.7, top4)")
        all_results.append({**r, 'label': f'atr12_c07_top4_{sm}', 'short_mode': sm,
    ...
    # WF baseline
    walk_forward(label="BASELINE WF", atr_norm_max=12.0, max_corr=0.5,
                 dd_tiers=DD_AGGR100, eq_method='none', regime_lo=0.5,
                 regime_hi=1.5, sl_pct=0.0, hold=1, top_n=3,
                 vol_mode='fixed', short_mode='long_only')

    # Top 3 by R/M
    ranked = sorted(all_results, key=lambda x: abs(x.get('ann', 0) / x.get('mdd', -1)), reverse=True)
    top3 = [r for r in ranked if r.get('short_mode') != 'long_only'][:3]
    for r in top3:
        lbl = r['label']; sm = r['short_mode']
        an = r.get('atr_norm_max', 12.0); mc = r.get('max_corr', 0.5)
        tn = r.get('top_n', 3)
        print(f"\n  Walk-forward: {lbl}")
        walk_forward(label=f"WF {lbl}", atr_norm_max=an, max_corr=mc,
                     dd_tiers=DD_AGGR100, eq_method='none', regime_lo=0.5,
                     regime_hi=1.5, sl_pct=0.0, hold=1, top_n=tn,
                     vol_mode='fixed', short_mode=sm)

    # ===================== FINAL SUMMARY =====================
    print("\n" + "=" * 130)
    print("  V177 FINAL SUMMARY: Dual-Side V121")
    print("=" * 130)

    print(f"\n  {'Config':40s} | {'Ann':>7s} | {'MDD':>5s} | {'R/M':>5s} | {'WR':>5s} | {'N':>4s} | Delta_R/M")
    print(f"  {'-'*40}-+-{'-'*7}-+-{'-'*5}-+-{'-'*5}-+-{'-'*5}-+-{'-'*4}-+-{'-'*8}")
    print(f"  {'BASELINE (Long-only)':40s} | {base_ann:>+7.0f}% | {base_mdd:>5.0f}% | {base_rm:>5.2f} | {r_base['wr']:>5.1f}% | {r_base['n']:>4d} |    ---")

    for r in ranked[:15]:
        if r.get('short_mode') == 'long_only': continue
        ann = r['ann']; mdd = r['mdd']
        rm = abs(ann / mdd) if mdd != 0 else 0
        delta = rm - base_rm
        print(f"  {r['label']:40s} | {ann:>+7.0f}% | {mdd:>5.0f}% | {rm:>5.2f} | {r['wr']:>5.1f}% | {r['n']:>4d} | {delta:>+8.2f}")
