...
class CSVSlideWaveAnalysisStrategy(BaseStrategy):
    ...
    def generate_signals(self):
        ...
        delay = 3  # window=2 → 延迟3天确认
        df = self.data.reset_index(drop=True)

        # 用整数索引找极值点, 然后延迟
        if 'is_high' in df_with_high_low.columns:
            high_bool_indices = df_with_high_low[df_with_high_low['is_high']].index.tolist()
        else:
            high_bool_indices = []

        if 'is_low' in df_with_high_low.columns:
            low_bool_indices = df_with_high_low[df_with_high_low['is_low']].index.tolist()
        else:
            low_bool_indices = []

        # 延迟确认, 用close价
        for idx in high_bool_indices:
            confirmed = idx + delay
            if confirmed < len(self.data):
                ts = self.data.index[confirmed]
                self._record_signal(
                    timestamp=ts,
                    action='sell',
                    price=float(self.data.iloc[confirmed]['close'])
                )

        for idx in low_bool_indices:
            confirmed = idx + delay
            if confirmed < len(self.data):
                ts = self.data.index[confirmed]
                self._record_signal(
