"""
V71: V61 + Cross-Sectional Momentum Rank as Tie-Breaker
========================================================
V61 is ALL-TIME BEST: Sharpe 6.71, MDD 5.5%, ann +15.7%, 78 trades.

V71 adds a secondary sort by momentum quality among candidates
that already pass the composite threshold:

  Tie-break score = (1 - mw - vw) * composite + mw * rank_ret3d + vw * rank_volratio

Where:
  - rank_ret3d: cross-sectional rank of 3-day return (INVERTED: lower ret = more oversold)
  - rank_volratio: cross-sectional rank of vol_5d / vol_20d (higher = volume surge)

Among instruments that pass threshold, prefer those with:
  1. Higher composite rank (primary, same as V61)
  2. Better 3-day return reversal potential (more oversold recently)
  3. Higher volume surge (vol_5d vs vol_20d ratio)

Parameter sweep:
  - momentum_weight: 0.10, 0.15, 0.20
  - volume_weight: 0.05, 0.10
  - max_per_sector: 1, 2
...
def compute_raw_factors(
    ...
    """Compute raw factor values for short-term (5d), medium-term (20d),
    and tie-breaker factors (ret_3d, vol_ratio)."""
    t0 = time.time()
    print("[V71] Computing raw factors (5d + 20d + tie-break)...", flush=True)
    ...
    # --- V71 tie-breaker factors ---
    # ret_3d: 3-day return for reversal potential
    ret_3d = np.full((NS, ND), np.nan)
    for si in range(NS):
        for di in range(3, ND):
            if (not np.isnan(C[si, di])
                    and not np.isnan(C[si, di - 3])
                    and C[si, di - 3] > 0):
                ret_3d[si, di] = C[si, di] / C[si, di - 3] - 1.0

    # vol_ratio = vol_5d / vol_20d for volume surge detection
    vol_ratio = np.full((NS, ND), np.nan)
    for si in range(NS):
        for di in range(20, ND):
            v5 = vol_5d[si, di]
            v20 = vol_20d[si, di]
            if (not np.isnan(v5) and not np.isnan(v20) and v20 > 0):
                vol_ratio[si, di] = v5 / v20
...
def compute_cross_sectional_ranks(
    ...
    t0 = time.time()
    print("[V71] Computing cross-sectional ranks...", flush=True)
    ...
    factors_to_rank = {
        ...
        # V71 tie-breaker ranks
        "rank_ret3d": raw_factors["ret_3d"],
        "rank_volratio": raw_factors["vol_ratio"],
    }

    # Inverted: low raw value -> high rank (most oversold = best MR)
    INVERT_FACTORS = {
        "rank_ret5d", "rank_ret10d", "rank_oi5d", "rank_rsi",
        "rank_ret20d", "rank_oi20d",
        # V71: ret_3d inverted (lower 3d return = more oversold = better MR)
        "rank_ret3d",
    }
...
def build_multi_tf_composite(
    ...
    t0 = time.time()
    print(f"[V71] Building multi-TF composite (st_w={st_weight:.2f})...",
          flush=True)
...
def compute_tiebreak_score(
    ...
    """V71 tie-break score: weighted blend of composite + momentum + volume ranks.

    tiebreak = (1 - mw - vw) * composite + mw * rank_ret3d + vw * rank_volratio
    """
    composite_weight = 1.0 - momentum_weight - volume_weight
    assert composite_weight > 0, (
        f"momentum_weight + volume_weight must be < 1, "
        f"got {momentum_weight + volume_weight}"
    )

    rank_ret3d = ranks["rank_ret3d"]
    rank_volratio = ranks["rank_volratio"]

    tiebreak = np.full((NS, ND), np.nan)
    for si in range(NS):
        for di in range(ND):
            c = composite[si, di]
            if np.isnan(c):
                continue
            r3 = rank_ret3d[si, di]
            vr = rank_volratio[si, di]
            if np.isnan(r3) and np.isnan(vr):
                # Fallback: use composite alone
                tiebreak[si, di] = c
                continue

            # Use 0.5 as neutral if one rank is missing
            r3_val = r3 if not np.isnan(r3) else 0.5
            vr_val = vr if not np.isnan(vr) else 0.5
            tiebreak[si, di] = (
                composite_weight * c
                + momentum_weight * r3_val
                + volume_weight * vr_val
            )
...
def compute_all_signals(
    ...
    # V71: compute tie-break score
    tiebreak = compute_tiebreak_score(
        combined, ranks, NS, ND,
...
def backtest_v71(
    ...
    """Backtest V71: multi-TF composite + tie-breaker + dynamic threshold + sector limits."""
    composite = sigs["composite"]
    tiebreak = sigs["tiebreak"]
    ...
    for di in range(max(start_di, 1), end_di):
        ...
        # V71: filter on composite, sort by tiebreak score
        candidates = []
        for si in range(NS):
            if si in held:
                continue
            ...
            # Use tiebreak score for sorting
            tb = tiebreak[si, di]
            if np.isnan(tb):
                tb = composite[si, di]
            candidates.append((tb, si, alloc))

        # V71: Sort by tie-break score (highest first)
...
def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("  V71: V61 + CROSS-SECTIONAL MOMENTUM TIE-BREAKER")
    print("  Adds ret_3d reversal + vol_ratio surge to composite selection")
    print("  Tiebreak = (1-mw-vw)*composite + mw*rank_ret3d + vw*rank_volratio")
    print("=" * 70)
    ...
    # === 1. Pre-compute signals for each (st_weight, mw, vw) combo ===
        ...
        for mw in [0.10, 0.15, 0.20]:
            for vw in [0.05, 0.10]:
                if mw + vw >= 1.0:
                    continue
                key = (st_w, mw, vw)
                print(f"\n--- Computing signals for st_w={st_w:.2f} "
                      f"mw={mw:.2f} vw={vw:.2f} ---")
    ...
    for r in results[:30]:
        print(
            f"{r['st_w']:>4.2f} {r['mw']:>4.2f} {r['vw']:>4.2f} "
    ...
    unique_top = []
    for r in results:
        key = (r["st_w"], r["mw"], r["vw"], r["wt"], r["nt"],
    ...
    for r in unique_top:
        sigs = signal_cache[(r["st_w"], r["mw"], r["vw"])]
        trades, eq, dd = backtest_v71(
            C, O, H, L, NS, ND, dates, syms, sigs,
            sector_lookup=sector_lookup,
            st_weight=r["st_w"],
        ...
        label = (
            f"st_w={r['st_w']:.2f} mw={r['mw']:.2f} vw={r['vw']:.2f} "
    ...
    if results:
        ...
        # === 5. Comparison: V71 vs V61 (no tie-break) ===
        print("\n" + "=" * 70)
        print("  COMPARISON: V71 (tie-break) vs V61 (no tie-break)")
        print("  (2019-2026 OOS)")
        print("=" * 70)

        # V71 with tie-break (best config)
        trades_v71, eq_v71, dd_v71 = backtest_v71(
            C, O, H, L, NS, ND, dates, syms,
            best_sigs,
            sector_lookup=sector_lookup,
            st_weight=best["st_w"],
        ...
        # V61 equivalent: mw=0, vw=0 (pure composite, no tie-break)
        sigs_v61like = compute_all_signals(
            C, O, H, L, V, OI, NS, ND,
            st_weight=best["st_w"],
            momentum_weight=0.0,
            volume_weight=0.0,
        )
        trades_v61, eq_v61, dd_v61 = backtest_v71(
            C, O, H, L, NS, ND, dates, syms,
            sigs_v61like,
            sector_lookup=sector_lookup,
            st_weight=best["st_w"],
        ...
        print(f"\n  V71 TIE-BREAK (mw={best['mw']:.2f} vw={best['vw']:.2f}):")
        analyze(trades_v71, eq_v71, dd_v71, "V71-tiebreak")
        print(f"\n  V61 NO-TIE-BREAK (mw=0.00 vw=0.00):")
        analyze(trades_v61, eq_v61, dd_v61, "V61-no-tiebreak")

        if trades_v71 and trades_v61:
            print(
                f"\n  V71 vs V61: "
                f"eq_delta={eq_v71 - eq_v61:+,.0f} "
                f"dd_delta={dd_v71 - dd_v61:+.1f}%"
            )

    print(f"\n[V71] Done. {time.time() - t0:.1f}s")
