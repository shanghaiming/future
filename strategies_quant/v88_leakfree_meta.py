"""
V88 — Leak-Free Walk-Forward Meta Model
=========================================
V86 had CRITICAL bug: meta model trained on first 70% of data, then predicted on ALL 100%.
This is train-on-apply leakage — the meta model "remembered" which trades were profitable.

Fix: walk-forward meta training, expanding window (ALL past data, no fixed window).
For each primary retraining point, train a fresh meta model using only data from before that point.
No early stopping on future data — use fixed num_boost_round=100.
Target: find the TRUE performance without leakage.
"""
...
print("=" * 70, flush=True)
print("  V88 — Leak-Free Walk-Forward Meta Model", flush=True)
print("=" * 70, flush=True)
...
# [3] V71 features (identical to V79/V86)
...
# ====================================================================
# [5] PRIMARY MODEL (same as V79/V86)
# ====================================================================
...
print(f"\n[5] Training primary LGB model...", flush=True)
t5 = time.time()

SCORE_PRI = np.full((NS, ND), np.nan)
retrain_points = []
    ...
    for di in range(train_di, pred_end):
        ...
        if pred_list:
            ...
            for k, si in enumerate(pred_si):
                SCORE_PRI[si, di] = scores[k]

    if len(retrain_points) % 5 == 0:
        print(f"    #{len(retrain_points)}: di={train_di}", flush=True)

print(f"    Done: {len(retrain_points)} trainings ({time.time()-t5:.1f}s)", flush=True)

# ====================================================================
# [6] LEAK-FREE WALK-FORWARD META MODEL
# ====================================================================
# Key fix: for each prediction period, meta model uses ONLY past data.
# Expanding window: use ALL data from MIN_TRAIN to (train_di - FWD_DAYS - 1).
# Fixed num_boost_round=100 (no early stopping on future data).
# ====================================================================
...
# Pre-compute SCORE_RANK for all days
...
# Meta model params — fixed rounds, NO early stopping on future data
META_PARAMS = {
...
META_ROUNDS = 100  # Fixed, no early stopping

# Helper: build meta features for a stock on a day
...
n_meta_trains = 0
MIN_META_SAMPLES = 200  # minimum samples to train meta model

for rp_idx, (train_di, pred_end) in enumerate(retrain_points):
    # Meta training: use ALL past data up to (train_di - FWD_DAYS - 1)
    # This ensures FWD_RET for all training samples has resolved
    meta_train_end = train_di - FWD_DAYS - 1
    meta_train_start = MIN_TRAIN
    if meta_train_end <= meta_train_start: continue

    # Collect meta samples from top-5 picks for each past day
    ...
    # Train meta model — fixed rounds, no early stopping
    try:
    ...
    # Predict for ALL stocks in the prediction window
    for di in range(train_di, pred_end):
        pred_list = []; pred_si = []
        for si in range(NS):
            if np.isnan(C[si,di]) or C[si,di]<=0: continue
...
# ====================================================================
# [7] Backtest engine (same as V79/V86)
# ====================================================================
...
print(f"  Leak-free walk-forward meta (expanding window, fixed rounds=100)", flush=True)
