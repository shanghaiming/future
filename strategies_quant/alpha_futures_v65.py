"""
V65: Triple-Layer Strategy = V61 + Breadth Filter
==================================================
V61 is ALL-TIME BEST: Sharpe 6.71, MDD 5.5%, ann +15.7%, 78 trades.
V60 (sector+breadth) had Sharpe 5.77 but MDD 5.1% -- best MDD profile.

V65 combines all three layers:
  1. Dynamic mode switching (from V43) -- WINNING/NORMAL/LOSING thresholds
  2. Sector concentration limit (from V47) -- max_per_sector greedy selection
  3. Multi-TF confirmation (from V61) -- ST(5d) + MT(20d) composite
  4. Breadth filter (from V40) -- only trade when A/D ratio <= max_ad_ratio

Thesis: MR works best when ALL three conditions align:
  - Multi-TF confirms oversold (both 5d and 20d horizons agree)
  - Sector diversified (no over-concentration in one commodity group)
  - Market breadth confirms (not too many commodities declining at once)

Architecture:
  1. Compute ST(5d) composite AND MT(20d) composite via cross-sectional ranks
  2. Combined = st_weight * ST + (1-st_weight) * MT
  3. A/D ratio breadth gate: only enter when ad_ratio <= max_ad_ratio
  4. Dynamic three-mode threshold (WINNING/NORMAL/LOSING)
  5. Sector-constrained greedy selection
  6. KER gate < 0.15, hold 5d, ATR stop 3.0
  7. Pyramid on day-1 winners (ratio varies by mode)

Parameter sweep:
  - max_ad_ratio: 0.40, 0.45, 0.50, 0.55, 0.60
  - st_weight: 0.50, 0.55, 0.60
  - max_per_sector: 1, 2
  - win_rate_window: 10, 15, 20

Walk-forward 2019-2026. Compare vs V61 (Sharpe 6.71) and V60 (MDD 5.1%).
...
def compute_raw_factors(
    ...
    t0 = time.time()
    print("[V65] Computing raw factors (5d + 20d)...", flush=True)
...
def compute_cross_sectional_ranks(
    ...
    t0 = time.time()
    print("[V65] Computing cross-sectional ranks...", flush=True)
...
def build_multi_tf_composite(
    ...
    t0 = time.time()
    print(f"[V65] Building multi-TF composite (st_w={st_weight:.2f})...",
          flush=True)
...
def compute_ad_ratio(C: np.ndarray, NS: int, ND: int) -> np.ndarray:
    ...
    """
    t0 = time.time()
    print("[V65] Computing A/D ratio...", flush=True)
...
def compute_all_signals(
    ...
    if len(valid_ad) > 0:
        print(
            ...
            f"<0.40: {np.sum(valid_ad < 0.40) / len(valid_ad) * 100:.1f}% "
...
def backtest_v65(
    ...
    """Backtest V65: multi-TF + sector limit + breadth filter."""
    ...
    for di in range(max(start_di, 1), end_di):
        ...
        # ============================================================
        # BREADTH FILTER: only enter when ad_ratio <= max_ad_ratio
        # Higher ad_ratio = more declines = market oversold
        # We want to avoid extreme panic: only trade when ad_ratio
        # is below threshold (market not too broadly declining)
        # ============================================================
...
def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("  V65: TRIPLE-LAYER STRATEGY")
    print("  V61 (multi-TF + sector) + V40 breadth filter")
    print("  Target: beat V61 Sharpe 6.71, V60 MDD 5.1%")
    print("=" * 70)
    ...
    # Note: ad_ratio is independent of st_weight, but we compute it
    # inside compute_all_signals which is cached per st_weight.
    # To save time, we compute ad_ratio once and share it.
    signal_cache: Dict[float, Dict] = {}
    ad_ratio_shared = None
    ...
    for st_w in [0.50, 0.55, 0.60]:
        ...
        if ad_ratio_shared is None:
            ad_ratio_shared = sigs["ad_ratio"]
        else:
            # Reuse the same ad_ratio (it does not depend on st_weight)
            sigs["ad_ratio"] = ad_ratio_shared
        signal_cache[st_w] = sigs
    ...
    sweep_ad_max = [0.40, 0.45, 0.50, 0.55, 0.60]
    sweep_st_w = [0.50, 0.55, 0.60]
    sweep_mps = [1, 2]
    sweep_ww = [10, 15, 20]
    sweep_wt = [0.55, 0.60, 0.65]
    sweep_nt = [0.80, 0.82, 0.85]
    sweep_lt = [0.88, 0.90, 0.92]
    sweep_tnw = [2, 3]

    total_combos = (
        len(sweep_ad_max) * len(sweep_st_w) * len(sweep_mps)
        * len(sweep_ww) * len(sweep_wt) * len(sweep_nt)
        * len(sweep_lt) * len(sweep_tnw)
    )
    ...
    for ad_max in sweep_ad_max:
        for st_w in sweep_st_w:
            sigs = signal_cache[st_w]
            for mps in sweep_mps:
                for ww in sweep_ww:
                    for wt in sweep_wt:
                        for nt in sweep_nt:
                            for lt in sweep_lt:
                                if lt <= nt:
                                    continue
                                for tnw in sweep_tnw:
                                    combo_count += 1
                                    trades, eq, dd = backtest_v65(
                                        C, O, H, L, NS, ND,
                                        dates, syms, sigs,
                                        sector_lookup=sector_lookup,
                                        max_ad_ratio=ad_max,
    ...
    print(
        f"\n{'AD':>4} {'STw':>4} {'WT':>4} {'NT':>4} {'LT':>4} {'WW':>3} "
        f"{'MPS':>3} {'TNW':>4} "
    ...
    for r in results[:30]:
        print(
            f"{r['ad_max']:>4.2f} {r['st_w']:>4.2f} {r['wt']:>4.2f} "
            f"{r['nt']:>4.2f} {r['lt']:>4.2f} {r['ww']:>3} "
            f"{r['mps']:>3} {r['tnw']:>4} "
    ...
    unique_top = []
    for r in results:
        key = (r["ad_max"], r["st_w"], r["wt"], r["nt"],
               r["lt"], r["ww"], r["mps"], r["tnw"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
        if len(unique_top) >= 5:
    ...
    for r in unique_top:
        ...
        label = (
            f"ad={r['ad_max']:.2f} st_w={r['st_w']:.2f} "
            f"wt={r['wt']:.2f} nt={r['nt']:.2f} "
    ...
    if results:
        ...
        # === 5. Head-to-head: V65 vs V61 (no breadth) ===
        print("\n" + "=" * 70)
        print("  HEAD-TO-HEAD: V65 (sector+multiTF+breadth) "
              "vs V61 (sector+multiTF only)")
        print("  (2019-2026 OOS)")
        print("=" * 70)

        # V65 best config
        trades_v65, eq_v65, dd_v65 = backtest_v65(
            C, O, H, L, NS, ND, dates, syms,
            signal_cache[best["st_w"]],
            sector_lookup=sector_lookup,
            max_ad_ratio=best["ad_max"],
        ...
        # V61 equivalent: no breadth filter (ad_max=1.0)
        trades_v61, eq_v61, dd_v61 = backtest_v65(
            C, O, H, L, NS, ND, dates, syms,
            signal_cache[best["st_w"]],
            sector_lookup=sector_lookup,
        ...
        print(f"\n  V65 TRIPLE-LAYER "
              f"(ad={best['ad_max']:.2f}, st_w={best['st_w']:.2f}):")
        analyze(trades_v65, eq_v65, dd_v65, "V65-triple")
        print(f"\n  V61 MULTI-TF ONLY (ad=1.00, st_w={best['st_w']:.2f}):")
        analyze(trades_v61, eq_v61, dd_v61, "V61-multiTF-only")

        if trades_v65 and trades_v61:

            def compute_sharpe(tl: List[dict]) -> float:
                ap = [t["pnl_abs"]
                      for t in sorted(tl, key=lambda x: x["di"])]
                rets_a = np.array(ap) / CASH0
                if np.std(rets_a) > 0:
                    return float(
                        np.mean(rets_a) / np.std(rets_a) * np.sqrt(252))
                return 0.0

            sh_v65 = compute_sharpe(trades_v65)
            sh_v61 = compute_sharpe(trades_v61)
            print(
                f"\n  VERDICT: V65 Sharpe={sh_v65:.2f} vs "
                f"V61 Sharpe={sh_v61:.2f} "
                f"delta={sh_v65 - sh_v61:+.2f}"
            )
            print(
                f"  V65 vs V61: "
                f"eq_delta={eq_v65 - eq_v61:+,.0f} "
                f"dd_delta={dd_v65 - dd_v61:+.1f}% "
                f"n_delta={len(trades_v65) - len(trades_v61):+d} trades"
            )
            if sh_v65 > sh_v61 and dd_v65 <= dd_v61:
                print(
                    f"  CONCLUSION: TRIPLE FILTER DOMINATES! "
                    f"V65 beats V61 on BOTH Sharpe ({sh_v65:.2f} vs "
                    f"{sh_v61:.2f}) AND MDD ({dd_v65:.1f}% vs "
                    f"{dd_v61:.1f}%)"
                )
            elif sh_v65 > sh_v61:
                print(
                    f"  CONCLUSION: BREADTH HELPS Sharpe "
                    f"({sh_v65:.2f} vs {sh_v61:.2f}) "
                    f"but MDD worse ({dd_v65:.1f}% vs {dd_v61:.1f}%)"
                )
            elif dd_v65 < dd_v61:
                print(
                    f"  CONCLUSION: BREADTH HELPS MDD "
                    f"({dd_v65:.1f}% vs {dd_v61:.1f}%) "
                    f"but Sharpe worse ({sh_v65:.2f} vs {sh_v61:.2f})"
                )
            else:
                print(
                    f"  CONCLUSION: BREADTH FILTER HURTS. "
                    f"V61 beats V65 on both metrics."
                )

    print(f"\n[V65] Done. {time.time() - t0:.1f}s")
