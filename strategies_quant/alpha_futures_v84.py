"""
V84: V80 + V68 Sector-Weighted Position Sizing
================================================
Combine V80's high-ann parameters with V68's rank-weighted sector allocation.

V80 best: ann +36.4%, Sharpe 5.62, MDD 5.2% (mps=3, mp=3, nt=0.85, lt=0.95)
V68 best: ann +21.9%, Sharpe 7.03, MDD 11.8% (rank-weighted sector sizing)

Key idea:
  - Use V80's "take ALL above threshold" entry (not top-N)
  - Use V80's dynamic mode thresholds (win/normal/lose)
  - Replace V80's equal-weight 1/N allocation with V68's sector-aware weighting
  - When a sector has stronger aggregate signal, its positions get more capital
  - This should improve capital allocation efficiency

Sector weight methods:
  - 'equal': baseline 1/N (same as V80)
  - 'rank_weighted': weight proportional to sector avg composite score
  - 'top_sector_double': strongest sector gets 2x allocation

Parameter sweep:
  - sector_weight_method: 'equal', 'rank_weighted', 'top_sector_double'
  - max_per_sector: 2, 3
  - max_positions: 3, 4, 5
  - normal_threshold: 0.83, 0.85, 0.87

Fixed from V80 best: st_w=0.60, wt=0.60, lt=0.95, ww=15, atr_stop=3.0
...
def compute_raw_factors(
    ...
    t0 = time.time()
    print("[V84] Computing raw factors (5d + 20d)...", flush=True)
...
def compute_cross_sectional_ranks(
    ...
    t0 = time.time()
    print("[V84] Computing cross-sectional ranks...", flush=True)
...
def build_multi_tf_composite(
    ...
    t0 = time.time()
    print(f"[V84] Building multi-TF composite (st_w={st_weight:.2f})...",
          flush=True)
...
def compute_sector_allocation_weights(
    selected_entries: List[Tuple[float, int, str]],
    sector_lookup: Dict[int, str],
    method: str,
    max_positions: int,
) -> Dict[str, float]:
    """Compute per-sector allocation weight based on aggregate signal strength.

    Args:
        selected_entries: list of (composite_score, symbol_index, sector)
        sector_lookup: symbol-index -> sector mapping
        method: 'equal', 'rank_weighted', or 'top_sector_double'
        max_positions: max concurrent positions

    Returns:
        Dict mapping sector -> allocation weight (sums to 1.0).
        For 'equal' method, returns empty dict (caller uses 1/N).
    """
    if method == 'equal' or not selected_entries:
        return {}

    # Aggregate scores by sector
    sector_scores: Dict[str, List[float]] = defaultdict(list)
    for score, si, sector in selected_entries:
        sector_scores[sector].append(score)

    if not sector_scores:
        return {}

    # Compute average composite per sector
    sector_avg: Dict[str, float] = {}
    for sector, scores in sector_scores.items():
        sector_avg[sector] = np.mean(scores)

    if method == 'rank_weighted':
        # Weight proportional to average composite score
        total_score = sum(sector_avg.values())
        if total_score <= 0:
            return {}
        weights = {}
        for sector, avg in sector_avg.items():
            weights[sector] = avg / total_score
        return weights

    if method == 'top_sector_double':
        # Strongest sector gets double weight, others share remainder
        if len(sector_avg) <= 1:
            return {}
        best_sector = max(sector_avg, key=sector_avg.get)
        remaining_sectors = [s for s in sector_avg if s != best_sector]
        n_remaining = len(remaining_sectors)
        if n_remaining == 0:
            return {best_sector: 1.0}
        # Best sector gets 2/(2+n_remaining), others get 1/(2+n_remaining) each
        denominator = 2.0 + n_remaining
        weights = {best_sector: 2.0 / denominator}
        for sec in remaining_sectors:
            weights[sec] = 1.0 / denominator
        return weights
...
def backtest_v84(
    ...
    hold_days: int = 5,
    sector_weight_method: str = 'equal',
    start_di: int = 60,
    end_di: Optional[int] = None,
) -> Tuple[List[dict], float, float]:
    """Backtest V84: V80 high-freq compound + V68 sector-weighted allocation.

    Key design:
    - V80 base: take ALL instruments above threshold (not top-N)
    - V80 dynamic thresholds based on mode (win/normal/lose)
    - V84 NEW: sector-aware allocation instead of equal 1/N
      When a sector has stronger aggregate signal, its positions get more capital
    ...
    for di in range(max(start_di, 1), end_di):
        ...
        # --- ENTRY: take ALL instruments above threshold (V80 style) ---
        ...
        # Sector-constrained selection (V80 style)
        sector_counts: Dict[str, int] = defaultdict(int)
        for si_held in held:
            sector_counts[sector_lookup.get(si_held, 'OTHER')] += 1

        new_entries: List[Tuple[float, int, str]] = []
        ...
        # === V84 NEW: Compute sector-aware allocation ===
        sector_weights = compute_sector_allocation_weights(
            new_entries, sector_lookup, sector_weight_method, max_positions)

        num_total = len(positions) + len(new_entries)

        # Build allocation map: si -> alloc
        alloc_map: Dict[int, float] = {}

        if not sector_weights:
            # Equal weight (V80 baseline)
            alloc_per_pos = LEVERAGE / num_total
            for _, si, _ in new_entries:
                alloc_map[si] = alloc_per_pos
        else:
            # Sector-weighted allocation
            # Group new entries by sector
            entries_by_sector: Dict[str, List[Tuple[float, int]]] = (
                defaultdict(list))
            for rank_val, si, sector in new_entries:
                entries_by_sector[sector].append((rank_val, si))

            for sector, entries in entries_by_sector.items():
                sector_w = sector_weights.get(sector, 0.0)
                if sector_w <= 0 or not entries:
                    continue
                # Distribute sector weight equally among positions in sector
                alloc_per_entry = sector_w * LEVERAGE / len(entries)
                for _, si in entries:
                    alloc_map[si] = alloc_per_entry

        # Update existing positions with blended allocation
        updated_positions = []
        # Existing positions get equal share of capital
        existing_alloc = LEVERAGE / num_total
        for si, edi, ep, sp, old_alloc, is_pyr in positions:
            updated_positions.append(
                (si, edi, ep, sp, existing_alloc, is_pyr))
        ...
        for rank_val, si, sym_sector in new_entries:
            ...
            if atr is None:
                continue
            alloc = alloc_map.get(si, LEVERAGE / num_total)
            updated_positions.append(
...
def walk_forward(
    ...
    for test_year in range(2019, years[-1] + 1):
        ...
        trades, _, _ = backtest_v84(
            ...
            use_ker_gate=True,
            hold_days=5,
            sector_weight_method=sector_weight_method,
            start_di=test_start,
...
def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("  V84: V80 + V68 SECTOR-WEIGHTED POSITION SIZING")
    print("  V80 base: high-freq compound (mps=3, mp=3)")
    print("  V68 enhancement: sector-aware capital allocation")
    print("  Target: ann > 40%, Sharpe > 6, MDD < 8%")
    ...
    print("  NO LEVERAGE. V80 base + V68 sector weighting.")
    print("  Target: ann > 40%, Sharpe > 6, MDD < 8%")
    print("=" * 70)

    results: List[dict] = []
    SECTOR_METHODS = ['equal', 'rank_weighted', 'top_sector_double']

    sweep_count = 0
    for swm in SECTOR_METHODS:
        for mps in [2, 3]:
            for mp in [3, 4, 5]:
                for nt in [0.83, 0.85, 0.87]:
                    sweep_count += 1
                    trades, eq, dd = backtest_v84(
                        C, O, H, L, NS, ND, dates, syms, sigs,
                        sector_lookup=sector_lookup,
                        st_weight=0.60,
                        win_threshold=0.60,
    ...
    for r in unique_top:
        trades, eq, dd = backtest_v84(
            ...
            use_ker_gate=True,
            hold_days=5,
            sector_weight_method=r["swm"],
            start_di=60,
        )
        label = (
            f"swm={r['swm']} nt={r['nt']:.2f} "
            f"mps={r['mps']} mp={r['mp']}"
        )
        print(f"\n  FULL {label}")
    ...
    walk_forward(
        ...
        sector_weight_method=best["swm"],
    )

    # === 5. Comparison: best V84 vs V80 baseline (equal weight) ===
    print("\n" + "=" * 70)
    print("  COMPARISON: V84 best vs V80 baseline (equal weight)")
    print("  (2019-2026 OOS)")
    print("=" * 70)

    # V84 best config
    trades_v84, eq_v84, dd_v84 = backtest_v84(
        C, O, H, L, NS, ND, dates, syms, sigs,
        sector_lookup=sector_lookup,
        st_weight=0.60,
        win_threshold=0.60,
    ...
    # V80 baseline: same params but equal weight
    trades_v80, eq_v80, dd_v80 = backtest_v84(
        C, O, H, L, NS, ND, dates, syms, sigs,
        sector_lookup=sector_lookup,
        st_weight=0.60,
        win_threshold=0.60,
        ...
        max_positions=best["mp"],
        max_per_sector=best["mps"],
        sector_weight_method='equal',
        start_di=bt_2019,
    )

    print(f"\n  V84 BEST ({best['swm']}):")
    analyze(trades_v84, eq_v84, dd_v84, "V84-sector-weighted")
    print(f"\n  V80 BASELINE (equal):")
    analyze(trades_v80, eq_v80, dd_v80, "V80-equal-weight")

    if trades_v84 and trades_v80:
        print(
            f"\n  V84 vs V80: "
            f"eq_delta={eq_v84 - eq_v80:+,.0f} "
            f"dd_delta={dd_v84 - dd_v80:+.1f}% "
            f"sh_delta={0:.2f}"
        )

    # === 6. Show best per sector method ===
    print("\n" + "=" * 70)
    print("  BEST PER SECTOR METHOD")
    print("=" * 70)
    for method in SECTOR_METHODS:
        method_results = [r for r in results if r["swm"] == method]
        if method_results:
            top = method_results[0]
            print(
                f"  {method:>16}: Sh={top['sharpe']:.2f} "
                f"ann={top['ann']:+.1f}% DD={top['dd']:.1f}% "
                f"WR={top['wr']:.1f}% N={top['n']} "
                f"nt={top['nt']:.2f} mps={top['mps']} "
                f"mp={top['mp']}"
            )

    # === 7. Target check: ann > 40%, Sharpe > 6, MDD < 8% ===
    print("\n" + "=" * 70)
    print("  TARGET CHECK: ann > 40%, Sharpe > 6, MDD < 8%")
    print("=" * 70)
    target_met = [r for r in results
                  if r["ann"] > 40 and r["sharpe"] > 6 and r["dd"] < 8]
    if target_met:
        print(f"  {len(target_met)} configs meet ALL targets:")
        for r in target_met[:10]:
            print(
                f"    swm={r['swm']:>16} nt={r['nt']:.2f} "
                f"mps={r['mps']} mp={r['mp']} "
                f"ann={r['ann']:+.1f}% Sh={r['sharpe']:.2f} "
                f"DD={r['dd']:.1f}% WR={r['wr']:.1f}%"
            )
    else:
        print("  No configs meet ALL targets.")
        # Show closest
        near_target = sorted(
            results,
            key=lambda x: (
                abs(x["ann"] - 40) + abs(x["sharpe"] - 6) * 5
                + abs(x["dd"] - 8) * 2
            )
        )[:5]
