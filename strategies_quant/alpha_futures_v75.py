"""
V75: V68 + Aggressive Leverage + Full Capital Allocation
========================================================
Goal: Maximize annual returns. User accepts high MDD for high returns.

Base: V68 best config (rank_weighted sector sizing, Sharpe 7.03).
Key changes:
  1. Position sizing: allocate FULL equity per trade (alloc=1.0)
  2. Allow up to 3 concurrent positions (each gets 1/N of equity)
  3. Leverage sweep: 3x, 5x, 8x, 10x, 15x, 20x, 25x, 30x
  4. Keep V68's sector-weighted signal quality
  5. Keep walk-forward validation

P&L with leverage:
  profit = equity * alloc * pnl * leverage
  where pnl = (exit_price - entry_price) / entry_price - commission

CASH0 = 1,000,000, COMM = 0.0005
...
LEVERAGE_LEVELS = [1, 3, 5, 8, 10, 15, 20, 25, 30]

# V68 best config parameters (Sharpe 7.03)
BEST_PARAMS = {
    "st_weight": 0.60,
    "win_threshold": 0.60,
    ...
    "top_n_winning": 2,
    "max_per_sector": 1,
    "sector_weight_method": "rank_weighted",
}

# Short-term (5d) factor weights -- same as V61/V68
ST_WEIGHTS = {
    "rank_ret5d": 0.25,
    "rank_oi5d": 0.20,
    "rank_rsi": 0.15,
...
# Medium-term (20d) factor weights -- same as V61/V68
MT_WEIGHTS = {
    "rank_ret20d": 0.25,
    "rank_oi20d": 0.20,
    "rank_rsi": 0.15,
...
def compute_raw_factors(
    ...
    t0 = time.time()
    print("[V75] Computing raw factors (5d + 20d)...", flush=True)
...
def compute_cross_sectional_ranks(
    ...
    t0 = time.time()
    print("[V75] Computing cross-sectional ranks...", flush=True)
...
def build_multi_tf_composite(
    ...
    t0 = time.time()
    print(f"[V75] Building multi-TF composite (st_w={st_weight:.2f})...",
          flush=True)
...
def backtest_v75(
    ...
    sector_weight_method: str = 'rank_weighted',
) -> Tuple[List[dict], float, float]:
    """Backtest V75: V68 signals + leverage + full capital allocation.

    Key difference from V68:
    - leverage: multiply P&L by leverage factor
    - max_positions: allow up to N concurrent positions
    - full_alloc: if True, alloc=1.0 per position (divided by N positions)
      instead of the V68 sector-weighted fractional allocation
    """
    ...
    for di in range(max(start_di, 1), end_di):
        ...
        # V75: cap top_n at max_positions
        current_top_n = min(current_top_n, max_positions)
        ...
        for si in range(NS):
            ...
            if di + 1 >= ND or np.isnan(O[si, di + 1]):
                continue
            # V75: full allocation = 1.0 per position slot
            if full_alloc:
        ...
        # V68 sector-aware position sizing
        ...
        for rank_val, si, base_alloc in candidates:
            ...
            # V75: if full_alloc, just use base_alloc (1/N)
            # Otherwise apply sector-aware allocation from V68
            if full_alloc:
                alloc = base_alloc
            else:
...
def analyze(trades: List[dict], equity: float, max_dd: float,
    ...
    nw = sum(1 for t in trades if t.get("pnl_raw", t["pnl_pct"]) > 0)
...
def walk_forward(
    ...
    print(
        ...
        f"st_w={st_weight:.2f} swm={sector_weight_method})"
    )
    print(f"{'=' * 70}")
    ...
    for test_year in range(2019, years[-1] + 1):
        ...
        if test_trades:
            n = len(test_trades)
            nw = sum(1 for t in test_trades if t.get("pnl_raw", t["pnl_pct"]) > 0)
            wr_val = nw / n * 100
    ...
    if all_trades:
        nw = sum(1 for t in all_trades if t.get("pnl_raw", t["pnl_pct"]) > 0)
...
def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("  V75: V68 + AGGRESSIVE LEVERAGE SWEEP")
    print("  Target: 600%+ annualized returns")
    print("  Base: V68 rank_weighted sector sizing (Sharpe 7.03)")
    print(f"  Leverage levels: {LEVERAGE_LEVELS}")
    print(f"  Max concurrent positions: up to 3")
    print(f"  Full capital allocation per trade slot")
    print("=" * 70)
    ...
    # === 1. Compute signals once (V68 best st_weight=0.60) ===
    st_w = BEST_PARAMS["st_weight"]
    print(f"\n--- Computing signals for st_weight={st_w:.2f} ---")
    sigs = compute_all_signals(
        C, O, H, L, V, OI, NS, ND, st_weight=st_w)

    # === 2. LEVERAGE SWEEP: 1x through 30x ===
    print("\n" + "=" * 70)
    print("  LEVERAGE SWEEP (2019-2026 OOS)")
    print("  Each position gets 1/N of equity, leveraged")
    print("=" * 70)
    ...
    for lev in LEVERAGE_LEVELS:
        for max_pos in [1, 2, 3]:
            trades, eq, dd = backtest_v75(
                C, O, H, L, NS, ND, dates, syms, sigs,
                sector_lookup=sector_lookup,
                leverage=float(lev),
                max_positions=max_pos,
                full_alloc=True,
                st_weight=bp["st_weight"],
                win_threshold=bp["win_threshold"],
                normal_threshold=bp["normal_threshold"],
                lose_threshold=bp["lose_threshold"],
                win_rate_window=bp["win_rate_window"],
                atr_stop=bp["atr_stop"],
                top_n_winning=bp["top_n_winning"],
                max_per_sector=bp["max_per_sector"],
                start_di=bt_2019,
                sector_weight_method=bp["sector_weight_method"],
            )

            if len(trades) < 5:
                print(
                    f"  lev={lev:>2}x pos={max_pos}: "
                    f"TOO FEW TRADES ({len(trades)})")
                sweep_results.append({
                    "leverage": lev, "max_pos": max_pos,
                    "n": len(trades), "ann": 0.0, "dd": dd,
                    "sharpe": 0.0, "eq": eq, "blew_up": eq <= 0,
                    "wr": 0.0,
                })
                continue
            ...
            status = "BlewUp" if blew else ""
            print(
                f"  lev={lev:>2}x pos={max_pos}: "
                f"{len(trades):>4}t WR={wr:.1f}% "
                f"ann={ann:>+10.1f}% DD={dd:>6.1f}% "
                f"Sh={sh_val:>6.2f} eq={eq:>14,.0f} {status}"
            )

    # === 3. Print formatted table ===
    print("\n" + "=" * 90)
    print("  LEVERAGE vs (Ann Return, MDD, Sharpe) TABLE")
    print("=" * 90)
    print(
        f"{'Lev':>4} {'Pos':>4} {'N':>5} {'WR':>6} "
        f"{'Ann':>12} {'MDD':>8} {'Sharpe':>8} {'Equity':>16} {'Status':>8}"
    )
    print("-" * 90)
    for r in sweep_results:
        status = "BlewUp" if r["blew_up"] else ("OK" if r["n"] >= 5 else "Few")
        print(
            f"{r['leverage']:>4}x {r['max_pos']:>4} "
            f"{r['n']:>5} {r['wr']:>5.1f}% "
            f"{r['ann']:>+10.1f}% {r['dd']:>7.1f}% "
            f"{r['sharpe']:>7.2f} {r['eq']:>15,.0f} {status:>8}"
        )

    # === 4. Find optimal leverage for 600% target ===
    print("\n" + "=" * 70)
    print("  OPTIMAL LEVERAGE FOR 600% ANNUAL TARGET")
    print("=" * 70)

    viable = [r for r in sweep_results if not r["blew_up"] and r["n"] >= 5]
    if viable:
        # Find closest to 600% ann without blowing up
        viable.sort(key=lambda x: abs(x["ann"] - 600))
        optimal = viable[0]
        print(
            f"\n  OPTIMAL (closest to 600% ann): "
            f"leverage={optimal['leverage']}x max_pos={optimal['max_pos']}"
        )
        print(
            f"    Ann={optimal['ann']:+.1f}% MDD={optimal['dd']:.1f}% "
            f"Sharpe={optimal['sharpe']:.2f} "
            f"WR={optimal['wr']:.1f}% N={optimal['n']}"
        )
        print(f"    Final equity: {optimal['eq']:,.0f}")

        # Also show best Sharpe among those with ann >= 300%
        high_return = [r for r in viable if r["ann"] >= 300]
        if high_return:
            high_return.sort(key=lambda x: -x["sharpe"])
            best_sh = high_return[0]
            print(
                f"\n  BEST SHARPE (ann>=300%): "
                f"leverage={best_sh['leverage']}x max_pos={best_sh['max_pos']}"
            )
            print(
                f"    Ann={best_sh['ann']:+.1f}% MDD={best_sh['dd']:.1f}% "
                f"Sharpe={best_sh['sharpe']:.2f} "
                f"WR={best_sh['wr']:.1f}% N={best_sh['n']}"
            )

        # Show max ann that survived
        viable_by_ann = sorted(viable, key=lambda x: -x["ann"])
        max_surv = viable_by_ann[0]
        print(
            f"\n  MAX SURVIVABLE: "
            f"leverage={max_surv['leverage']}x max_pos={max_surv['max_pos']}"
        )
        print(
            f"    Ann={max_surv['ann']:+.1f}% MDD={max_surv['dd']:.1f}% "
            f"Sharpe={max_surv['sharpe']:.2f} "
            f"WR={max_surv['wr']:.1f}% N={max_surv['n']}"
        )
    else:
        print("  No viable configurations (all blew up)")

    # === 5. Full 10-year backtest for top configs ===
    print("\n" + "=" * 70)
    print("  FULL 10-YEAR BACKTEST (2016-2026)")
    print("=" * 70)

    # Test top 3 viable configs on full period
    if viable:
        seen_configs = set()
        unique_configs = []
        for r in sorted(viable, key=lambda x: abs(x["ann"] - 600)):
            key = (r["leverage"], r["max_pos"])
            if key not in seen_configs:
                seen_configs.add(key)
                unique_configs.append(r)
            if len(unique_configs) >= 3:
                break

        for r in unique_configs:
            trades, eq, dd = backtest_v75(
                C, O, H, L, NS, ND, dates, syms, sigs,
                sector_lookup=sector_lookup,
            ...
            label = f"lev={r['leverage']}x pos={r['max_pos']}"
            print(f"\n  FULL {label}")
            analyze(trades, eq, dd, label)

    # === 6. Walk-forward for optimal ===
    if viable:
        optimal = sorted(viable, key=lambda x: abs(x["ann"] - 600))[0]
        walk_forward(
            C, O, H, L, NS, ND, dates, syms, sigs,
            sector_lookup=sector_lookup,
            leverage=float(optimal["leverage"]),
            max_positions=optimal["max_pos"],
            st_weight=bp["st_weight"],
            win_threshold=bp["win_threshold"],
            normal_threshold=bp["normal_threshold"],
            lose_threshold=bp["lose_threshold"],
