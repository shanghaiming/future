"""
V47: Dynamic Mode + Sector Concentration Limit
================================================
V43 is our ALL-TIME BEST: Sharpe 4.99, MDD 13.3%, ann +21.6%, 150 trades.
V47 adds sector concentration limits to improve diversification.

Innovation:
  - Never take more than N trades from the same sector on the same day
  - Sector definitions: BLACK, METAL, ENERGY, CHEMICAL, AGRI, SOFTS
  - After ranking all instruments, greedily select top_n but skip if
    sector limit is reached
  - This forces diversification across commodity groups

Architecture (same as V43):
  1. Compute V18's 7 cross-sectional ranks, composite score
  2. Dynamic three-mode threshold (WINNING/NORMAL/LOSING)
  3. NEW: Sector-constrained selection
...
Parameter sweep:
  ...
  - V43 mode thresholds: winning=0.75, normal=0.82, losing=0.88
...
def build_sector_lookup(syms: List[str]) -> Dict[int, str]:
    ...
    for si, sym in enumerate(syms):
        ...
        # Strip digit suffixes for futures symbols (e.g., 'i2401' -> 'i')
...
def compute_raw_factors(
    ...
    t0 = time.time()
    print("[V47] Computing raw factors...", flush=True)
...
def compute_cross_sectional_ranks(
    ...
    t0 = time.time()
    print("[V47] Computing cross-sectional ranks...", flush=True)
...
def build_composite_signal(
    ...
    t0 = time.time()
    print("[V47] Building composite signal...", flush=True)
...
def backtest_v47(
    ...
    """Backtest V47 with dynamic threshold switching + sector limits."""
    ...
    for di in range(max(start_di, 1), end_di):
        ...
        # NEW: Sector-constrained greedy selection
        sector_counts: Dict[str, int] = defaultdict(int)
        # Count sectors already held in positions
        for si_held in held:
        ...
        if test_trades:
            ...
            for t in test_trades:
                ...
                if m == "WIN":
                    ...
                    modes["N"] += 1
            # Sector distribution for test year
...
def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("  V47: DYNAMIC MODE + SECTOR CONCENTRATION LIMIT")
    print("  V43 dynamic threshold + sector diversification constraint")
    print("=" * 70)
    ...
    default_configs = [
        # (wt, nt, lt, ww, ats, tnw, mps)
        (0.60, 0.82, 0.90, 15, 3.0, 2, 1),
        (0.60, 0.82, 0.90, 15, 3.0, 2, 2),
        (0.60, 0.80, 0.88, 15, 3.0, 2, 1),
        (0.60, 0.80, 0.88, 15, 3.0, 2, 2),
        (0.55, 0.82, 0.90, 15, 3.0, 2, 1),
    ]

    for wt, nt, lt, ww, ats, tnw, mps in default_configs:
        walk_forward(
            C, O, H, L, NS, ND, dates, syms, sigs,
            sector_lookup=sector_lookup,
            win_threshold=wt,
    ...
    profiles = [
        (0.60, 0.82, 0.90, 15, 3.0, 2, 1, "Default mps=1"),
        (0.60, 0.82, 0.90, 15, 3.0, 2, 2, "Default mps=2"),
        (0.55, 0.80, 0.88, 15, 3.0, 2, 1, "Relaxed mps=1"),
        (0.55, 0.80, 0.88, 15, 3.0, 2, 2, "Relaxed mps=2"),
        (0.65, 0.85, 0.92, 15, 3.0, 2, 1, "Tight mps=1"),
        (0.65, 0.85, 0.92, 15, 3.0, 2, 2, "Tight mps=2"),
        (0.60, 0.82, 0.90, 10, 3.0, 2, 1, "Short window mps=1"),
        (0.60, 0.82, 0.90, 20, 3.0, 2, 1, "Long window mps=1"),
    ]

    for wt, nt, lt, ww, ats, tnw, mps, label in profiles:
        trades, eq, dd = backtest_v47(
            C, O, H, L, NS, ND, dates, syms, sigs,
            sector_lookup=sector_lookup,
            win_threshold=wt,
            normal_threshold=nt,
    ...
    if results:
        ...
        # === 6. Compare: V47 with sector limit vs V43 without ===
        print("\n" + "=" * 70)
        print("  COMPARISON: V47 (sector limit) vs V43 (no sector limit)")
        print("  (2019-2026 OOS)")
        print("=" * 70)

        # V47 with sector limit (best config)
        trades_v47, eq_v47, dd_v47 = backtest_v47(
            C, O, H, L, NS, ND, dates, syms, sigs,
            sector_lookup=sector_lookup,
            win_threshold=best["wt"],
            normal_threshold=best["nt"],
        ...
        # V43-like: no sector limit (mps=999)
        trades_v43like, eq_v43like, dd_v43like = backtest_v47(
            C, O, H, L, NS, ND, dates, syms, sigs,
            sector_lookup=sector_lookup,
            win_threshold=best["wt"],
            normal_threshold=best["nt"],
            ...
            atr_stop=3.0,
            top_n_winning=best["tnw"],
            max_per_sector=999,  # effectively no sector limit
            start_di=bt_2019,
        )

        print(f"\n  V47 WITH SECTOR LIMIT (mps={best['mps']}):")
        analyze(trades_v47, eq_v47, dd_v47, "V47-sector")
        print(f"\n  V43-LIKE NO SECTOR LIMIT:")
        analyze(trades_v43like, eq_v43like, dd_v43like, "V43-nosec")

        if trades_v47:
            print(
                f"\n  V47 vs V43: "
                f"eq_delta={eq_v47 - eq_v43like:+,.0f} "
                f"dd_delta={dd_v47 - dd_v43like:+.1f}%"
            )

    print(f"\n[V47] Done. {time.time() - t0:.1f}s")
