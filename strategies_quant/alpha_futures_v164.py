"""
Alpha Futures V164 — Volatility Regime Adaptive Strategy
==============================================================================
Problem: ALL signal parameter tuning converges to +267%/-26% WF.
Solution: Fundamentally different approach — VOLATILITY REGIME FILTERING.

Instead of trading every day, only enter when volatility conditions are
favorable. This can skip high-vol drawdown periods, increase size in low-vol
trending periods, and use different parameters per regime.

Tests:
A. Individual commodity vol filter (atr_norm): ATR14/Close < threshold
B. Market-wide vol regime (mkt_vol): MKT_VOL < percentile threshold
C. Vol-of-vol (vol_of_vol): 5d vol < 20d vol (declining vol = trend)
D. ATR expansion filter (atr_expansion): Skip when ATR14 > 1.5x ATR_20avg
E. Combined vol gate (combined): ATR/Close < threshold AND MKT_VOL < 75th pct

Signals: V121 + Union (no accel), Kitchen Sink sizing.
Top_n=3, DD tiers aggro100, regime 0.5-1.5, max_corr=0.5, no SL.
"""
...
def main():
    print("=" * 130)
    print("  V164 — Volatility Regime Adaptive Strategy")
    print("  Core idea: Only trade when vol conditions are favorable")
    print("=" * 130)
    t_start = time.time()
    ...
    # ATR14 normalized by close price (individual commodity vol measure)
    ATR_NORM = np.full((NS, ND), np.nan)
    for si in range(NS):
        for di in range(ND):
            if not np.isnan(ATR14[si, di]) and not np.isnan(C[si, di]) and C[si, di] > 0:
                ATR_NORM[si, di] = ATR14[si, di] / C[si, di] * 100  # as percentage

    # ATR 20-day moving average (for expansion detection)
    ATR_MA20 = np.full((NS, ND), np.nan)
    for si in range(NS):
        atr = ATR14[si]
        for di in range(20, ND):
            ...
            if len(valid) >= 10:
                ATR_MA20[si, di] = np.mean(valid)

    # Rolling 5-day and 20-day vol of daily returns (for vol-of-vol)
    ...
    for si in range(NS):
        for di in range(5, ND):
            ...
            v5 = r5[~np.isnan(r5)]
            if len(v5) >= 3:
                VOL5[si, di] = np.std(v5, ddof=1)
        for di in range(20, ND):
            r20 = RET[si, di-20:di]
            v20 = r20[~np.isnan(r20)]
            if len(v20) >= 10:
                VOL20[si, di] = np.std(v20, ddof=1)
    ...
    # Precompute MKT_VOL percentile thresholds
    MKT_VOL_P50 = np.percentile(valid_vols, 50) if len(valid_vols) > 0 else 1.0
    MKT_VOL_P75 = np.percentile(valid_vols, 75) if len(valid_vols) > 0 else 1.0
    MKT_VOL_P90 = np.percentile(valid_vols, 90) if len(valid_vols) > 0 else 1.0
    print(f"  Market vol median: {VOL_MEDIAN:.4f}% | P50={MKT_VOL_P50:.4f} P75={MKT_VOL_P75:.4f} P90={MKT_VOL_P90:.4f}")
    ...
    # ===================== VOLATILITY FILTER =====================
    # Returns True if the entry is ALLOWED by the vol filter, False if blocked.
    # vol_method: 'atr_norm', 'mkt_vol', 'vol_of_vol', 'atr_expansion', 'combined', 'none'
    def vol_filter_pass(di, si, vol_method, vol_threshold):
        """
        vol_threshold meaning varies by method:
          - atr_norm: ATR14/Close < threshold (in %, e.g. 1.0, 2.0, 3.0, 5.0, 10.0)
          - mkt_vol: MKT_VOL < percentile threshold (50th, 75th, 90th)
          - vol_of_vol: VOL5 < VOL20 * threshold (threshold=1.0 means declining vol)
          - atr_expansion: ATR14 < ATR_MA20 * threshold (threshold=1.5 means skip >1.5x)
          - combined: ATR/Close < threshold AND MKT_VOL < 75th pct
          - none: always pass (no filter)
        """
        if vol_method == 'none':
            return True

        elif vol_method == 'atr_norm':
            # Only enter when individual commodity normalized vol is below threshold
            an = ATR_NORM[si, di]
            if np.isnan(an): return False
            return an < vol_threshold

        elif vol_method == 'mkt_vol':
            # Only enter when market-wide vol is below a percentile threshold
            mv = MKT_VOL[di]
            if np.isnan(mv): return False
            # vol_threshold is the percentile value to compare against
            return mv < vol_threshold

        elif vol_method == 'vol_of_vol':
            # Only enter when short-term vol < long-term vol (declining vol)
            v5 = VOL5[si, di]
            v20 = VOL20[si, di]
            if np.isnan(v5) or np.isnan(v20) or v20 <= 0: return False
            return v5 < v20 * vol_threshold

        elif vol_method == 'atr_expansion':
            # Skip when ATR14 > threshold * ATR_MA20 (abnormal vol spike)
            a14 = ATR14[si, di]
            ama = ATR_MA20[si, di]
            if np.isnan(a14) or np.isnan(ama) or ama <= 0: return False
            return a14 < ama * vol_threshold

        elif vol_method == 'combined':
            # Require ATR/Close < threshold AND MKT_VOL < 75th percentile
            an = ATR_NORM[si, di]
            mv = MKT_VOL[di]
            if np.isnan(an) or np.isnan(mv): return False
            return an < vol_threshold and mv < MKT_VOL_P75
    ...
    # ===================== UNIFIED BACKTEST ENGINE WITH VOL FILTER =====================
    def backtest(start_di=MIN_TRAIN, end_di=None,
                 vol_method='none', vol_threshold=999,
                 max_corr=0.5,
                 dd_tiers=None,
        ...
        for di in range(start_di, end_di - 1):
            ...
            # Apply vol filter to candidates
            cands_v121_f = [c for c in cands_v121 if vol_filter_pass(di, c[1], vol_method, vol_threshold)]
            cands_union_f = [c for c in cands_union if vol_filter_pass(di, c[1], vol_method, vol_threshold)]
    ...
    def walk_forward(vol_method='none', vol_threshold=999,
    ...
    # Vol method -> list of thresholds to sweep
    VOL_CONFIGS = {
        'none':          [999],                             # no filter baseline
        'atr_norm':      [1.0, 2.0, 3.0, 5.0, 10.0],      # ATR14/Close < X%
        'mkt_vol':       [MKT_VOL_P50, MKT_VOL_P75, MKT_VOL_P90],  # MKT_VOL < percentile
        'vol_of_vol':    [0.8, 1.0, 1.2],                  # VOL5 < VOL20 * X
        'atr_expansion': [1.2, 1.5, 2.0],                  # ATR14 < ATR_MA20 * X
        'combined':      [1.0, 2.0, 3.0, 5.0, 10.0],      # ATR/Close < X AND MKT_VOL < P75
    }

    # ===================== SECTION 0: BASELINE (no vol filter) =====================
    print("\n" + "=" * 120)
    print("  SECTION 0: BASELINE — No vol filter (champion config: V121+Union, aggro100, top3)")
    print("=" * 120)

    r = backtest(vol_method='none', vol_threshold=999,
                 max_corr=0.5, dd_tiers=dd_t,
                 regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                 hold=1, top_n=3)
    pr(r, "BASELINE: no vol filter, aggro100, top3, reg0.5-1.5, noSL")

    # ===================== SECTION 1: FULL SWEEP — All vol methods & thresholds =====================
    print("\n" + "=" * 120)
    print("  SECTION 1: VOL METHOD SWEEP — full-period backtest")
    print("  Each method x threshold, aggro100 DD, top_n=3, reg0.5-1.5, noSL, corr<0.5")
    print("=" * 120)

    all_results = []  # (ann, mdd, sharpe, n, label, config_dict)

    for vol_method, thresholds in VOL_CONFIGS.items():
        for vol_thresh in thresholds:
            if vol_method == 'mkt_vol':
                # Display which percentile this threshold corresponds to
                if abs(vol_thresh - MKT_VOL_P50) < 1e-6:
                    thresh_label = "P50"
                elif abs(vol_thresh - MKT_VOL_P75) < 1e-6:
                    thresh_label = "P75"
                elif abs(vol_thresh - MKT_VOL_P90) < 1e-6:
                    thresh_label = "P90"
                else:
                    thresh_label = f"{vol_thresh:.4f}"
            else:
                thresh_label = f"{vol_thresh}"
            label = f"vol={vol_method:15s} thresh={thresh_label:>6s} | aggro100 top3 noSL"
            r = backtest(vol_method=vol_method, vol_threshold=vol_thresh,
                         max_corr=0.5, dd_tiers=dd_t,
                         regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                         hold=1, top_n=3)
            pr(r, label)
            all_results.append((r['ann'], r['mdd'], r['sharpe'], r['n'],
                                label,
                                {'vol_method': vol_method, 'vol_threshold': vol_thresh}))
    ...
    for i, (ann, mdd, sh, n, label, cfg) in enumerate(all_results):
    ...
    all_results_rm = sorted(all_results, key=lambda x: -abs(x[0] / x[1]) if x[1] != 0 else 0)
    for i, (ann, mdd, sh, n, label, cfg) in enumerate(all_results_rm):
        ratio = abs(ann / mdd) if mdd != 0 else 0
        print(f"  #{i+1:2d} | Ann={ann:+8.1f}% | MDD={mdd:6.1f}% | Sh={sh:4.2f} | R/M={ratio:.2f} | N={n:4d} | {label}")

    # ===================== SECTION 4: WALK-FORWARD — Top configs =====================
    print("\n" + "=" * 120)
    print("  SECTION 4: WALK-FORWARD VALIDATION — Top 10 by full-period ann")
    print("=" * 120)

    wf_all = {}
    # WF for top 10 by ann
    for i, (ann, mdd, sh, n, label, cfg) in enumerate(all_results[:10]):
        wf_res = walk_forward(vol_method=cfg['vol_method'],
                              vol_threshold=cfg['vol_threshold'],
                              max_corr=0.5, dd_tiers=dd_t,
                              regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                              hold=1, top_n=3, label=label)
        wf_all[label] = (wf_res, cfg)
        print_wf(wf_res, label)

    # Also WF for top 10 by R/M ratio (if different configs)
    for i, (ann, mdd, sh, n, label, cfg) in enumerate(all_results_rm[:10]):
    ...
    # Also WF for the baseline (no vol filter)
    baseline_label = "vol=none            thresh=   999 | aggro100 top3 noSL"
    if baseline_label not in wf_all:
        wf_res = walk_forward(vol_method='none', vol_threshold=999,
                              max_corr=0.5, dd_tiers=dd_t,
                              regime_lo=0.5, regime_hi=1.5, sl_pct=0.0,
                              hold=1, top_n=3, label=baseline_label)
        wf_all[baseline_label] = (wf_res, {'vol_method': 'none', 'vol_threshold': 999})
        print_wf(wf_res, baseline_label + " [BASELINE]")

    # ===================== SECTION 5: TOP 5 BY WF AVG =====================
    print("\n" + "=" * 120)
    print("  SECTION 5: TOP 5 CONFIGS BY WF AVERAGE")
    print("=" * 120)
    ...
    for i, (avg_ann, worst_mdd, best_ann, pos, total_n, avg_wr, label, cfg, wf_res) in enumerate(wf_ranked[:5]):
        ws = " | ".join([f"{yr}:{r['ann']:+.0f}%/{r['mdd']:.0f}%"
                         for yr, r in sorted(wf_res.items())])
    ...
    print("\n" + "=" * 120)
    print("  SECTION 6: BEST RISK-ADJUSTED (WF avg / |worst MDD|)")
    print("=" * 120)

    wf_ra = sorted(wf_ranked, key=lambda x: -abs(x[0] / x[1]) if x[1] != 0 else 0)
    for i, (avg_ann, worst_mdd, best_ann, pos, total_n, avg_wr, label, cfg, wf_res) in enumerate(wf_ra[:5]):
    ...
    # ===================== SECTION 7: VOL METHOD COMPARISON =====================
    print("\n" + "=" * 120)
    print("  SECTION 7: BEST PER VOL METHOD (WF avg)")
    print("=" * 120)

    for method in ['none', 'atr_norm', 'mkt_vol', 'vol_of_vol', 'atr_expansion', 'combined']:
        method_results = [(avg, wmdd, bann, pos, tn, awr, lbl, cfg, wfr)
                          for avg, wmdd, bann, pos, tn, awr, lbl, cfg, wfr in wf_ranked
                          if cfg['vol_method'] == method]
        ...
        ws = " | ".join([f"{yr}:{r['ann']:+.0f}%/{r['mdd']:.0f}%"
                         for yr, r in sorted(wf_res.items())])
        print(f"\n  {method:15s} best: WF AVG={avg_ann:+.0f}% | WorstMDD={worst_mdd:.0f}% | {pos}/6 pos | Thresh={cfg['vol_threshold']}")
        print(f"     {label}")
        print(f"     {ws}")

    # ===================== SECTION 8: BASELINE vs BEST VOL FILTER =====================
    print("\n" + "=" * 120)
    print("  SECTION 8: BASELINE vs BEST VOL FILTER COMPARISON")
    print("=" * 120)

    baseline_wf = wf_all.get(baseline_label)
    if baseline_wf and wf_ranked:
        b_wf_res, b_cfg = baseline_wf
        b_avg = np.mean([r['ann'] for r in b_wf_res.values()])
        b_wmdd = min(r['mdd'] for r in b_wf_res.values())
        b_pos = sum(1 for r in b_wf_res.values() if r['ann'] > 0)
        print(f"\n  BASELINE (no vol filter):")
        print(f"    WF AVG={b_avg:+.0f}% | WorstMDD={b_wmdd:.0f}% | {b_pos}/6 pos")
        b_ws = " | ".join([f"{yr}:{r['ann']:+.0f}%/{r['mdd']:.0f}%"
                           for yr, r in sorted(b_wf_res.items())])
        print(f"    {b_ws}")

        # Find best vol-filtered (not baseline)
        vol_filtered = [(avg, wmdd, bann, pos, tn, awr, lbl, cfg, wfr)
                        for avg, wmdd, bann, pos, tn, awr, lbl, cfg, wfr in wf_ranked
                        if cfg['vol_method'] != 'none']
        if vol_filtered:
            best_vf = vol_filtered[0]  # already sorted by avg_ann desc
            avg_ann, worst_mdd, best_ann, pos, total_n, avg_wr, label, cfg, wf_res = best_vf
            print(f"\n  BEST VOL FILTER:")
            ...
            ann_diff = avg_ann - b_avg
            mdd_diff = worst_mdd - b_wmdd  # positive = better (less negative)
            print(f"\n  IMPROVEMENT: Ann {ann_diff:+.0f}% | MDD {mdd_diff:+.0f}%")
        else:
            print("\n  No vol-filtered results available for comparison.")

    # ===================== SECTION 9: TARGET CHECK =====================
    print("\n" + "=" * 120)
    print("  SECTION 9: CONFIGS MEETING TARGET (WF avg > baseline, any MDD improvement)")
    print("=" * 120)

    if baseline_wf and wf_ranked:
        b_avg_val = np.mean([r['ann'] for r in baseline_wf[0].values()])
        b_mdd_val = min(r['mdd'] for r in baseline_wf[0].values())

        # Show configs with higher WF avg than baseline
        better = [(avg, wmdd, bann, pos, tn, awr, lbl, cfg, wfr)
                  for avg, wmdd, bann, pos, tn, awr, lbl, cfg, wfr in wf_ranked
                  if cfg['vol_method'] != 'none' and avg > b_avg_val]
        if better:
            print(f"\n  Configs with WF avg > baseline ({b_avg_val:+.0f}%):")
            for avg_ann, worst_mdd, best_ann, pos, total_n, avg_wr, label, cfg, wf_res in better[:10]:
                ws = " | ".join([f"{yr}:{r['ann']:+.0f}%/{r['mdd']:.0f}%"
                                 for yr, r in sorted(wf_res.items())])
                print(f"  *** WF AVG={avg_ann:+.0f}% | WorstMDD={worst_mdd:.0f}% | {label}")
                print(f"      {ws}")
        else:
            print(f"\n  No configs beat baseline WF avg of {b_avg_val:+.0f}%")

        # Show configs with better MDD than baseline
        better_mdd = [(avg, wmdd, bann, pos, tn, awr, lbl, cfg, wfr)
                      for avg, wmdd, bann, pos, tn, awr, lbl, cfg, wfr in wf_ranked
                      if cfg['vol_method'] != 'none' and wmdd > b_mdd_val]
        if better_mdd:
            better_mdd.sort(key=lambda x: -x[0])
            print(f"\n  Configs with better worst WF MDD than baseline ({b_mdd_val:.0f}%):")
            for avg_ann, worst_mdd, best_ann, pos, total_n, avg_wr, label, cfg, wf_res in better_mdd[:10]:
                ws = " | ".join([f"{yr}:{r['ann']:+.0f}%/{r['mdd']:.0f}%"
                                 for yr, r in sorted(wf_res.items())])
                print(f"  *** WF AVG={avg_ann:+.0f}% | WorstMDD={worst_mdd:.0f}% | {label}")
                print(f"      {ws}")
        else:
            print(f"\n  No configs beat baseline worst WF MDD of {b_mdd_val:.0f}%")
    ...
    print(f"\n  Champion config: V121 + Union, aggro100 DD, top_n=3, reg0.5-1.5, noSL, corr<0.5")
    print(f"  Baseline (no vol filter):")
    ...
    if baseline_wf:
        ...
        b_ws = " | ".join([f"{yr}:{r['ann']:+.0f}%/{r['mdd']:.0f}%"
                           for yr, r in sorted(b_wf.items())])
        print(f"    WF AVG={b_avg:+.0f}% | WorstMDD={b_wmdd:.0f}%")
        print(f"    {b_ws}")

    if wf_ranked:
        best = wf_ranked[0]
        avg_ann, worst_mdd, best_ann, pos, total_n, avg_wr, label, cfg, wf_res = best
        print(f"\n  Best V164 config: {label}")
        print(f"    WF AVG={avg_ann:+.0f}% | WorstMDD={worst_mdd:.0f}% | {pos}/6 pos")
        ws = " | ".join([f"{yr}:{r['ann']:+.0f}%/{r['mdd']:.0f}%"
                         for yr, r in sorted(wf_res.items())])
        print(f"    {ws}")

        # Best vol-filtered only
        vol_only = [(avg, wmdd, bann, pos, tn, awr, lbl, cfg, wfr)
                    for avg, wmdd, bann, pos, tn, awr, lbl, cfg, wfr in wf_ranked
                    if cfg['vol_method'] != 'none']
        if vol_only:
            best_v = vol_only[0]
            avg_ann, worst_mdd, best_ann, pos, total_n, avg_wr, label, cfg, wf_res = best_v
            print(f"\n  Best VOL-FILTERED V164: {label}")
            print(f"    WF AVG={avg_ann:+.0f}% | WorstMDD={worst_mdd:.0f}% | {pos}/6 pos")
            ws = " | ".join([f"{yr}:{r['ann']:+.0f}%/{r['mdd']:.0f}%"
                             for yr, r in sorted(wf_res.items())])
            print(f"    {ws}")

            # Best risk-adjusted vol-filtered
            vol_ra = sorted(vol_only, key=lambda x: -abs(x[0] / x[1]) if x[1] != 0 else 0)
            best_ra = vol_ra[0]
            avg_ann, worst_mdd, best_ann, pos, total_n, avg_wr, label, cfg, wf_res = best_ra
            ra_ratio = abs(avg_ann / worst_mdd) if worst_mdd != 0 else 0
            print(f"\n  Best RISK-ADJUSTED VOL-FILTERED V164: {label}")
            print(f"    WF AVG={avg_ann:+.0f}% | WorstMDD={worst_mdd:.0f}% | R/M={ra_ratio:.2f} | {pos}/6 pos")
            ws = " | ".join([f"{yr}:{r['ann']:+.0f}%/{r['mdd']:.0f}%"
                             for yr, r in sorted(wf_res.items())])
            print(f"    {ws}")
