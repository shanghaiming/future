"""
V48: V43 Dynamic Mode + Dynamic Hold Period
=============================================
V43 is our ALL-TIME BEST: Sharpe 4.99, MDD 13.3%, ann +21.6%, 150 trades.

V48 innovation: Hold period adapts based on mode AND trade P&L.
- WINNING mode: hold longer (5-10 days) to capture bigger moves
- NORMAL mode: hold 5 days (standard)
- LOSING mode: hold shorter (3-4 days) for quick exits
- Early exit: If trade is profitable after day 2, allow trailing stop
- Late exit: Force exit at max hold period (no hoping)

Implementation:
  hold_by_mode = {'winning': hold_win, 'normal': 5, 'losing': hold_lose}
  After day 2, if unrealized P&L > 0, set trailing stop at entry price
  Force exit at max hold period
  ATR stop still applies regardless
...
3. NEW: Mode-dependent hold period + early profit-taking
4. KER gate < 0.15, ATR stop 3.0
...
# ---------------------------------------------------------------------------
# Factor computation (identical to V43)
# ---------------------------------------------------------------------------
...
def compute_raw_factors(
    ...
    t0 = time.time()
    print("[V48] Computing raw factors...", flush=True)
...
def compute_cross_sectional_ranks(
    ...
    t0 = time.time()
    print("[V48] Computing cross-sectional ranks...", flush=True)
...
def build_composite_signal(
    ...
    t0 = time.time()
    print("[V48] Building composite signal...", flush=True)
...
# ---------------------------------------------------------------------------
# V48 Backtest: dynamic hold period based on mode + early profit-taking
# ---------------------------------------------------------------------------
...
def backtest_v48(
    ...
    """Backtest V48 with dynamic hold periods per mode."""
    ...
    for di in range(max(start_di, 1), end_di):
        ...
        for si, pos_list in pos_by_si.items():
            ...
            # Determine max hold from entry mode (not current mode)
            entry_mode = pos_list[0][5]
            max_hold = hold_by_mode.get(entry_mode, hold_normal)

            # ATR stop check
            stopped = any(c < sp for _, _, sp, _, _, _ in pos_list)

            # NEW V48: Early profit-taking with trailing stop
            trailing_stop_hit = False
            if early_exit and hold >= 2 and not stopped:
                # Check if all positions for this symbol are profitable
                all_profitable = True
                avg_ep = np.mean([ep for _, ep, _, _, _, _ in pos_list])
                if c <= avg_ep:
                    all_profitable = False

                if all_profitable and hold >= 3:
                    # Trailing stop: if price drops below entry after day 3
                    # and was previously profitable, exit
                    trailing_stop_hit = c < avg_ep
                    # Also check if any individual position has a trailing stop
                    # triggered (entry price as floor)
                    for _, ep, _, _, _, _ in pos_list:
                        if c < ep:
                            trailing_stop_hit = True
                            break

            if stopped:
                for edi, ep, sp, alloc, is_pyr, em in pos_list:
                    pnl = (c - ep) / ep - COMM
                ...
                # Early profit-taking: exit with trailing stop
                for edi, ep, sp, alloc, is_pyr, em in pos_list:
                    pnl = (c - ep) / ep - COMM
                    profit = equity * alloc * pnl
                    daily_pnl += profit
                ...
                # NEW V48: Update trailing stop if profitable after day 2
                for edi, ep, sp, alloc, is_pyr, em in pos_list:
                    new_sp = sp
                    if early_exit and hold >= 2 and c > ep:
                        # Trail stop to entry price (breakeven)
                        new_sp = max(sp, ep)
                    new_positions.append(
                        (si, edi, ep, new_sp, alloc, is_pyr, em))

        # Pyramid on day-1 winners (ratio varies by mode)
        if current_pyramid_ratio > 0:
            held_with_pos: Dict[int, List] = defaultdict(list)
            for si, edi, ep, sp, alloc, is_pyr, em in new_positions:
                held_with_pos[si].append((edi, ep, sp, alloc, is_pyr, em))
        ...
        for rank_val, si, alloc in candidates[:current_top_n]:
            ...
            if atr is None:
                continue
            positions.append(
                (si, di + 1, ep, ep - atr_stop * atr, alloc, False, mode))
            held.add(si)

    # Close remaining positions
    for si, edi, ep, sp, alloc, is_pyr, em in positions:
        c = C[si, ND - 1]
...
# ---------------------------------------------------------------------------
# Analysis (extended for V48 hold period stats)
# ---------------------------------------------------------------------------
...
def analyze(trades: List[dict], equity: float, max_dd: float,
    ...
    n_trail = sum(1 for t in trades if t["reason"] == "trailing")
    ...
    for t in trades:
        em = t.get("entry_mode", "normal")
        if em == "winning":
            entry_mode_counts["WIN"] += 1
        elif em == "losing":
            entry_mode_counts["LOSE"] += 1
        else:
            entry_mode_counts["NORM"] += 1

    # Average hold by exit reason
    avg_hold_stop = 0.0
    avg_hold_trail = 0.0
    avg_hold_time = 0.0
    stop_days = [t["days"] for t in trades if t["reason"] == "stop"]
    trail_days = [t["days"] for t in trades if t["reason"] == "trailing"]
    hold_days = [t["days"] for t in trades if t["reason"] == "hold"]
    if stop_days:
        avg_hold_stop = np.mean(stop_days)
    if trail_days:
        avg_hold_trail = np.mean(trail_days)
    if hold_days:
        avg_hold_time = np.mean(hold_days)

    print(
        f"  {label}: {len(trades)}t (base:{n_base} pyr:{n_pyr} "
        f"stop:{n_stop} hold:{n_hold} trail:{n_trail}) "
    ...
    print(
        ...
        f"entry=[W:{entry_mode_counts['WIN']} N:{entry_mode_counts['NORM']} "
        f"L:{entry_mode_counts['LOSE']}]"
    )
    print(
        f"    avg_hold: stop={avg_hold_stop:.1f}d trail={avg_hold_trail:.1f}d "
        f"time={avg_hold_time:.1f}d"
    )
...
def walk_forward(
    ...
    print(
        ...
        f"hw={hold_win} hl={hold_lose} ee={early_exit})"
    )
    print(f"{'=' * 70}")
    ...
    for test_year in range(2019, years[-1] + 1):
        ...
        if test_trades:
            ...
            modes = {"W": 0, "N": 0, "L": 0}
            exit_reasons = {"stop": 0, "hold": 0, "trailing": 0}
            for t in test_trades:
                ...
                if m == "WIN":
                    ...
                    modes["N"] += 1
                exit_reasons[t["reason"]] = exit_reasons.get(
                    t["reason"], 0) + 1
            print(
                f"  {test_year}: {n}t WR={wr_val:.1f}% avg={avg:+.2f}% "
                f"modes=[W:{modes['W']} N:{modes['N']} L:{modes['L']}] "
                f"exit=[s:{exit_reasons.get('stop', 0)} "
                f"h:{exit_reasons.get('hold', 0)} "
                f"t:{exit_reasons.get('trailing', 0)}]",
                flush=True,
            )
        else:
    ...
    if all_trades:
        ...
        n_trail = sum(1 for t in all_trades if t["reason"] == "trailing")
        ...
        print(
            f"  Exit distribution: stop={n_stop} hold={n_hold} "
            f"trailing={n_trail}"
        )
        return all_trades
    return []
...
# ---------------------------------------------------------------------------
# Main: sweep + walk-forward + full 10yr
# ---------------------------------------------------------------------------

def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("  V48: V43 DYNAMIC MODE + DYNAMIC HOLD PERIOD")
    print("  Mode-dependent hold + early profit-taking + trailing stop")
    print("=" * 70)
    ...
    # === 1. Default configs walk-forward ===
    print("\n" + "=" * 70)
    print("  WALK-FORWARD VALIDATION (2019-2026) -- DEFAULT CONFIGS")
    print("=" * 70)

    default_configs = [
        # (wt, nt, lt, ww, ats, tnw, hw, hl, ee, label)
        (0.60, 0.82, 0.90, 15, 3.0, 2, 7, 3, True, "Default (hw7/hl3/ee)"),
        (0.60, 0.82, 0.90, 15, 3.0, 2, 5, 3, True, "Short win hold (hw5/hl3)"),
        (0.60, 0.82, 0.90, 15, 3.0, 2, 10, 4, True, "Long win hold (hw10/hl4)"),
        (0.60, 0.82, 0.90, 15, 3.0, 2, 7, 3, False, "No early exit (hw7/hl3)"),
        (0.60, 0.82, 0.90, 10, 3.0, 2, 7, 3, True, "Short window (ww10)"),
    ]

    for wt, nt, lt, ww, ats, tnw, hw, hl, ee, label in default_configs:
        print(f"\n  --- {label} ---")
        walk_forward(
            C, O, H, L, NS, ND, dates, syms, sigs,
            win_threshold=wt, normal_threshold=nt, lose_threshold=lt,
            win_rate_window=ww, atr_stop=ats, top_n_winning=tnw,
            hold_win=hw, hold_lose=hl, early_exit=ee,
        )
    ...
    sweep_params = {
        ...
        # Keep V43 best mode thresholds fixed
        "win_threshold": [0.60],
    ...
    combo_count = 0
    for hw, hl, ee, ww, wt, nt, lt, ats, tnw in product(
        sweep_params["hold_win"],
        sweep_params["hold_lose"],
        ...
        results.append({
            "hw": hw, "hl": hl, "ee": ee, "ww": ww,
            "wt": wt, "nt": nt, "lt": lt, "ats": ats, "tnw": tnw,
            "n": len(trades), "wr": wr, "ann": ann,
            "dd": dd, "sharpe": sh_val, "eq": eq,
            "n_trail": n_trail, "avg_days": avg_days,
        })
    ...
    for r in results[:30]:
        print(
            f"{r['hw']:>3} {r['hl']:>3} "
            f"{'T' if r['ee'] else 'F':>5} {r['ww']:>3} "
            f"{r['n']:>5} {r['wr']:>5.1f} {r['ann']:>+8.1f} "
            f"{r['dd']:>6.1f} {r['sharpe']:>6.2f} "
            f"{r['n_trail']:>4} {r['avg_days']:>5.1f}"
        )
    ...
    for r in results[:5]:
        ...
        label = (
            f"hw={r['hw']} hl={r['hl']} ee={'T' if r['ee'] else 'F'} "
            f"ww={r['ww']}"
        )
        print(f"\n  FULL {label}")
    ...
    if results:
        ...
        print("\n" + "=" * 70)
        print(
            f"  BEST WF: hw={best['hw']} hl={best['hl']} "
            f"ee={'T' if best['ee'] else 'F'} ww={best['ww']}"
        )
        print("=" * 70)
        walk_forward(
            C, O, H, L, NS, ND, dates, syms, sigs,
        ...
        # === 5. Compare V48 vs V43 baseline ===
        print("\n" + "=" * 70)
        print("  COMPARISON: V48 DYNAMIC HOLD vs V43 FIXED HOLD")
        print("  (2019-2026 OOS)")
        print("=" * 70)

        # V48 best config
        trades_v48, eq_v48, dd_v48 = backtest_v48(
            C, O, H, L, NS, ND, dates, syms, sigs,
            win_threshold=best["wt"],
            normal_threshold=best["nt"],
            lose_threshold=best["lt"],
        ...
        # V43-like: fixed hold=5, no early exit
        trades_v43, eq_v43, dd_v43 = backtest_v48(
            C, O, H, L, NS, ND, dates, syms, sigs,
            win_threshold=best["wt"],
            normal_threshold=best["nt"],
            lose_threshold=best["lt"],
        ...
        print(f"\n  V48 DYNAMIC HOLD:")
        analyze(trades_v48, eq_v48, dd_v48, "V48-dynamic")
        print(f"\n  V43-LIKE FIXED HOLD=5:")
        analyze(trades_v43, eq_v43, dd_v43, "V43-fixed")

        if trades_v48 and trades_v43:
            print(
                f"\n  V48 vs V43: "
                f"eq_delta={eq_v48 - eq_v43:+,.0f} "
                f"dd_delta={dd_v48 - dd_v43:+.1f}%"
            )

    print(f"\n[V48] Done. {time.time() - t0:.1f}s")
