"""
V58: V47 Sector Limit + Leverage Parameter
============================================
V47 = ALL-TIME BEST: Sharpe 6.65, MDD 8.8%, ann +19.8%, 112 trades (OOS 2019-2026).
V47 = V43 dynamic mode switching + max 2 trades per sector per day.

V58 adds leverage support to V47:
  1. `leverage` parameter (default=1.0) scales all P&L
  2. Contract counts and notional exposure via contract_specs
  3. Margin utilization tracking
  4. Leverage sweep: 1.0, 1.5, 2.0, 2.5, 3.0x
  5. Walk-forward at each leverage level

Architecture (same as V47):
  - V18's 7 cross-sectional ranks, composite score
  - Dynamic three-mode threshold (WINNING/NORMAL/LOSING)
  - Sector-constrained selection (max N per sector per day)
  - KER gate < 0.15, hold 5d, ATR stop 3.0
...
def compute_raw_factors(
    ...
    t0 = time.time()
    print("[V58] Computing raw factors...", flush=True)
...
def compute_cross_sectional_ranks(
    ...
    t0 = time.time()
    print("[V58] Computing cross-sectional ranks...", flush=True)
...
def build_composite_signal(
    ...
    t0 = time.time()
    print("[V58] Building composite signal...", flush=True)
...
def backtest_v58(
    ...
    # V58 NEW: leverage parameter
    leverage: float = 1.0,
) -> Tuple[List[dict], float, float]:
    """Backtest V58: V47 sector limit + leverage + contract specs.

    leverage: multiplier on position exposure.
      - 1.0 = no leverage (standard V47 behavior)
      - 2.0 = 2x leverage
      - 0.5 = half exposure
        ...
        for si, pos_list in pos_by_si.items():
            ...
            if stopped:
                for edi, ep, sp, alloc, is_pyr in pos_list:
                    ...
                    margin_rate = get_margin_rate(sym)
                    contracts = max(
                        1, int(equity * alloc * leverage / (ep * mult)))
...
def analyze(trades: List[dict], equity: float, max_dd: float,
    ...
    total_margin = sum(t.get("margin_used", 0) for t in trades)
    ...
    print(f"    contracts: avg={avg_contracts:.0f} "
          f"total_notional={total_notional:,.0f} "
          f"total_margin={total_margin:,.0f}")
    ...
    return {
        ...
        "ann": ann, "sh": sh, "eq": equity, "leverage": leverage,
...
def walk_forward(
    ...
    """Walk-forward validation: year-by-year out-of-sample with leverage."""
    print(f"\n{'=' * 70}")
    print(
        f"  WALK-FORWARD V58 "
        f"(lev={leverage:.1f}x wt={win_threshold:.2f} "
    ...
    if all_trades:
        ...
        avg_contracts = np.mean(
            [t.get("contracts", 0) for t in all_trades])
        total_notional = sum(t.get("notional", 0) for t in all_trades)
        print(
        ...
        print(f"  WF CONTRACTS: avg={avg_contracts:.0f} "
              f"total_notional={total_notional:,.0f}")
        return all_trades
    return []
...
def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("  V58: V47 SECTOR LIMIT + LEVERAGE PARAMETER")
    print("  V47 dynamic threshold + sector limit + leverage scaling")
    print("=" * 70)
    ...
    multipliers = get_all_multipliers(syms)
    print(f"  Contract multipliers loaded for {len(multipliers)} instruments")
    ...
    # ================================================================
    # 1. LEVERAGE SWEEP (default V47 params)
    # ================================================================
    print("\n" + "=" * 70)
    print("  LEVERAGE IMPACT ANALYSIS (V47 default params)")
    print("=" * 70)
    ...
    for lev in leverage_levels:
        ...
        trades, eq, dd = backtest_v58(
            C, O, H, L, NS, ND, dates, syms, sigs,
            sector_lookup=sector_lookup,
            leverage=lev,
            **best_v47_params,
        )
        label = f"V58-lev{lev:.1f}"
    ...
    print(
        f"  {'Lev':>4} {'N':>5} {'WR':>5} {'Ann':>8} "
        f"{'DD':>6} {'Sh':>6} {'Eq':>14} {'AvgCtr':>7}"
    )
    print("-" * 70)
    for r in leverage_results:
        print(
            f"  {r['leverage']:>4.1f}x {r['n']:>5} {r['wr']:>5.1f} "
            f"{r['ann']:>+8.1f}% {r['dd']:>6.1f}% {r['sh']:>6.2f} "
            f"{r['eq']:>14,.0f} {r.get('avg_contracts', 0):>7.0f}"
        )

    # ================================================================
    # 2. WALK-FORWARD AT EACH LEVERAGE LEVEL
    # ================================================================
    print("\n" + "=" * 70)
    print("  WALK-FORWARD BY LEVERAGE (2019-2026)")
    print("=" * 70)
    ...
    if wf_summary:
        print("\n" + "-" * 60)
        print("  WF LEVERAGE SUMMARY (2019-2026 OOS)")
        print(
            f"  {'Lev':>4} {'N':>5} {'WR':>5} {'Cum':>10} {'Sh':>6}"
        )
        print("-" * 60)
        for r in wf_summary:
            print(
                f"  {r['leverage']:>4.1f}x {r['n']:>5} "
                f"{r['wr']:>5.1f}% {r['cum']:>+9.1f}% {r['sharpe']:>6.2f}"
            )

    # ================================================================
    # 3. PARAMETER SWEEP AT LEVERAGE 1.0
    # ================================================================
    print("\n" + "=" * 70)
    print("  PARAMETER SWEEP (leverage=1.0, OOS 2019-2026)")
    print("=" * 70)
    ...
    combo_count = 0
    for wt, nt, lt, ww, mps, tnw in product(
        ...
        if combo_count % 50 == 0:
            print(f"  Progress: {combo_count}/{total_combos} "
                  f"best_sh={results[0]['sharpe']:.2f}"
                  if results else "N/A", flush=True)
    ...
    # ================================================================
    # 4. TOP CONFIG: FULL 10-YEAR + WF + LEVERAGE SWEEP
    # ================================================================
    if results:
        best = results[0]
        print("\n" + "=" * 70)
        print(
            f"  BEST CONFIG: wt={best['wt']:.2f} nt={best['nt']:.2f} "
            f"lt={best['lt']:.2f} ww={best['ww']} "
            f"mps={best['mps']} tnw={best['tnw']}"
        )
        print("=" * 70)

        # Full 10-year at leverage 1.0
        trades, eq, dd = backtest_v58(
            C, O, H, L, NS, ND, dates, syms, sigs,
            sector_lookup=sector_lookup,
            leverage=1.0,
        ...
        print("\n  FULL 10-YEAR (lev=1.0)")
        analyze(trades, eq, dd, "V58-best-lev1.0", leverage=1.0)
        ...
        print("\n" + "=" * 70)
        print("  BEST CONFIG + LEVERAGE SWEEP (full 10yr)")
        print("=" * 70)
        ...
        for lev in leverage_levels:
            ...
            label = f"V58-best-lev{lev:.1f}"
            result = analyze(trades, eq, dd, label, leverage=lev)
            if result:
                best_lev_results.append(result)

            # WF for each leverage
            print(f"\n  WF (best, lev={lev}):", flush=True)
            walk_forward(
                C, O, H, L, NS, ND, dates, syms, sigs,
                sector_lookup=sector_lookup,
                leverage=lev,
        ...
        # Final leverage comparison table
        if best_lev_results:
            print("\n" + "=" * 70)
            print("  FINAL LEVERAGE COMPARISON (best config, 10yr)")
            print(
                f"  {'Lev':>4} {'N':>5} {'WR':>5} {'Ann':>8} "
                f"{'DD':>6} {'Sh':>6} {'Eq':>14} {'AvgCtr':>7}"
            )
            print("-" * 70)
            for r in best_lev_results:
                print(
                    f"  {r['leverage']:>4.1f}x {r['n']:>5} "
