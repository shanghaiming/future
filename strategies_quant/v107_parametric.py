"""
V107 — Parametric exploration: one script, many parallel experiments
====================================================================
Usage: python3 v107_parametric.py --seed 42 --boost-rounds 300 --variant myexp
All args have defaults matching V98 base config.
"""
import sys, os, time, warnings, pickle, argparse
...
parser.add_argument('--train-window-years', type=float, default=3.0)
parser.add_argument('--retrain-freq', type=int, default=42)
parser.add_argument('--meta-rounds', type=int, default=100)
parser.add_argument('--n-ensemble', type=int, default=3, help='3 or 5 ensemble configs')
parser.add_argument('--top-k', type=int, default=1, help='Pick from top-K stocks')
parser.add_argument('--n-jobs', type=int, default=2)
parser.add_argument('--variant', type=str, default='base')
parser.add_argument('--quick', action='store_true', help='Skip no-meta search')
args = parser.parse_args()

COMMISSION = 0.0003; STAMP_DUTY = 0.001; CASH0 = 500_000

print("=" * 70, flush=True)
print(f"  V107 variant={args.variant} seed={args.seed} boost={args.boost_rounds} "
      f"win={args.train_window_years}yr freq={args.retrain_freq}d "
      f"meta_rounds={args.meta_rounds} n_ens={args.n_ensemble} top_k={args.top_k}", flush=True)
print("=" * 70, flush=True)
...
# ====================================================================
# Training infrastructure — parametric
# ====================================================================
TRAIN_WINDOW = int(252 * args.train_window_years)
RETRAIN_FREQ = args.retrain_freq
MIN_TRAIN = 252 * 2; FWD_DAYS = 5

if args.n_ensemble == 5:
    ENSEMBLE_CONFIGS = [
        ...
        {'num_leaves': 10, 'feature_fraction': 0.5, 'learning_rate': 0.04,
         'bagging_fraction': 0.7, 'min_data_in_leaf': 120},
        {'num_leaves': 3,  'feature_fraction': 0.3, 'learning_rate': 0.08,
         'bagging_fraction': 0.5, 'min_data_in_leaf': 60},
    ]
else:
    ENSEMBLE_CONFIGS = [
        {'num_leaves': 7,  'feature_fraction': 0.4, 'learning_rate': 0.05,
...
N_ENS = len(ENSEMBLE_CONFIGS)

LGB_BASE = {
    'objective': 'lambdarank', 'metric': 'ndcg',
    'bagging_freq': 3, 'label_gain': [1, 2, 3, 4, 5],
    'verbose': -1, 'n_jobs': args.n_jobs,
}
...
def train_primary(global_seed):
    ...
    for train_di in range(MIN_TRAIN, ND, RETRAIN_FREQ):
        ...
        for ci, cfg in enumerate(ENSEMBLE_CONFIGS):
            ...
            model = lgb.train(params, train_data, num_boost_round=args.boost_rounds)
            ...
            for di in range(train_di, pred_end):
                ...
                if pred_list:
                    ...
                    for k, si in enumerate(pred_si):
                        SCORE_ALL[ci][si, di] = scores[k]

    # Rank-normalize and average
    SCORE_NORM = [np.full((NS, ND), np.nan) for _ in range(N_ENS)]
    for ci in range(N_ENS):
        for di in range(MIN_TRAIN, ND):
            vals = SCORE_ALL[ci][:, di]
            mask = ~np.isnan(vals)
            if mask.sum() < 50: continue
            ranked = np.argsort(np.argsort(vals[mask])).astype(float)
            n = len(ranked)
            for k, idx in enumerate(np.where(mask)[0]):
                SCORE_NORM[ci][idx, di] = ranked[k] / max(n-1, 1) * 100

    return np.nanmean(np.stack(SCORE_NORM, axis=0), axis=0), retrain_points

def train_meta(SCORE_PRI, retrain_points):
    ...
    for rp_idx, (train_di, pred_end) in enumerate(retrain_points):
        ...
        try:
            ...
            meta_model = lgb.train(META_PARAMS, meta_train_data, num_boost_round=args.meta_rounds)
        except: continue
...
def run_backtest(score_arr, meta_arr, meta_threshold, sl_pct, tp_pct,
    ...
    for di in range(MIN_TRAIN, ND):
        ...
        if pos is None and pending is None:
            ...
            # Pick the best among top-K (or random from top-K)
            if args.top_k > 1:
                pick_idx = rng.randint(0, top_k)
            else:
                pick_idx = 0
            best_si = scored[pick_idx][1]
            best_score = scored[pick_idx][0]
    ...
    return {'ann': round(ann,1), 'n': len(trades), 'wr': round(wr,1),
            ...
            'max_dd': round(max_dd,1), 'trades': trades,
            'n_filtered': n_filtered, 'n_total': n_total}

def quick_search(SCORE_PRI, SCORE_META):
    results = []
        ...
        for hm in [5, 6, 7, 8]:
            for sl in [4, 5, 6]:
                for ts in [3, 4, 5]:
                    ...
                    if r:
                        t = r.pop('trades')
                        results.append({**r, 'hm':hm,'sl':sl,'ts':ts,'meta_th':th,
                                       'label':f'meta{th:.2f}_hm{hm}_sl{sl}_ts{ts}'})
                    for amin in [0.5, 0.6]:
                        for arng in [0.8, 1.0]:
                            r = run_backtest(SCORE_PRI, SCORE_META, th, sl, 50, hm, 1, ts,
                            ...
                            if r:
                                t = r.pop('trades')
                                results.append({**r, 'hm':hm,'sl':sl,'ts':ts,'meta_th':th,
                                               'label':f'meta{th:.2f}_hm{hm}_sl{sl}_atr'})
    if not args.quick:
        for hm in [5, 6, 7, 8]:
            for sl in [4, 5, 6, 7]:
                for ts in [3, 4, 5]:
                    ...
                    if r:
                        t = r.pop('trades')
                        results.append({**r, 'hm':hm,'sl':sl,'ts':ts,'meta_th':0,
                                       'label':f'nometa_hm{hm}_sl{sl}_ts{ts}'})
...
# ====================================================================
t_seed = time.time()
print(f"\n[5] Training primary (seed={args.seed})...", flush=True)
SCORE_PRI, retrain_points = train_primary(args.seed)
print(f"  Primary done ({time.time()-t_seed:.1f}s)", flush=True)

print(f"[6] Training meta (rounds={args.meta_rounds})...", flush=True)
t_meta = time.time()
SCORE_META = train_meta(SCORE_PRI, retrain_points)
print(f"  Meta done ({time.time()-t_meta:.1f}s)", flush=True)

print(f"[7] Parameter search...", flush=True)
t_search = time.time()
results = quick_search(SCORE_PRI, SCORE_META)
print(f"  {len(results)} configs ({time.time()-t_search:.1f}s)", flush=True)

if results:
    best = results[0]
    print(f"\n{'='*70}", flush=True)
    print(f"  RESULT [{args.variant}]: {best['ann']:+.1f}% WR={best['wr']:.0f}% "
          f"Edge={best['edge']:+.2f}% TPY={best['tpy']:.0f} DD={best['max_dd']:.0f}% "
          f"({best['label']})", flush=True)
    # Top 5
    for i, r in enumerate(results[:5]):
        print(f"  #{i+1}: {r['ann']:+.1f}% WR={r['wr']:.0f}% Edge={r['edge']:+.2f}% "
              f"TPY={r['tpy']:.0f} DD={r['max_dd']:.0f}% ({r['label']})", flush=True)
    print(f"{'='*70}", flush=True)
else:
    print(f"  [{args.variant}] No valid results!", flush=True)
