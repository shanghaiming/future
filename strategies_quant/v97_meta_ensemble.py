"""
V97 — Meta Model Ensemble (3 meta configs averaged)
====================================================
Same 3-LGB ensemble primary as V89. Key change in meta:
Train 3 meta models with different configs, rank-normalize and average.
Goal: More robust meta signal → higher Edge.
"""
...
print("=" * 70, flush=True)
print("  V97 — Meta Model Ensemble (3 meta configs averaged)", flush=True)
print("=" * 70, flush=True)
...
# ====================================================================
# [6] META ENSEMBLE — 3 configs, rank-normalized average
# ====================================================================
print(f"\n[6] Meta model ensemble (3 configs)...", flush=True)
t6 = time.time()
...
META_CONFIGS = [
    ...
    {'objective': 'binary', 'metric': 'auc',
     ...
     'bagging_freq': 3, 'min_data_in_leaf': 150,
     'verbose': -1, 'n_jobs': -1, 'seed': 43},
    {'objective': 'binary', 'metric': 'auc',
     'learning_rate': 0.07, 'num_leaves': 7,
     'feature_fraction': 0.4, 'bagging_fraction': 0.6,
     'bagging_freq': 3, 'min_data_in_leaf': 250,
     'verbose': -1, 'n_jobs': -1, 'seed': 44},
]
N_META_ENS = len(META_CONFIGS)
META_ROUNDS = 100
...
# Store individual meta model predictions
META_SCORES = [np.full((NS, ND), np.nan) for _ in range(N_META_ENS)]
n_meta_trains = 0
...
for rp_idx, (train_di, pred_end) in enumerate(retrain_points):
    ...
    # Train all meta configs
    meta_models = []
    for mc in META_CONFIGS:
        try:
            meta_train_data = lgb.Dataset(mX, label=my)
            mm = lgb.train(mc, meta_train_data, num_boost_round=META_ROUNDS)
            meta_models.append(mm)
        except Exception:
    ...
    for di in range(train_di, pred_end):
        ...
        if pred_list:
            pX = np.array(pred_list, dtype=np.float32)
            for mi, mm in enumerate(meta_models):
                if mm is None: continue
                probs = mm.predict(pX)
                for k, si in enumerate(pred_si):
                    META_SCORES[mi][si, di] = probs[k]

    if n_meta_trains % 5 == 0:
        n_pos = my.sum()
        print(f"    meta #{n_meta_trains}: di={train_di}, samples={len(my)}, "
              f"pos={int(n_pos)}, neg={len(my)-int(n_pos)}, "
              f"models={sum(1 for m in meta_models if m is not None)}", flush=True)

# Average meta scores
SCORE_META = np.nanmean(np.stack(META_SCORES, axis=0), axis=0)
print(f"  Meta ensemble: {n_meta_trains} trainings × {N_META_ENS} configs ({time.time()-t6:.1f}s)", flush=True)
...
print(f"  Ensemble primary ({N_ENS} configs rank-normalized) + walk-forward meta", flush=True)
