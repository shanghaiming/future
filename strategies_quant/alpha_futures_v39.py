"""
Alpha Futures V39 — Supply Chain Pair/Spread Trading
=====================================================
Market-neutral pair trading within commodity supply chains.

Core idea: Commodity supply chains have stable price relationships.
When the spread between upstream and downstream deviates from its
rolling mean, trade the convergence:
  - spread > mean + k*std: Short downstream, Long upstream (will compress)
  - spread < mean - k*std: Long downstream, Short upstream (will expand)

Supply chain pairs:
  Iron ore (ifi) -> Rebar (rbfi), Hot coil (hcfi)
  Crude oil (scfi) -> Methanol (mafi), Fuel oil (fufi), Bitumen (bfi)
  Soybean (afi) -> Soybean meal (mfi), Soybean oil (yfi)
  Coal (jmfi) -> Coke (jfi)
  Palm oil (pfi) vs Soybean oil (yfi)
  PP/V/EG vs Methanol (mafi)
...
Config sweep:
  ...
  hold_days = [3, 5, 7, 10]
  max_pairs = [1, 2, 3]
  Walk-forward for best configs (2023, 2024)
"""
...
# Supply chain pairs: (downstream, upstream)
PAIRS = [
...
def main():
    t_start = time.time()
    print("=" * 120)
    print("Alpha Futures V39 — Supply Chain Pair/Spread Trading")
    print("Market-neutral: long one leg + short the other when spread deviates from rolling mean")
    print("=" * 120)

    NS, ND, dates, C, O, H, L, V, syms, sym_set = load_all_data()
    sym_to_si = {syms[si]: si for si in range(NS)}

    # Build pair index mapping: list of (down_si, up_si) for pairs that exist in data
    pair_indices = []
    ...
    # For each pair, compute spread = price_down - price_up for all dates
    # We'll precompute and store as dict keyed by (down_si, up_si)
    spreads = {}
    ...
    def run_backtest(lookback, z_thresh, hold_max, max_pairs,
        ...
        For each pair, compute rolling z-score of the spread.
        When |z| > z_thresh, enter a pair trade (both legs simultaneously).
        Exit when z crosses 0, or after hold_max days, or stop loss.

        max_pairs: max number of concurrent pair positions.
        """
        cash = float(CASH0)
        trades = []          # list of trade dicts
        pair_positions = []  # list of active pair position dicts

        # Pre-compute per-pair arrays
        pair_data = {}
        for down_si, up_si, down_sym, up_sym in pair_indices:
            ...
            for di in range(lookback, ND):
                ...
                if len(valid) >= lookback * 0.8:  # at least 80% of window valid
        ...
        for di in range(MIN_TRAIN, ND):
            ...
            for pos in pair_positions:
                ...
                # Exit 1: Mean reversion — z crosses 0
                ...
                if exit_reason:
                    ...
                    # Costs: open + close for both legs
                    ...
                    # Return capital (net of original investment)
                    if pos_dir == 1:
                        # Long down: receive c_down*mult*lots, Short up: pay c_up*mult*lots
                        cash_return = c_down * mult_down * lots_down - c_up * mult_up * lots_up
                    else:
                        # Short down: pay c_down*mult*lots, Long up: receive c_up*mult*lots
                        cash_return = -c_down * mult_down * lots_down + c_up * mult_up * lots_up

                    # Add back the original margin (entry values were subtracted from cash)
            ...
            # --- Check if any pair already in position uses a commodity ---
            occupied = set()
            for pos in pair_positions:
            ...
            # Score all pairs by |z| (strongest deviation first)
            candidates = []
            for down_si, up_si, down_sym, up_sym in pair_indices:
            ...
            for _, down_si, up_si, down_sym, up_sym, z_val in candidates[:n_can_open]:
                ...
                # Split cash equally between two legs
                cash_per_leg = cash / 2
                ...
                # Direction: z > threshold -> short down, long up (spread too wide)
                #            z < -threshold -> long down, short up (spread too narrow)
                if z_val > 0:
        ...
        for zt in z_thresholds:
            for hd in hold_days_list:
                for mp in max_pairs_list:
                    ...
                    configs.append((lb, zt, hd, mp, None, name))
            ...
            for hd in [3, 5, 7]:
                for mp in [1, 2, 3]:
                    for wf_year in [2023, 2024]:
                        name = f"LB{lb}_Z{zt:.1f}_H{hd}_MP{mp}_WF{wf_year}"
                        configs.append((lb, zt, hd, mp, wf_year, name))

    print(f"  {len(configs)} configurations to test", flush=True)

    for ci, (lb, zt, hd, mp, wf, name) in enumerate(configs):
        r = run_backtest(lb, zt, hd, mp, wf_split_year=wf, config_name=name)
        if r is not None:
            results.append(r)
            if r['ann'] > 10:
    ...
    print(f"{'=' * 130}")
    hdr = (f"  {'Config':40s} | {'Ann':>7s} | {'WR':>5s} | {'N':>4s} | {'DD':>6s} | "
    ...
    # Per-pair summary across all top 20
    if full_results:
