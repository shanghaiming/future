"""
V101: Adaptive Benchmark Deviation Strategy
=============================================
Core Insight: Price deviation from "smart benchmarks" has more predictive
power than deviation from fixed moving averages.

Innovations (on top of V96's NW+BMA+Vol framework):
1. KAMA_deviation: KAMA auto-adapts - slow in ranges (close to price),
   fast in trends (tracks trend). Deviation from KAMA = regime-aware signal.
2. LSMA_deviation: Least-squares regression line represents "fair value"
   more accurately than SMA. Deviation = statistical mispricing.
3. BOP: Balance of Power - intrabar buy/sell force balance.
   Microstructure signal from candle body ratios.

Factor Combination:
  KAMA_deviation(10)_rank   0.20  Price vs adaptive benchmark
  LSMA_deviation(20)_rank   0.15  Price vs regression line
  CCI(14)_rank              0.15  Statistical deviation
  LINEARREG_ANGLE(20)_rank  0.15  Trend slope
  oi5d_rank                 0.15  Open interest momentum
  MACD_HIST(12,26,9)_rank   0.10  Momentum acceleration
  BOP_rank                  0.10  Intra-bar buy/sell force

Built on V96's NW kernel regression + volatility-adaptive sizing framework.
...
# Sector definitions (same as V96)
SECTOR_MAP = {
...
FACTOR_NAMES = [
    "kama_dev", "lsma_dev", "cci14", "linreg_angle",
    "oi5d", "macd_hist", "bop",
]
N_FACTORS = len(FACTOR_NAMES)

# Factor weights for composite score
FACTOR_WEIGHTS = np.array([0.20, 0.15, 0.15, 0.15, 0.15, 0.10, 0.10])
...
# =====================================================================
# FACTOR COMPUTATION (TA-Lib based adaptive benchmarks)
# =====================================================================

def compute_kama(
    C: np.ndarray, NS: int, ND: int, period: int = 10,
) -> np.ndarray:
    """Compute Kaufman Adaptive Moving Average (KAMA).

    KAMA adapts to market noise:
    - Trending market: fast like EMA
    - Ranging market: slow like SMA (stays close to price)
    """
    ...
    for si in range(NS):
        ...
        try:
            result = talib.KAMA(c, timeperiod=period)
            kama[si] = np.where(nan_mask, np.nan, result)
        except Exception:
            pass
    return kama
...
def compute_lsma(
    C: np.ndarray, NS: int, ND: int, period: int = 20,
) -> np.ndarray:
    """Compute Least Squares Moving Average (LSMA / LINEARREG).

    LSMA fits a linear regression line over the lookback window.
    The endpoint of that line represents the statistically optimal
    "fair value" estimate.
    """
    lsma = np.full((NS, ND), np.nan)
    if not HAS_TALIB:
        return lsma
    ...
    for si in range(NS):
        ...
        nan_mask = np.isnan(C[si])
        try:
            result = talib.LINEARREG(c, timeperiod=period)
            lsma[si] = np.where(nan_mask, np.nan, result)
        except Exception:
            pass
    return lsma
...
def compute_cci(
    ...
    for si in range(NS):
        ...
        try:
            ...
            cci[si] = np.where(nan_mask, np.nan, result)
        except Exception:
            pass
    return cci


def compute_linearreg_angle(
    C: np.ndarray, NS: int, ND: int, period: int = 20,
) -> np.ndarray:
    """Compute Linear Regression Angle (trend slope in degrees)."""
    angle = np.full((NS, ND), np.nan)
    if not HAS_TALIB:
        return angle
    ...
    for si in range(NS):
        ...
        nan_mask = np.isnan(C[si])
        try:
            result = talib.LINEARREG_ANGLE(c, timeperiod=period)
            angle[si] = np.where(nan_mask, np.nan, result)
        except Exception:
            pass
    return angle
...
def compute_macd_hist(
    ...
    """Compute MACD histogram (momentum acceleration)."""
    macd_h = np.full((NS, ND), np.nan)
    if not HAS_TALIB:
        return macd_h
    ...
    for si in range(NS):
        ...
        nan_mask = np.isnan(C[si])
        try:
            _, _, hist = talib.MACD(c, fastperiod=fast, slowperiod=slow,
                                    signalperiod=signal)
            macd_h[si] = np.where(nan_mask, np.nan, hist)
        except Exception:
            pass
    return macd_h
...
def compute_bop(
    ...
    """Compute Balance of Power (intra-bar buy/sell force).

    BOP = (Close - Open) / (High - Low)
    Measures the balance between buyers and sellers within each bar.
    """
    ...
    for si in range(NS):
        o = np.where(np.isnan(O[si]), 0, O[si]).astype(np.float64)
        h = np.where(np.isnan(H[si]), 0, H[si]).astype(np.float64)
        l = np.where(np.isnan(L[si]), 0, L[si]).astype(np.float64)
        c = np.where(np.isnan(C[si]), 0, C[si]).astype(np.float64)
        nan_mask = (np.isnan(O[si]) | np.isnan(H[si])
                    | np.isnan(L[si]) | np.isnan(C[si]))
        try:
            result = talib.BOP(o, h, l, c)
            bop[si] = np.where(nan_mask, np.nan, result)
        except Exception:
            pass
    return bop
...
def compute_raw_factors(
    ...
    """Compute all 7 V101 factors using TA-Lib."""
    t0 = time.time()
    print("[V101] Computing adaptive benchmark factors...", flush=True)

    # 1. KAMA deviation: (Close - KAMA) / KAMA
    kama = compute_kama(C, NS, ND, period=kama_period)
    kama_dev = np.full((NS, ND), np.nan)
    for si in range(NS):
        for di in range(ND):
            if (not np.isnan(C[si, di]) and not np.isnan(kama[si, di])
                    and kama[si, di] > 0):
                kama_dev[si, di] = (C[si, di] - kama[si, di]) / kama[si, di]

    # 2. LSMA deviation: (Close - LSMA) / LSMA
    lsma = compute_lsma(C, NS, ND, period=20)
    lsma_dev = np.full((NS, ND), np.nan)
    for si in range(NS):
        for di in range(ND):
            if (not np.isnan(C[si, di]) and not np.isnan(lsma[si, di])
                    and lsma[si, di] > 0):
                lsma_dev[si, di] = (C[si, di] - lsma[si, di]) / lsma[si, di]

    # 3. CCI(14)
    cci14 = compute_cci(H, L, C, NS, ND, period=14)

    # 4. Linear Regression Angle(20)
    linreg_angle = compute_linearreg_angle(C, NS, ND, period=20)
    ...
    # 6. MACD histogram
    macd_hist = compute_macd_hist(C, NS, ND)

    # 7. BOP
    bop = compute_bop(O, H, L, C, NS, ND)

    # Forward 5-day return (target for NW kernel)
    ...
    return {
        "kama_dev": kama_dev, "lsma_dev": lsma_dev,
        "cci14": cci14, "linreg_angle": linreg_angle,
        "oi5d": oi5d, "macd_hist": macd_hist, "bop": bop,
...
def cross_sectional_rank(
    ...
    """Cross-sectional rank normalization: rank among instruments, scale to [0,1]."""
...
def compute_composite_score(
    ...
    """Compute weighted composite score from ranked factors.

    Uses cross-sectional rank (percentile) for each factor, then
    applies fixed weights from FACTOR_WEIGHTS.
    """
    t0 = time.time()
    print("[V101] Computing composite score...", flush=True)

    # Rank each factor cross-sectionally
    ranked = {}
    for fname in FACTOR_NAMES:
        ranked[fname] = cross_sectional_rank(
    ...
    for fi, fname in enumerate(FACTOR_NAMES):
        ...
        for si in range(NS):
            for di in range(ND):
                if not np.isnan(r[si, di]):
                    composite[si, di] += w * r[si, di]
                    valid_count[si, di] += 1

    # Only keep where we have enough factor coverage
    result = np.full((NS, ND), np.nan)
    for si in range(NS):
        for di in range(ND):
            if valid_count[si, di] >= 5:
                result[si, di] = composite[si, di]

    print(f"  Composite score done: {time.time() - t0:.1f}s", flush=True)
    return result


# =====================================================================
# BMA (Bayesian Model Averaging) for adaptive factor weighting
# =====================================================================
...
def compute_rolling_ic(
    ...
    """Compute rolling Information Coefficient for each factor."""
    t0 = time.time()
    print(f"[V101] Computing rolling IC (window={ic_window})...", flush=True)
...
def compute_bma_weights(
    ...
    t0 = time.time()
    print(f"[V101] Computing BMA weights (prior={prior_strength:.1f})...",
          flush=True)
...
def compute_nw_predicted_returns(
    ...
    """NW kernel regression with BMA-weighted V101 features."""
    t0 = time.time()
    print(f"[V101] Computing NW+BMA predicted returns "
    ...
    for di in range(training_window + 10, ND):
        ...
        if di % 100 == 0:
            ...
            print(f"  di={di}/{ND} valid={valid_count}/{NS} "
...
# =====================================================================
# Backtest engine (V96 framework with V101 composite score)
# =====================================================================
...
def backtest_v101(
    ...
    """Backtest V101: NW+BMA kernel + composite score + vol-adaptive sizing.

    Uses a blended signal: 60% NW predicted return + 40% composite score.
    """
    if end_di is None:
        end_di = ND - 1
    ...
    for di in range(max(start_di, 1), end_di):
        ...
        # --- ENTRY: blend NW predicted + composite score ---
        ...
        for si in range(NS):
            ...
            comp = composite_score[si, di]
            if np.isnan(pred) and np.isnan(comp):
                continue
            ...
            # Blended signal: 60% NW predicted + 40% composite score
            if np.isnan(pred):
                blend = comp * 0.4
            elif np.isnan(comp):
                blend = pred * 0.6
            else:
                blend = pred * 0.6 + comp * 0.4

            # Filter: require composite score > 0.5 (above median)
            if not np.isnan(comp) and comp < current_threshold * 0.5:
                continue

            candidates.append((blend, si))
        ...
        new_entries = []
        for blend_val, si in candidates:
            ...
            if blend_val <= 0:
                continue
            new_entries.append((blend_val, si, sym_sector))
        ...
        for blend_val, si, sym_sector in new_entries:
            ep = O[si, di + 1]
...
def walk_forward(
    ...
    print(f"\n{'=' * 70}")
    print(f"  WALK-FORWARD V101 {label}")
    print(f"  {cfg_str}")
...
def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("  V101: ADAPTIVE BENCHMARK DEVIATION (KAMA + LSMA)")
    print("  Innovation 1: KAMA deviation - regime-aware price benchmark")
    print("  Innovation 2: LSMA deviation - regression-line fair value")
    print("  Innovation 3: BOP - intrabar buy/sell force")
    print("  Innovation 4: Blended signal (60% NW + 40% composite)")
    print("  On V96's NW+BMA+Vol framework")
    ...
    # === 1. Compute factors for each KAMA period ===
    kama_periods = [10, 15, 20]
    factor_cache: Dict[int, Dict[str, np.ndarray]] = {}
    composite_cache: Dict[int, np.ndarray] = {}

    for kp in kama_periods:
        raw_factors = compute_raw_factors(
            C, O, H, L, V, OI, NS, ND, kama_period=kp)
        factor_cache[kp] = raw_factors
        composite_cache[kp] = compute_composite_score(raw_factors, NS, ND)

    # === 2. Compute KER regime (shared) ===
    ker_regime = compute_ker(C, NS, ND)

    # === 3. Compute rolling IC and BMA for each kama_period ===
    bma_cache: Dict[Tuple[int, int, float], np.ndarray] = {}
    pred_cache: Dict[Tuple[int, int, float], np.ndarray] = {}

    for kp in kama_periods:
        raw_factors = factor_cache[kp]
        for icw in [40, 60]:
            ic_array = compute_rolling_ic(
                raw_factors, NS, ND, ic_window=icw)
            for ps in [5, 10]:
                bma_w = compute_bma_weights(ic_array, ND, prior_strength=ps)
                bma_cache[(kp, icw, ps)] = bma_w
                pred_cache[(kp, icw, ps)] = compute_nw_predicted_returns(
    ...
    print("  NO LEVERAGE. NW+BMA + Adaptive Benchmark + Vol-adaptive sizing.")
    print("=" * 70)
    ...
    for kp in kama_periods:
        composite = composite_cache[kp]
        for pred_key in [(kp, icw, ps) for icw in [40, 60] for ps in [5, 10]]:
            pred = pred_cache[pred_key]
            icw, ps = pred_key[1], pred_key[2]
            for top_n in [2, 3]:
                for mps in [2, 3]:
                    for vlb in [15, 20]:
                        for sr in [0.3, 0.5]:
    ...
    for label, best in [
        ...
        pred = pred_cache[(best["kp"], best["icw"], best["ps"])]
        composite = composite_cache[best["kp"]]
    ...
    # === 7. Final summary ===
    print("\n" + "=" * 70)
    print("  V101 FINAL SUMMARY")
    print("  Adaptive Benchmark Deviation (KAMA + LSMA + BOP)")
    print("  vs V96 baseline ann +73.1%")
    print("=" * 70)
    ...
    for label, best in [
        ...
        analyze(trades, eq, dd, f"V101-{label}")

    print(f"\n[V101] Done. {time.time() - t0:.1f}s")
