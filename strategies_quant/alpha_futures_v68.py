"""
V68: V61 + Adaptive Sector-Aware Position Sizing
==================================================
V61 is our ALL-TIME BEST: Sharpe 6.71, MDD 5.5%, ann +15.7%, 78 trades.

V68 adds sector-aware position sizing:
  When multiple sectors have signals, size positions based on
  sector-level aggregate signal strength instead of equal weight.

Three methods:
  - 'equal': baseline, equal allocation (same as V61)
  - 'rank_weighted': weight by average composite rank per sector
  - 'top_sector_only': allocate only to the strongest sector

Signal pipeline unchanged from V61 (multi-TF composite + dynamic mode).

Parameter sweep:
  - sector_weight_method: 'equal', 'rank_weighted', 'top_sector_only'
  - max_per_sector: 1, 2
  - win_rate_window: 10, 15, 20
  - Keep V61's other best params (st_w, thresholds, etc.)

Walk-forward 2019-2026. Full 10yr for best.
...
def compute_raw_factors(
    ...
    t0 = time.time()
    print("[V68] Computing raw factors (5d + 20d)...", flush=True)
...
def compute_cross_sectional_ranks(
    ...
    t0 = time.time()
    print("[V68] Computing cross-sectional ranks...", flush=True)
...
def build_multi_tf_composite(
    ...
    t0 = time.time()
    print(f"[V68] Building multi-TF composite (st_w={st_weight:.2f})...",
          flush=True)
...
def compute_sector_weights(
    ...
    """Compute per-sector allocation weights based on aggregate signal strength.

    Args:
        candidates: list of (composite_score, symbol_index, base_alloc)
        sector_lookup: symbol-index -> sector mapping
        method: 'equal', 'rank_weighted', or 'top_sector_only'
        total_slots: max concurrent positions

    Returns:
        Dict mapping sector -> weight fraction (0.0 to 1.0)
    """
    if method == 'equal':
        return {}
    ...
    if method == 'top_sector_only':
        # Only allocate to the strongest sector
...
def backtest_v68(
    ...
    """Backtest V68: V61 + sector-aware position sizing."""
    ...
    for di in range(max(start_di, 1), end_di):
        ...
        for si in range(NS):
            ...
            candidates.append((composite[si, di], si, base_alloc))

        # Sort by composite score (highest first)
        candidates.sort(key=lambda x: -x[0])

        # === V68 NEW: Sector-aware position sizing ===
        sector_alloc_weights = compute_sector_weights(
            candidates, sector_lookup, sector_weight_method, current_top_n)

        # Sector-constrained greedy selection with sector-aware sizing
        sector_counts: Dict[str, int] = defaultdict(int)
        for si_held in held:
            sector_counts[sector_lookup.get(si_held, 'OTHER')] += 1

        remaining_slots = current_top_n - len(positions)

        for rank_val, si, base_alloc in candidates:
            ...
            # Apply sector-aware allocation
            if sector_weight_method == 'equal' or not sector_alloc_weights:
                alloc = base_alloc
            else:
                sector_w = sector_alloc_weights.get(sym_sector, 0.0)
                # Scale sector weight by remaining slots
                # Each position in this sector gets sector_weight / n_positions_in_sector
                n_in_sector = sum(
                    1 for _, s, _ in candidates[:candidates.index(
                        (rank_val, si, base_alloc)) + 1]
                    if sector_lookup.get(s, 'OTHER') == sym_sector
                    and s not in held
                    and sector_counts.get(
                        sector_lookup.get(s, 'OTHER'), 0) < max_per_sector
                )
                alloc = sector_w / max(n_in_sector, 1)
                # Ensure minimum allocation
                alloc = max(alloc, 0.1 / max(current_top_n, 1))
...
def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("  V68: V61 + ADAPTIVE SECTOR-AWARE POSITION SIZING")
    print("  Sector-level signal aggregation -> information-weighted allocation")
    print("  Methods: equal (baseline), rank_weighted, top_sector_only")
    print("=" * 70)
    ...
    results: List[dict] = []
    SECTOR_METHODS = ['equal', 'rank_weighted', 'top_sector_only']
    ...
    for r in results[:30]:
        print(
            f"{r['swm']:>14} {r['st_w']:>4.2f} {r['wt']:>4.2f} "
    ...
    for r in unique_top:
        ...
        label = (
            f"swm={r['swm']} st_w={r['st_w']:.2f} "
            f"wt={r['wt']:.2f} nt={r['nt']:.2f} "
            f"lt={r['lt']:.2f} ww={r['ww']} "
            f"mps={r['mps']} tnw={r['tnw']}"
        )
        print(f"\n  FULL {label}")
    ...
    if results:
        ...
        # === 5. Head-to-head: best V68 vs V61 baseline (equal) ===
        print("\n" + "=" * 70)
        print("  COMPARISON: V68 best vs V61 baseline (equal weight)")
        print("  (2019-2026 OOS)")
        print("=" * 70)

        # V68 best
        trades_v68, eq_v68, dd_v68 = backtest_v68(
            C, O, H, L, NS, ND, dates, syms,
            signal_cache[best["st_w"]],
            sector_lookup=sector_lookup,
            st_weight=best["st_w"],
        ...
        # V61 baseline: same params but equal weight
        trades_v61, eq_v61, dd_v61 = backtest_v68(
            C, O, H, L, NS, ND, dates, syms,
            signal_cache[best["st_w"]],
            sector_lookup=sector_lookup,
            st_weight=best["st_w"],
        ...
        print(f"\n  V68 BEST ({best['swm']}):")
        analyze(trades_v68, eq_v68, dd_v68, "V68-best")
        print(f"\n  V61 BASELINE (equal):")
        analyze(trades_v61, eq_v61, dd_v61, "V61-equal")

        if trades_v68 and trades_v61:
            print(
                f"\n  V68 vs V61: "
                f"eq_delta={eq_v68 - eq_v61:+,.0f} "
                f"dd_delta={dd_v68 - dd_v61:+.1f}%"
            )

        # === 6. Show best per method ===
        print("\n" + "=" * 70)
        ...
        for method in SECTOR_METHODS:
            ...
            if method_results:
                ...
                print(
                    f"  {method:>14}: Sh={top['sharpe']:.2f} "
                    f"ann={top['ann']:+.1f}% DD={top['dd']:.1f}% "
                    f"WR={top['wr']:.1f}% N={top['n']} "
                    f"st_w={top['st_w']:.2f} mps={top['mps']} "
                    f"ww={top['ww']} tnw={top['tnw']}"
                )

    print(f"\n[V68] Done. {time.time() - t0:.1f}s")
