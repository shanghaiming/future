"""
V73: V61 + Intra-Sector Correlation Filter
===========================================
V61 is ALL-TIME BEST: Sharpe 6.71, MDD 5.5%, ann +15.7%, 78 trades.

V73 adds a correlation filter within sectors to avoid redundant positions:
  1. Keep V61's full signal pipeline (multi-TF composite, dynamic mode, sector limit)
  2. After computing composite ranks and filtering by threshold, group by sector
  3. For each sector with >1 candidate, compute pairwise return correlation
  4. Use greedy selection: pick highest rank first, then pick next that has
     lowest avg correlation to already-picked instruments in that sector
  5. This maximizes diversification benefit within the allowed sector count

Parameter sweep:
  - corr_lookback: 30, 60, 90
  - max_per_sector: 1, 2
  - win_rate_window: 10, 15, 20
  - Keep V61's best st_weight / threshold / lose_th ranges
...
def compute_raw_factors(
    ...
    t0 = time.time()
    print("[V73] Computing raw factors (5d + 20d)...", flush=True)
...
def compute_cross_sectional_ranks(
    ...
    t0 = time.time()
    print("[V73] Computing cross-sectional ranks...", flush=True)
...
def build_multi_tf_composite(
    ...
    t0 = time.time()
    print(f"[V73] Building multi-TF composite (st_w={st_weight:.2f})...",
          flush=True)
...
def precompute_daily_returns(
    ...
    """Precompute daily returns for all symbols. Fast: O(NS*ND)."""
    t0 = time.time()
    print("[V73] Precomputing daily returns...", flush=True)
    ...
    print(f"  Daily returns done: {time.time() - t0:.1f}s", flush=True)
    return daily_rets


def compute_pair_corr(
    daily_rets: np.ndarray, si1: int, si2: int,
    di: int, lookback: int,
) -> float:
    """Compute rolling correlation between si1 and si2 at day di.

    Returns NaN if insufficient data or lookback=0 (disabled).
    """
    if lookback <= 0:
        return np.nan
    r1 = daily_rets[si1, di - lookback:di]
    r2 = daily_rets[si2, di - lookback:di]
    valid = ~np.isnan(r1) & ~np.isnan(r2)
    n = np.sum(valid)
    min_overlap = max(lookback // 2, 10)
    if n < min_overlap:
        return np.nan
    v1 = r1[valid]
    v2 = r2[valid]
...
def backtest_v73(
    ...
    """Backtest V73: V61 + intra-sector correlation filter.

    Correlations are computed on-the-fly only for intra-sector candidate
    pairs, avoiding the O(NS^2 * ND) precompute cost.
    """
    ...
    for di in range(max(start_di, 1), end_di):
        ...
        # --- V73 NEW: Intra-sector correlation-aware selection ---
        sector_counts: Dict[str, int] = defaultdict(int)
        for si_held in held:
            sector_counts[sector_lookup.get(si_held, 'OTHER')] += 1

        # Group candidates by sector
        sector_candidates: Dict[str, List[Tuple[float, int, float]]] = (
            defaultdict(list))
        for rank_val, si, alloc in candidates:
            sym_sector = sector_lookup.get(si, 'OTHER')
            sector_candidates[sym_sector].append((rank_val, si, alloc))

        # For sectors with >1 candidate and max_per_sector > 1,
        # use correlation-aware greedy: pick best rank, then pick next
        # that has lowest avg corr to already-picked in that sector.
        # Correlations computed on-the-fly from daily_rets.
        selected: List[Tuple[float, int, float]] = []

        for sym_sector, cands in sector_candidates.items():
            cands_sorted = sorted(cands, key=lambda x: -x[0])
            remaining_quota = max_per_sector - sector_counts.get(
                sym_sector, 0)
            if remaining_quota <= 0:
                continue

            if len(cands_sorted) == 1 or remaining_quota == 1:
                for rank_val, si, alloc in cands_sorted[:remaining_quota]:
                    selected.append((rank_val, si, alloc))
            else:
                # Correlation-aware greedy selection
                picked_in_sector: List[int] = []
                best_cand = cands_sorted[0]
                picked_in_sector.append(best_cand[1])
                selected.append(best_cand)
                remaining_quota -= 1

                unpicked = list(cands_sorted[1:])
                while unpicked and remaining_quota > 0:
                    best_idx = -1
                    best_score = -1.0
                    best_avg_corr = 999.0

                    for idx, (rank_val, si_cand, alloc_cand) in enumerate(
                            unpicked):
                        avg_corr = 0.0
                        corr_count = 0
                        for picked_si in picked_in_sector:
                            c_val = compute_pair_corr(
                                daily_rets, picked_si, si_cand,
                                di, corr_lookback)
                        ...
                        if (avg_corr < best_avg_corr
                                or (avg_corr == best_avg_corr
                                    and rank_val > best_score)):
                            best_avg_corr = avg_corr
                            best_score = rank_val
                            best_idx = idx

                    if best_idx >= 0:
                        picked_cand = unpicked.pop(best_idx)
                        picked_in_sector.append(picked_cand[1])
                        selected.append(picked_cand)
                        remaining_quota -= 1
                    else:
                        break

        # Fill positions from selected candidates, respecting total limit
        for rank_val, si, alloc in selected:
...
def walk_forward(
    ...
    print(
        f"  WALK-FORWARD V73 "
        f"(clb={corr_lookback} st_w={st_weight:.2f} "
        f"wt={win_threshold:.2f} "
...
def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("  V73: V61 + INTRA-SECTOR CORRELATION FILTER")
    print("  Adds correlation-aware greedy selection within sectors")
    print("  V61 baseline: Sharpe 6.71, MDD 5.5%, ann +15.7%, 78 trades")
    print("=" * 70)
    ...
    # === 2. Pre-compute daily returns (fast, O(NS*ND)) ===
    daily_rets = precompute_daily_returns(C, NS, ND)
    ...
    print(
        f"\n{'CLB':>4} {'STw':>4} {'WT':>4} {'NT':>4} {'LT':>4} {'WW':>3} "
        f"{'MPS':>3} {'TNW':>4} "
    ...
    for r in results[:30]:
        print(
            f"{r['corr_lb']:>4} {r['st_w']:>4.2f} {r['wt']:>4.2f} "
            f"{r['nt']:>4.2f} "
    ...
    unique_top = []
    for r in results:
        key = (r["corr_lb"], r["st_w"], r["wt"], r["nt"], r["lt"],
               r["ww"], r["mps"], r["tnw"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for r in unique_top:
        ...
        label = (
            f"clb={r['corr_lb']} st_w={r['st_w']:.2f} "
    ...
    if results:
        ...
        # === 6. Comparison: V73 (corr filter ON) vs V61 (corr OFF) ===
        print("\n" + "=" * 70)
        print("  COMPARISON: V73 (corr filter) vs V61 baseline")
        print("  (2019-2026 OOS)")
        print("=" * 70)

        # V73 (best config with correlation filter)
        trades_v73, eq_v73, dd_v73 = backtest_v73(
            C, O, H, L, NS, ND, dates, syms,
            signal_cache[best["st_w"]],
            sector_lookup=sector_lookup,
            daily_rets=daily_rets,
        ...
        # V61 baseline: same params but corr_lookback=0 (no filter)
        # When max_per_sector=1, correlation filter has no effect
        # because we never pick >1 from same sector. So only mps=2
        # configs are meaningful to compare.
        trades_v61, eq_v61, dd_v61 = backtest_v73(
            C, O, H, L, NS, ND, dates, syms,
            signal_cache[best["st_w"]],
            sector_lookup=sector_lookup,
            daily_rets=daily_rets,
            corr_lookback=0,  # disables corr filter
            st_weight=best["st_w"],
            win_threshold=best["wt"],
            normal_threshold=best["nt"],
            lose_threshold=best["lt"],
        ...
        print(f"\n  V73 (corr_lb={best['corr_lb']}, corr filter ON):")
        analyze(trades_v73, eq_v73, dd_v73, "V73-corr-ON")
        print(f"\n  V61-like (corr filter OFF, corr_lb=0):")
        analyze(trades_v61, eq_v61, dd_v61, "V61-corr-OFF")

        if trades_v73 and trades_v61:
            print(
                f"\n  V73 vs V61-like: "
                f"eq_delta={eq_v73 - eq_v61:+,.0f} "
                f"dd_delta={dd_v73 - dd_v61:+.1f}%"
            )

    print(f"\n[V73] Done. {time.time() - t0:.1f}s")
