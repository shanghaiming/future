"""
V70: V61 + Open Interest Extreme Confirmation
==============================================
V61 is ALL-TIME BEST: Sharpe 6.71, MDD 5.5%, ann +15.7%, 78 trades.
V12 showed OI contrarian works well (Sharpe 1.71).

When OI drops sharply while price drops, it signals capitulation -- ideal for MR.

Logic:
  - Compute oi_5d change (already in V61's factors)
  - When oi_5d < oi_drop_threshold (e.g., OI dropping >10% in 5d)
    AND composite > threshold: BOOST the signal rank
  - boosted_rank = composite * (1 + oi_boost)
  - oi_boost scales with how extreme the OI drop is:
      linear_scale = min(1.0, abs(oi_5d) / 0.20)  -- 0 at threshold, 1 at 20% drop
      oi_boost_effective = oi_boost_amount * linear_scale
  - This does NOT filter out non-OI trades, just boosts the ones with OI confirmation
  - apply_only_in_losing: if True, only boost in losing mode (conservative)

Parameter sweep:
  - oi_boost_amount: 0.05, 0.10, 0.15 (rank boost when OI confirms)
  - oi_drop_threshold: -0.05, -0.10, -0.15 (minimum OI drop to trigger boost)
  - apply_only_in_losing: True/False (only boost in losing mode)
  - max_per_sector: 1, 2
...
def compute_raw_factors(
    ...
    t0 = time.time()
    print("[V70] Computing raw factors (5d + 20d)...", flush=True)
...
def compute_cross_sectional_ranks(
    ...
    t0 = time.time()
    print("[V70] Computing cross-sectional ranks...", flush=True)
...
def build_multi_tf_composite(
    ...
    t0 = time.time()
    print(f"[V70] Building multi-TF composite (st_w={st_weight:.2f})...",
          flush=True)
...
def compute_oi_boost(
    raw_oi_5d: np.ndarray,
    composite: np.ndarray,
    oi_drop_threshold: float,
    oi_boost_amount: float,
    NS: int, ND: int,
) -> np.ndarray:
    """Compute OI-boosted composite signal.

    When oi_5d drops below oi_drop_threshold (e.g., -0.10 means OI dropped >10%),
    the composite score is boosted proportionally to how extreme the drop is.

    boost_scale = min(1.0, abs(oi_5d - oi_drop_threshold) / (0.20 - abs(oi_drop_threshold)))
    effective_boost = oi_boost_amount * max(0, boost_scale)
    boosted_composite = composite * (1 + effective_boost)

    This ensures:
    - No boost when OI drop is less extreme than threshold
    - Linear scaling from threshold to 20% drop (full boost)
    - Non-OI trades are not filtered out, just not boosted
    """
    boosted = composite.copy()
    abs_threshold = abs(oi_drop_threshold)
    scale_range = max(0.20 - abs_threshold, 0.05)  # avoid div by zero

    for si in range(NS):
        for di in range(ND):
            if np.isnan(composite[si, di]) or np.isnan(raw_oi_5d[si, di]):
                continue
            oi_val = raw_oi_5d[si, di]
            if oi_val < oi_drop_threshold:
                excess = abs(oi_val) - abs_threshold
                scale = min(1.0, excess / scale_range)
                effective_boost = oi_boost_amount * scale
                boosted[si, di] = composite[si, di] * (1.0 + effective_boost)
...
def compute_base_signals(
    ...
    """Compute base signals (raw factors, ranks, KER, multi-TF composite).
    These do NOT depend on OI boost params, so they can be cached.
    """
...
def apply_oi_boost_to_signals(
    base_sigs: Dict[str, np.ndarray],
    oi_drop_threshold: float,
    oi_boost_amount: float,
    NS: int, ND: int,
) -> Dict[str, np.ndarray]:
    """Apply OI extreme boost to pre-computed base signals.
    Returns a new dict with 'composite' (boosted) and 'composite_raw' (original).
    """
    boosted_composite = compute_oi_boost(
        base_sigs["oi_5d"], base_sigs["composite_raw"],
        oi_drop_threshold, oi_boost_amount, NS, ND)

    result = dict(base_sigs)
    result["composite"] = boosted_composite
    return result
...
def backtest_v70(
    ...
    apply_oi_boost_in_losing_only: bool = False,
) -> Tuple[List[dict], float, float]:
    """Backtest V70: multi-TF composite + OI extreme boost + dynamic threshold.

    When apply_oi_boost_in_losing_only is True, the OI-boosted composite
    is used ONLY during losing mode to prefer capitulation trades when
    the strategy is struggling. In winning/normal mode, use raw composite.
    """
    composite_boosted = sigs["composite"]
    composite_raw = sigs["composite_raw"]
    ...
    for di in range(max(start_di, 1), end_di):
        ...
        # Select which composite to use based on mode
        if apply_oi_boost_in_losing_only and mode != "losing":
            composite = composite_raw
        else:
            composite = composite_boosted
...
def walk_forward(
    ...
    print(
        ...
        f"oi_losing_only={apply_oi_boost_in_losing_only})"
    )
    print(f"{'=' * 70}")
    ...
    for test_year in range(2019, years[-1] + 1):
        ...
        trades, _, _ = backtest_v70(
            ...
            apply_oi_boost_in_losing_only=apply_oi_boost_in_losing_only,
        )
...
def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("  V70: V61 + OPEN INTEREST EXTREME CONFIRMATION")
    print("  V61 base (Sharpe 6.71, MDD 5.5%) + OI capitulation boost")
    print("  When OI drops sharply, boost MR signal for capitulation plays")
    print("=" * 70)
    ...
    # === 1. Pre-compute base signals (raw factors + ranks + composite) ===
    # These depend only on st_weight, NOT on OI boost params.
    # Only 3 st_weight values needed, then OI boost is applied cheaply.
    oi_configs = [
        (oi_boost, oi_thresh)
        for oi_boost in [0.05, 0.10, 0.15]
        for oi_thresh in [-0.05, -0.10, -0.15]
    ]

    base_signal_cache: Dict[float, Dict] = {}
    for st_w in [0.55, 0.60, 0.65]:
        print(f"\n--- Computing base signals for st_weight={st_w:.2f} ---")
        base_signal_cache[st_w] = compute_base_signals(
            C, O, H, L, V, OI, NS, ND, st_weight=st_w)

    # Apply OI boost on top (very fast -- no recomputation)
    signal_cache: Dict[Tuple[float, float, float], Dict] = {}
    for st_w in [0.55, 0.60, 0.65]:
        for oi_boost, oi_thresh in oi_configs:
            key = (st_w, oi_boost, oi_thresh)
            signal_cache[key] = apply_oi_boost_to_signals(
                base_signal_cache[st_w],
                oi_thresh, oi_boost, NS, ND)

    # V61 baseline = no OI boost (boost amount = 0)
    signal_cache["V61_baseline"] = apply_oi_boost_to_signals(
        base_signal_cache[0.60], -1.0, 0.0, NS, ND)
    ...
    for st_w in [0.55, 0.60, 0.65]:
        for oi_boost in [0.05, 0.10, 0.15]:
            for oi_thresh in [-0.05, -0.10, -0.15]:
                sigs = signal_cache[(st_w, oi_boost, oi_thresh)]
                for mps in [1, 2]:
                    for losing_only in [False, True]:
                        for ww in [10, 15, 20]:
    ...
    for r in results[:40]:
        print(
            f"{r['st_w']:>4.2f} {r['oi_boost']:>5.2f} "
            f"{r['oi_thresh']:>6.2f} "
            f"{'Y' if r['losing_only'] else 'N':>2} "
    ...
    unique_top = []
    for r in results:
        key = (r["st_w"], r["oi_boost"], r["oi_thresh"],
               r["losing_only"], r["nt"], r["lt"],
               r["ww"], r["mps"], r["tnw"])
        if key not in seen:
            seen.add(key)
            unique_top.append(r)
    ...
    for r in unique_top:
        sigs = signal_cache[(r["st_w"], r["oi_boost"], r["oi_thresh"])]
        trades, eq, dd = backtest_v70(
            C, O, H, L, NS, ND, dates, syms, sigs,
            sector_lookup=sector_lookup,
            st_weight=r["st_w"],
            ...
            start_di=60,
            apply_oi_boost_in_losing_only=r["losing_only"],
        )
        label = (
            f"st_w={r['st_w']:.2f} oi_b={r['oi_boost']:.2f} "
            f"oi_t={r['oi_thresh']:.2f} "
            f"lo={'Y' if r['losing_only'] else 'N'} "
    ...
    if results:
        ...
        print(
            ...
            f"oi_b={best['oi_boost']:.2f} "
            f"oi_t={best['oi_thresh']:.2f} "
            f"lo={'Y' if best['losing_only'] else 'N'} "
        ...
        walk_forward(
            ...
            signal_cache[(best["st_w"], best["oi_boost"],
                          best["oi_thresh"])],
            sector_lookup=sector_lookup,
            st_weight=best["st_w"],
            ...
            apply_oi_boost_in_losing_only=best["losing_only"],
        )

        # === 5. Comparison: V70 (OI boost) vs V61 baseline ===
        print("\n" + "=" * 70)
        print("  COMPARISON: V70 (OI boost) vs V61 BASELINE (no OI boost)")
        print("  (2019-2026 OOS)")
        print("=" * 70)

        # V70 with OI boost
        trades_v70, eq_v70, dd_v70 = backtest_v70(
            C, O, H, L, NS, ND, dates, syms,
            signal_cache[(best["st_w"], best["oi_boost"],
                          best["oi_thresh"])],
            sector_lookup=sector_lookup,
        ...
        # V61 baseline (no OI boost)
        sigs_baseline = signal_cache["V61_baseline"]
        trades_v61, eq_v61, dd_v61 = backtest_v70(
            C, O, H, L, NS, ND, dates, syms, sigs_baseline,
            sector_lookup=sector_lookup,
            st_weight=0.60,
            normal_threshold=best["nt"],
            lose_threshold=best["lt"],
            ...
            max_per_sector=best["mps"],
            start_di=bt_2019,
            apply_oi_boost_in_losing_only=False,
        )

        print(f"\n  V70 (OI EXTREME BOOST):")
        analyze(trades_v70, eq_v70, dd_v70, "V70-OI-boost")
        print(f"\n  V61 BASELINE (NO OI BOOST):")
        analyze(trades_v61, eq_v61, dd_v61, "V61-baseline")

        if trades_v70 and trades_v61:
            print(
                f"\n  V70 vs V61: "
                f"eq_delta={eq_v70 - eq_v61:+,.0f} "
                f"dd_delta={dd_v70 - dd_v61:+.1f}%"
            )

    print(f"\n[V70] Done. {time.time() - t0:.1f}s")
