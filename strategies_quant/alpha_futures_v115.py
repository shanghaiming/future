"""
V115: "Ultimate Synthesis" - Combined Best Innovations Strategy
================================================================
Combines ONLY proven innovations from meta-analysis of 101 strategies:

Layer 1: NW Kernel Regression (V86, +16.5% ann over linear)
Layer 2: KER Regime Gate + Dynamic Mode (V39/V43, +5-10% ann)
Layer 3: Sector Diversification (V47, -50% MDD)
Layer 4: Sigmoid Vol-Adaptive Sizing (V96 + smooth transition)
Layer 5: Rolling Strategy Sharpe Gate (lightweight V111)

NOT included (proven to NOT work):
- NO BMA (IC too unstable)
- NO circuit breakers (destroy returns)
- NO signal-strength sizing
- NO dynamic hold period
- NO additional factors beyond 7
...
def compute_raw_factors(
    ...
    t0 = time.time()
    print("[V115] Computing raw factors...", flush=True)
...
def normalize_factor(factor: np.ndarray, NS: int, ND: int,
    ...
    """Cross-sectional percentile ranking (proven superior to z-score)."""
    ...
    for di in range(ND):
        ...
        for si in range(NS):
            if not np.isnan(vals[si]):
                ranked[si, di] = ranks[idx] * 2.0 - 1.0  # scale to [-1, 1]
                idx += 1
    return ranked


# =====================================================================
# Layer 1: NW Kernel Regression (from V86)
# =====================================================================
...
def compute_nw_predicted_returns(
    ...
    """NW kernel regression with percentile-ranked features."""
    t0 = time.time()
    print(
        f"[V115] Computing NW predicted returns "
        f"(window={training_window}, bw={kernel_bandwidth:.1f})...",
        flush=True)

    ranked = {}
    for fname in FACTOR_NAMES:
        ranked[fname] = normalize_factor(raw_factors[fname], NS, ND)
    ...
    for di in range(training_window + 10, ND):
        ...
        for tdi in range(start_di, di):
            for si in range(NS):
                feat = np.array([
                    ranked[fname][si, tdi] for fname in FACTOR_NAMES
                ])
        ...
        for si in range(NS):
            query_feat = np.array([
                ranked[fname][si, di] for fname in FACTOR_NAMES
            ])
...
# =====================================================================
# Layer 2: KER Regime Detection + Dynamic Mode
# =====================================================================
...
def compute_ker_regime(
    ker_10: np.ndarray, NS: int, ND: int,
    skip_threshold: float = 0.30,
    prioritize_threshold: float = 0.15,
) -> np.ndarray:
    """KER regime: -1=skip (strong trend), 0=normal, 1=prioritize (MR)."""
    ...
    for si in range(NS):
        for di in range(ND):
            ...
            if np.isnan(val):
                continue
            if val < prioritize_threshold:
                ker_regime[si, di] = 1
            elif val > skip_threshold:
                ker_regime[si, di] = -1
    return ker_regime
...
# =====================================================================
# Layer 4: Sigmoid Vol-Adaptive Position Sizing
# =====================================================================
...
def compute_portfolio_volatility(
    ...
    """Rolling portfolio volatility proxy (ND array)."""
...
def sigmoid_vol_multiplier(
    vol_ratio: float,
    center: float = 1.5,
    steepness: float = 5.0,
) -> float:
    """Smooth sigmoid transition for vol-adaptive sizing.

    Returns multiplier in [0.4, 1.2] range.
    At vol_ratio = center: mult ~ 0.8 (midpoint).
    Low vol: mult approaches 1.2. High vol: mult approaches 0.4.
    """
    if np.isnan(vol_ratio):
        return 1.0
    return 0.4 + 0.8 / (1.0 + np.exp(steepness * (vol_ratio - center)))


# =====================================================================
# Layer 5: Rolling Strategy Sharpe Gate
# =====================================================================

def strategy_sharpe_gate(
    recent_pnl: List[float],
    window: int = 60,
) -> float:
    """Reduce position size if rolling strategy Sharpe is negative.

    Returns multiplier: 0.5 (bad), 1.0 (normal), 1.2 (excellent).
    """
    if len(recent_pnl) < window:
        return 1.0
    recent = recent_pnl[-window:]
    arr = np.array(recent)
    std = np.std(arr)
    if std < 1e-12:
        return 1.0
...
def backtest_v115(
    ...
    # Sharpe gate params
    sharpe_gate_window: int = 60,
    # Run params
    start_di: int = 60,
    end_di: Optional[int] = None,
) -> Tuple[List[dict], float, float]:
    """Backtest V115 with all 5 innovation layers."""
    if end_di is None:
        end_di = ND - 1

    # Compute median vol for sigmoid input
    ...
    for di in range(max(start_di, 1), end_di):
        ...
        # Layer 4: Sigmoid vol multiplier
        vol_ratio = (
            port_vol[di] / vol_median
            if not np.isnan(port_vol[di]) and vol_median > 1e-12
            else 1.0)
        vol_mult = sigmoid_vol_multiplier(
            vol_ratio, sigmoid_center, sigmoid_steepness)

        # Layer 5: Strategy Sharpe gate
        sharpe_mult = strategy_sharpe_gate(recent_pnl, sharpe_gate_window)

        # Combined sizing multiplier
        combined_mult = vol_mult * sharpe_mult
        ...
        for si in range(NS):
            ...
            if di + 1 >= ND or np.isnan(O[si, di + 1]):
                continue
            # Layer 2: KER regime gate - skip strong trends
            if ker_regime[si, di] < 0:
                continue
            candidates.append((pred, si, ker_regime[si, di]))

        if not candidates:
            continue

        # Sort: prioritize KER=1 (consolidation) first, then by predicted
        candidates.sort(key=lambda x: (-x[2], -x[0]))

        # Dynamic mode adjusts number of positions
        n_to_take = top_n
        if mode == "winning":
        ...
        # Layer 3: Sector-constrained selection
        sector_counts: Dict[str, int] = defaultdict(int)
        for si_held in held:
            sector_counts[sector_lookup.get(si_held, 'OTHER')] += 1

        new_entries = []
        for pred_val, si, ker_val in candidates:
...
def walk_forward(
    ...
    sigmoid_center: float = 1.5,
    sigmoid_steepness: float = 5.0,
    ker_skip_threshold: float = 0.30,
    win_threshold: float = 0.60,
    sharpe_gate_window: int = 60,
    label: str = "",
) -> List[dict]:
    cfg_str = (
        f"tn={top_n} mps={max_per_sector} sc={sigmoid_center:.1f} "
        f"ss={sigmoid_steepness:.1f} ker={ker_skip_threshold:.2f} "
        f"wt={win_threshold:.2f} sgw={sharpe_gate_window}")
    print(f"\n{'=' * 70}")
    print(f"  WALK-FORWARD V115 {label}")
    print(f"  {cfg_str}")
    print(f"  LEVERAGE=1.0 CASH0={CASH0:,} COMM={COMM}")
    print(f"{'=' * 70}")
    ...
    for test_year in range(2019, years[-1] + 1):
        ...
        trades, _, _ = backtest_v115(
            ...
            sharpe_gate_window=sharpe_gate_window,
            start_di=test_start,
...
def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("  V115: ULTIMATE SYNTHESIS - Combined Best Innovations")
    print("  L1: NW Kernel (V86 proven)")
    print("  L2: KER Regime + Dynamic Mode (V39/V43 proven)")
    print("  L3: Sector Diversification (V47 proven)")
    print("  L4: Sigmoid Vol-Adaptive Sizing (smooth transition)")
    print("  L5: Rolling Strategy Sharpe Gate (lightweight)")
    ...
    # === Step 2: Compute NW predicted returns ===
    ...
    # === Step 3: Compute KER (raw values + regime arrays) ===
    ker_10 = compute_ker(C, NS, ND)
    ...
    for sigmoid_center in [1.3, 1.5, 1.7]:
    ...
    for r in results[:15]:
        print(
            f"{r['sc']:>4.1f} {r['ss']:>4.1f} {r['top_n']:>3} "
            f"{r['mps']:>3} {r['ker']:>4.2f} {r['wt']:>4.2f} "
            f"{r['sgw']:>4} "
    ...
    for label, best in [
        ...
        walk_forward(
            ...
            hold_days=5,
            sigmoid_center=best["sc"],
            sigmoid_steepness=best["ss"],
            ker_skip_threshold=best["ker"],
            win_threshold=best["wt"],
            sharpe_gate_window=best["sgw"],
            label=label,
        )
    ...
    trades_v115, eq_v115, dd_v115 = backtest_v115(
        ...
        hold_days=5,
        sigmoid_center=best_by_ann["sc"],
        sigmoid_steepness=best_by_ann["ss"],
        win_threshold=best_by_ann["wt"],
        sharpe_gate_window=best_by_ann["sgw"],
        start_di=bt_2019,
    )

    print(f"\n  V115 BEST-ANN (All 5 Layers):")
    analyze(trades_v115, eq_v115, dd_v115, "V115-Ultimate")
    print(f"\n  Reference: V96 = +73.1% ann, V80 = +36.4% ann")
    print(f"  V115 equity: {eq_v115:,.0f} "
          f"({eq_v115 / CASH0 * 100 - 100:+.1f}% total return)")

    print(f"\n[V115] Done. {time.time() - t0:.1f}s")
