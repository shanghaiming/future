"""
V92 — Cascade Selection (fallback when top-1 filtered)
======================================================
Same 3-LGB ensemble primary as V89. Key change in backtest:
When top-1 stock is filtered by meta (score < threshold), cascade to
top-2, top-3, ... top-K until one passes meta. Tests K=[1,2,3,5].
Goal: increase TPY from 27→34 while maintaining Edge≈7.5%.
"""
...
print("=" * 70, flush=True)
print("  V92 — Cascade Selection (fallback when top-1 filtered)", flush=True)
print("=" * 70, flush=True)
...
# ====================================================================
# [5] ENSEMBLE PRIMARY MODEL (3 configs, rank-normalized average)
# ====================================================================
TRAIN_WINDOW = 252 * 3; RETRAIN_FREQ = 42; MIN_TRAIN = 252 * 2; FWD_DAYS = 5

ENSEMBLE_CONFIGS = [
    {'num_leaves': 7,  'feature_fraction': 0.4, 'learning_rate': 0.05,
     'bagging_fraction': 0.7, 'min_data_in_leaf': 100},   # Config A: current
    {'num_leaves': 15, 'feature_fraction': 0.6, 'learning_rate': 0.03,
     'bagging_fraction': 0.8, 'min_data_in_leaf': 150},   # Config B: complex
    {'num_leaves': 5,  'feature_fraction': 0.3, 'learning_rate': 0.07,
     'bagging_fraction': 0.6, 'min_data_in_leaf': 80},    # Config C: simple
]
...
print(f"\n[5] Training ENSEMBLE primary ({N_ENS} LGB configs)...", flush=True)
t5 = time.time()
...
for train_di in range(MIN_TRAIN, ND, RETRAIN_FREQ):
    ...
    tX, rank_y, train_group = make_train_data(train_start, train_end)
...
print(f"    Done: {len(retrain_points)} trainings × {N_ENS} configs ({time.time()-t5:.1f}s)", flush=True)

# ====================================================================
# [6] Leak-free walk-forward meta model (same as V88)
# ====================================================================
...
def run_backtest(score_arr, meta_arr, meta_threshold, sl_pct, tp_pct,
    ...
    for di in range(MIN_TRAIN, ND):
        ...
        if pos is None and pending is None:
            # Sort stocks by primary score, try top-K until one passes meta
            scored = []
            for si in range(NS):
                s = score_arr[si, di]
            ...
            for rank, (s, si) in enumerate(scored[:cascade_k]):
                n_total += 1
                if meta_threshold > 0 and meta_arr is not None:
                    meta_prob = meta_arr[si, di]
                    # Tiered threshold: each rank needs higher meta confidence
                    tier_th = meta_threshold + rank * tier_step
                    if np.isnan(meta_prob) or meta_prob < tier_th:
                        n_filtered += 1
                        continue  # try next stock
                pending = ('open_long', si)
                break  # found valid stock
...
# No meta baseline (cascade_k=1 only)
for hm in [6, 8]:
    for sl in [4, 5, 6]:
        for ts in [4, 5]:
            ...
            if r:
                ...
                all_results.append({**r, 'hm': hm, 'sl': sl, 'ts': ts,
                                   'meta_th': 0, 'cascade_k': 1, 'mode': 'atr',
                                   'label': 'no_meta_atr'})

# Cascade search: meta + ATR, varying cascade_k, threshold, and tier_step
for ck in [2, 3, 5]:
    for th in [0.50, 0.55, 0.60, 0.65, 0.70]:
        for ts_step in [0.03, 0.05, 0.08, 0.10]:
            for hm in [6, 7, 8]:
                for sl in [4, 5, 6]:
                    for ts in [4, 5]:
                        r = run_backtest(SCORE_PRI, SCORE_META, th, sl, 50, hm, 1, ts,
                                        atr_adaptive=True, atr_scale_min=0.6, atr_scale_range=0.8,
                                        cascade_k=ck, tier_step=ts_step)
                        if r:
                            t = r.pop('trades')
                            all_results.append({**r, 'hm': hm, 'sl': sl, 'ts': ts,
                                               'meta_th': th, 'cascade_k': ck,
                                               'tier_step': ts_step, 'mode': 'atr',
                                               'label': f'k{ck}_t{th:.2f}_s{ts_step:.2f}_atr'})

# Also test k=1 (original) for baseline
    ...
    for hm in [6, 7, 8]:
        for sl in [4, 5, 6]:
            for ts in [4, 5]:
                r = run_backtest(SCORE_PRI, SCORE_META, th, sl, 50, hm, 1, ts,
                                atr_adaptive=True, atr_scale_min=0.6, atr_scale_range=0.8,
                                cascade_k=1, tier_step=0)
                if r:
                    t = r.pop('trades')
                    all_results.append({**r, 'hm': hm, 'sl': sl, 'ts': ts,
                                       'meta_th': th, 'cascade_k': 1,
                                       'tier_step': 0, 'mode': 'atr',
                                       'label': f'k1_t{th:.2f}_s0_atr'})
...
print(f"  {'Label':>22s} {'CK':>3s} {'HM':>3s} {'SL':>3s} {'TS':>3s} | "
      f"{'Ann':>7s} {'N':>4s} {'WR':>5s} {'W':>6s} "
      f"{'L':>6s} {'Edge':>6s} {'TPY':>5s} {'DD':>5s}", flush=True)
print(f"  {'-'*115}", flush=True)
for r in all_results[:30]:
    ck = r.get('cascade_k', 1)
    print(f"  {r['label']:>22s} {ck:3d} {r['hm']:3d} {r['sl']:3d} {r['ts']:3d} | "
...
# Best per cascade_k
print(f"\n  Best per cascade_k:", flush=True)
for ck in [1, 2, 3, 5]:
    sub = [r for r in all_results if r.get('cascade_k', 1) == ck]
    if sub:
        b = sub[0]
        print(f"    k={ck}: {b['ann']:+.1f}% WR={b['wr']:.0f}% "
              f"Edge={b['edge']:+.2f}% TPY={b['tpy']:.0f} DD={b['max_dd']:.0f}% "
              f"filt={b['n_filtered']}/{b['n_total']}", flush=True)

# Best per threshold
print(f"\n  Best per threshold:", flush=True)
for th in [0.55, 0.60, 0.65, 0.70, 0.75, 0.80]:
    ...
    if sub:
        ...
        print(f"    th={th:.2f}: {b['ann']:+.1f}% WR={b['wr']:.0f}% "
              f"Edge={b['edge']:+.2f}% TPY={b['tpy']:.0f} DD={b['max_dd']:.0f}% "
              f"k={b.get('cascade_k',1)} filt={b['n_filtered']}/{b['n_total']}", flush=True)
...
r_full = run_backtest(SCORE_PRI, SCORE_META if best['meta_th'] > 0 else None,
                      ...
                      trail_pct=1, trail_start=best['ts'],
                      cascade_k=best.get('cascade_k', 1),
                      tier_step=best.get('tier_step', 0), **atr_kw)
...
print(f"\n{'='*70}", flush=True)
ck = best.get('cascade_k', 1)
ts = best.get('tier_step', 0)
print(f"  FINAL: {best['label']} -> {best['ann']:+.1f}% | WR={best['wr']:.0f}% "
      f"Edge={best['edge']:+.2f}% TPY={best['tpy']:.0f} DD={best['max_dd']:.0f}%", flush=True)
print(f"  Ensemble primary + walk-forward meta + cascade_k={ck} tier_step={ts}", flush=True)
