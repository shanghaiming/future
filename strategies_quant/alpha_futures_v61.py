"""
V61: V47 Sector Limit + Multi-Timeframe Confirmation
=====================================================
V47 is our ALL-TIME BEST: Sharpe 6.65, MDD 8.8%, ann +19.8%, 112 trades.
V27 multi-TF rank also showed strong results (Sharpe 2.99, MDD 18.6%).

V61 combines V47 sector limit with V27 multi-timeframe rank:
  1. Compute short-term (5d) composite AND medium-term (20d) composite
  2. Combined = st_weight * ST + (1-st_weight) * MT
  3. Apply sector concentration limit on the combined signal
  4. Dynamic mode switching (from V43/V47)

Short-term factors (5d lookback):
  - ret_5d, oi_5d, vol_5d, rsi14 (same), range_5d, atrp_5d, ret_10d

Medium-term factors (20d lookback):
  - ret_20d, oi_20d, vol_20d, rsi14 (shared), range_20d, atrp_20d, ret_10d (shared)

Parameter sweep:
  - st_weight: 0.50, 0.55, 0.60, 0.65, 0.70
  - max_per_sector: 1, 2
  - win_rate_window: 10, 15, 20
  - Dynamic mode switching from V43/V47
...
# Short-term (5d) factor weights -- same as V47
ST_WEIGHTS = {
    "rank_ret5d": 0.25,
    "rank_oi5d": 0.20,
    "rank_rsi": 0.15,
...
def _extract_base_symbol(sym: str) -> str:
    ...
    while s and s[-1].isdigit():
        s = s[:-1]
    # Strip common suffixes: 'fi' (financial futures), 'm'
    if s.endswith('fi'):
        s = s[:-2]
    # 'im' is a valid base symbol (iron ore mid-stream), keep it
    # Single letter + nothing is valid too
    return s
...
def compute_raw_factors(
    ...
    t0 = time.time()
    print("[V61] Computing raw factors (5d + 20d)...", flush=True)
    ...
    # range_5d: average daily range over past 5 days
    ...
    # range_20d: average daily range over past 20 days
    ...
    # atrp_20d: ATR% over past 20 days
...
def compute_cross_sectional_ranks(
    ...
    t0 = time.time()
    print("[V61] Computing cross-sectional ranks...", flush=True)
...
def build_multi_tf_composite(
    ...
    Returns (combined, st_composite, mt_composite, n_confirm_st, n_confirm_mt).
    """
    t0 = time.time()
    print(f"[V61] Building multi-TF composite (st_w={st_weight:.2f})...",
          flush=True)
...
def backtest_v61(
    ...
    """Backtest V61: multi-TF composite + dynamic threshold + sector limits."""
...
def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("  V61: V47 SECTOR LIMIT + MULTI-TIMEFRAME CONFIRMATION")
    print("  Combines V47 sector limit + dynamic mode with V27 multi-TF rank")
    print("  Short-term (5d) + Medium-term (20d) cross-sectional composite")
    print("=" * 70)
    ...
    if results:
        ...
        # === 5. Comparison: V61 vs V47-like (st_weight=1.0, pure ST) ===
        print("\n" + "=" * 70)
        print("  COMPARISON: V61 (multi-TF) vs V47-like (single-TF)")
        print("  (2019-2026 OOS)")
        print("=" * 70)

        # V61 multi-TF (best config)
        trades_v61, eq_v61, dd_v61 = backtest_v61(
            C, O, H, L, NS, ND, dates, syms,
            signal_cache[best["st_w"]],
            sector_lookup=sector_lookup,
            st_weight=best["st_w"],
        ...
        # V47-like: single-TF (use V47's own signal, no MT component)
        # Run V61 with st_weight=1.0 (pure short-term = V47-like)
        sigs_v47like = compute_all_signals(
            C, O, H, L, V, OI, NS, ND, st_weight=1.0)
        trades_v47, eq_v47, dd_v47 = backtest_v61(
            C, O, H, L, NS, ND, dates, syms, sigs_v47like,
            sector_lookup=sector_lookup,
            st_weight=1.0,
            win_threshold=best["wt"],
            normal_threshold=best["nt"],
        ...
        print(f"\n  V61 MULTI-TF (st_w={best['st_w']:.2f}):")
        analyze(trades_v61, eq_v61, dd_v61, "V61-multi-TF")
        print(f"\n  V47-LIKE SINGLE-TF (st_w=1.00):")
        analyze(trades_v47, eq_v47, dd_v47, "V47-single-TF")

        if trades_v61 and trades_v47:
            print(
                f"\n  V61 vs V47-like: "
                f"eq_delta={eq_v61 - eq_v47:+,.0f} "
                f"dd_delta={dd_v61 - dd_v47:+.1f}%"
            )

    print(f"\n[V61] Done. {time.time() - t0:.1f}s")
