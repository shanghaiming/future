"""
V59: V47 with Optimized Sector Parameters
==========================================
V47 is our ALL-TIME BEST: Sharpe 6.65, MDD 8.8%, ann +19.8%, 112 trades (OOS 2019-2026).
V47 = V43 dynamic mode switching + max 2 trades per sector per day.

V59 does a DETAILED parameter sweep around V47's best config to find the true optimum.

Sweep parameters:
  - max_per_sector: 1, 2, 3
  - win_threshold: 0.55, 0.58, 0.60, 0.62, 0.65
  - normal_threshold: 0.80, 0.82, 0.85
  - lose_threshold: 0.85, 0.88, 0.90, 0.92
  - win_rate_window: 10, 12, 15, 18, 20
  - atr_stop: 2.5, 3.0, 3.5
  - top_n_winning: 2, 3

Fixed: hold_days=5, pyramid_day=1, use_ker_gate=True, min_confidence=3
...
def compute_raw_factors(
    ...
    t0 = time.time()
    print("[V59] Computing raw factors...", flush=True)
...
def compute_cross_sectional_ranks(
    ...
    t0 = time.time()
    print("[V59] Computing cross-sectional ranks...", flush=True)
...
def build_composite_signal(
    ...
    t0 = time.time()
    print("[V59] Building composite signal...", flush=True)
...
def backtest_v59(
    ...
    """Backtest with dynamic threshold switching + sector limits."""
...
def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("  V59: V47 WITH OPTIMIZED SECTOR PARAMETERS")
    print("  Detailed parameter sweep around V47's best config")
    print("=" * 70)
    ...
    # === 1. FULL PARAMETER SWEEP (2019-2026) ===
    print("\n" + "=" * 70)
    ...
    for v in sweep_params.values():
        total_combos *= len(v)
    print(f"  Total raw combinations: {total_combos}")

    # Filter valid combos: win_threshold > 0.50, lose > normal
    valid_combos = []
    for mps, wt, nt, lt, ww, ats, tnw in product(
        ...
        if lt <= nt:
            continue
        valid_combos.append((mps, wt, nt, lt, ww, ats, tnw))

    print(f"  Valid combinations after constraints: {len(valid_combos)}")

    results: List[dict] = []
    combo_count = 0

    for mps, wt, nt, lt, ww, ats, tnw in valid_combos:
        combo_count += 1
        if combo_count % 500 == 0:
            elapsed = time.time() - t0
            print(
                f"  ... {combo_count}/{len(valid_combos)} combos "
                f"({elapsed:.0f}s elapsed)",
                flush=True,
            )

        trades, eq, dd = backtest_v59(
            C, O, H, L, NS, ND, dates, syms, sigs,
            sector_lookup=sector_lookup,
            win_threshold=wt,
            normal_threshold=nt,
    ...
    for r in results[:30]:
        print(
            f"{r['mps']:>3} {r['wt']:>4.2f} {r['nt']:>4.2f} "
            f"{r['lt']:>4.2f} {r['ww']:>3} {r['ats']:>4.1f} "
            f"{r['tnw']:>4} "
    ...
    # === 4. WALK-FORWARD FOR TOP 5 ===
    print("\n" + "=" * 70)
    print("  WALK-FORWARD VALIDATION FOR TOP 5 (2019-2026)")
    print("=" * 70)
    ...
    # === 5. SUMMARY TABLE ===
    print("\n" + "=" * 70)
    print("  TOP 5 SUMMARY (ranked by OOS Sharpe)")
    print("=" * 70)
    for rank, r in enumerate(top5, 1):
        print(
            f"  #{rank}: mps={r['mps']} wt={r['wt']:.2f} "
            f"nt={r['nt']:.2f} lt={r['lt']:.2f} "
            f"ww={r['ww']} ats={r['ats']:.1f} tnw={r['tnw']} | "
            f"OOS: N={r['n']} WR={r['wr']:.1f}% "
            f"Ann={r['ann']:+.1f}% DD={r['dd']:.1f}% "
            f"Sh={r['sharpe']:.2f}"
        )

    print(f"\n[V59] Done. {time.time() - t0:.1f}s")
