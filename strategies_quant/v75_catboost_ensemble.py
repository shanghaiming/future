"""
V75 — LightGBM + CatBoost Ensemble with ATR-Adaptive Exits
============================================================
V74 achieved +360.7% with V71 features + ATR-adaptive stops.
This adds CatBoost with ordered boosting as an orthogonal model.
Ensemble: average(LGB_score, CB_score) → potentially higher WR + edge.
"""
...
print("=" * 70, flush=True)
print("  V75 — LightGBM + CatBoost Ensemble + ATR-Adaptive", flush=True)
print("=" * 70, flush=True)
...
# [5] Training — LightGBM + CatBoost
TRAIN_WINDOW = 252 * 3; RETRAIN_FREQ = 42; MIN_TRAIN = 252 * 2; FWD_DAYS = 5

print(f"\n[5] Training LGB + CatBoost ensemble...", flush=True)
t5 = time.time()
...
def make_train_data(train_start, train_end):
    """Build training data with rank labels"""
    ...
    if len(train_X) < 2000: return None, None, None, None
    ...
    for g in train_group:
        ...
        offset += g
    return tX, rank_y, train_group, ty

SCORE_LGB = np.full((NS, ND), np.nan)
SCORE_CB = np.full((NS, ND), np.nan)
n_trains = 0
...
for train_di in range(MIN_TRAIN, ND, RETRAIN_FREQ):
    ...
    tX, rank_y, train_group, _ = make_train_data(train_start, train_end)
    ...
    # CatBoost
    cb_model = CatBoostRanker(**CB_PARAMS)
    cb_pool = Pool(data=tX, label=rank_y, group_id=np.repeat(np.arange(len(train_group)), train_group))
    cb_model.fit(cb_pool)
    ...
    for di in range(train_di, pred_end):
        ...
        if pred_list:
            ...
            lgb_scores = lgb_model.predict(pX)
            cb_scores = cb_model.predict(pX)
            for k, si in enumerate(pred_si):
                SCORE_LGB[si, di] = lgb_scores[k]
                SCORE_CB[si, di] = cb_scores[k]

    if n_trains % 5 == 0:
        print(f"    #{n_trains}: di={train_di}", flush=True)

print(f"    Done: {n_trains} trainings ({time.time()-t5:.1f}s)", flush=True)

# Create ensemble scores
# Normalize each model's scores to [0, 1] per day, then average
SCORE_ENS = np.full((NS, ND), np.nan)
for di in range(MIN_TRAIN, ND):
    for score_arr in [SCORE_LGB, SCORE_CB]:
        vals = score_arr[:, di]
        mask = ~np.isnan(vals)
        if mask.sum() < 10: continue
    lgb_vals = SCORE_LGB[:, di]; cb_vals = SCORE_CB[:, di]
    both_mask = ~np.isnan(lgb_vals) & ~np.isnan(cb_vals)
    if both_mask.sum() < 10: continue
    # Rank-normalize each
    lgb_rank = np.argsort(np.argsort(lgb_vals[both_mask])).astype(float) / max(both_mask.sum()-1, 1)
    cb_rank = np.argsort(np.argsort(cb_vals[both_mask])).astype(float) / max(both_mask.sum()-1, 1)
    ens_rank = (lgb_rank + cb_rank) / 2
    for k, si in enumerate(np.where(both_mask)[0]):
        SCORE_ENS[si, di] = ens_rank[k]

# [6] Backtest engine with ATR-adaptive
def run_backtest(score_arr, sl_pct, tp_pct, hold_max, trail_pct,
                 trail_start=5, atr_adaptive=False,
                 atr_scale_min=0.6, atr_scale_range=0.8):
...
# [7] Parameter search on all 3 models
print(f"\n[7] Parameter search...", flush=True)

all_results = []
for label, score in [('lgb', SCORE_LGB), ('cb', SCORE_CB), ('ens', SCORE_ENS)]:
    # ATR-adaptive (proven best from V74)
    for hm in [7, 8, 10]:
        for sl_base in [4, 5, 6]:
            for trail in [1]:
                for ts in [4, 5]:
                    for scale_min in [0.5, 0.6, 0.7]:
                        for scale_range in [0.8, 1.0]:
                            r = run_backtest(score, sl_pct=sl_base, tp_pct=50,
                                            hold_max=hm, trail_pct=trail, trail_start=ts,
                                            atr_adaptive=True,
                                            atr_scale_min=scale_min,
                                            atr_scale_range=scale_range)
                            if r:
                                t = r.pop('trades')
                                all_results.append({**r, 'hm': hm, 'sl': sl_base,
                                                   'trail': trail, 'ts': ts,
                                                   'atr_min': scale_min,
                                                   'atr_range': scale_range,
                                                   'model': label})
    # Fixed SL (for comparison)
    for hm in [6, 7, 8]:
        for sl in [4, 5, 6]:
            for trail in [1]:
                for ts in [3, 4, 5]:
                    r = run_backtest(score, sl_pct=sl, tp_pct=50, hold_max=hm,
                                    trail_pct=trail, trail_start=ts)
                    if r:
                        t = r.pop('trades')
                        all_results.append({**r, 'hm': hm, 'sl': sl, 'trail': trail,
                                           'ts': ts, 'model': label, 'mode': 'fixed'})

all_results.sort(key=lambda x: -x['ann'])

# [8] Bug check
print(f"\n[8] Bug check...", flush=True)
np.random.seed(42)
SCORE_RAND = np.full_like(SCORE_ENS, np.nan)
mask = ~np.isnan(SCORE_ENS)
SCORE_RAND[mask] = np.random.randn(mask.sum())

r_rand = run_backtest(SCORE_RAND, sl_pct=7, tp_pct=50, hold_max=8,
                      trail_pct=1, trail_start=4, atr_adaptive=True,
                      atr_scale_min=0.6, atr_scale_range=0.8)
if r_rand:
    t = r_rand.pop('trades')
...
# [9] Results
print(f"\n[9] RESULTS (top 20)", flush=True)
print(f"  {'Model':>5s} {'HM':>3s} {'SL':>3s} {'Tr':>3s} {'TS':>3s} "
      f"{'Amin':>4s} {'Arng':>4s} | {'Ann':>7s} {'N':>4s} {'WR':>5s} {'W':>6s} "
      f"{'L':>6s} {'Edge':>6s} {'TPY':>5s} {'DD':>5s}", flush=True)
print(f"  {'-'*95}", flush=True)
for r in all_results[:20]:
    amin = f"{r.get('atr_min',''):.1f}" if 'atr_min' in r else ''
    arng = f"{r.get('atr_range',''):.1f}" if 'atr_range' in r else ''
    print(f"  {r['model']:>5s} {r['hm']:3d} {r['sl']:3d} {r['trail']:3d} {r['ts']:3d} "
          f"{amin:>4s} {arng:>4s} | {r['ann']:+7.1f}% {r['n']:4d} {r['wr']:5.1f}% "
          f"{r['avg_w']:+6.1f}% {r['avg_l']:6.1f}% {r['edge']:+6.2f}% {r['tpy']:5.1f} {r['max_dd']:4.1f}%",
          flush=True)

# Best per model
for model in ['lgb', 'cb', 'ens']:
    sub = [r for r in all_results if r['model'] == model]
    if sub:
        print(f"\n  Best {model}: {sub[0]['ann']:+.1f}% HM={sub[0]['hm']} SL={sub[0]['sl']} "
              f"Tr={sub[0]['trail']} TS={sub[0]['ts']}", flush=True)

# [10] Year-by-year
print(f"\n[10] Year-by-year...", flush=True)
best = all_results[0]
print(f"  Best: {best['model']} HM={best['hm']} SL={best['sl']} "
      f"Tr={best['trail']} TS={best['ts']} → {best['ann']:+.1f}%", flush=True)

score = {'lgb': SCORE_LGB, 'cb': SCORE_CB, 'ens': SCORE_ENS}[best['model']]
r_full = run_backtest(score, sl_pct=best['sl'], tp_pct=50, hold_max=best['hm'],
                      trail_pct=best['trail'], trail_start=best['ts'],
                      atr_adaptive=True, atr_scale_min=best.get('atr_min', 0.6),
                      atr_scale_range=best.get('atr_range', 0.8))
...
print(f"  FINAL: {best['model']} → {best['ann']:+.1f}% | WR={best['wr']:.0f}% Edge={best['edge']:+.2f}% "
      f"TPY={best['tpy']:.0f} DD={best['max_dd']:.0f}%", flush=True)
print(f"  ≥300%: {above_300} | ≥400%: {above_400} | ≥500%: {above_500}", flush=True)
print(f"  Total configs: {len(all_results)}", flush=True)
