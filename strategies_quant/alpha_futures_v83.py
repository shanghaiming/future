"""
V83: Fine-Grained V80 Parameter Sweep
======================================
Sweep in very small increments around V80's best parameters to find
the true optimum.

V80 best: st_w=0.65, nt=0.85, lt=0.95, max_positions=3, max_per_sector=3
          ann=+36.4%, Sharpe=5.62, MDD=5.2%, 53 trades, 66% WR

Sweep grid (fine-grained):
  st_weight:       0.55, 0.60, 0.65, 0.70, 0.75
  normal_threshold: 0.82, 0.83, 0.84, 0.85, 0.86, 0.87
  lose_threshold:   0.90, 0.92, 0.95, 0.98
  max_positions:    2, 3, 4, 5
  max_per_sector:   2, 3, 4
  win_rate_window:  10, 12, 15
  win_threshold:    0.60, 0.65, 0.70

Target: ann > 40%, Sharpe > 5, MDD < 10%
...
def compute_raw_factors(
    ...
    t0 = time.time()
    print("[V83] Computing raw factors (5d + 20d)...", flush=True)
...
def compute_cross_sectional_ranks(
    ...
    t0 = time.time()
    print("[V83] Computing cross-sectional ranks...", flush=True)
...
def build_multi_tf_composite(
    ...
    t0 = time.time()
    print(f"[V83] Building multi-TF composite (st_w={st_weight:.2f})...",
          flush=True)
...
def main() -> None:
    t0 = time.time()
    print("=" * 70)
    print("  V83: FINE-GRAINED V80 PARAMETER SWEEP")
    print("  V80 best: st_w=0.65 nt=0.85 lt=0.95 mp=3 mps=3")
    print("  V80 result: ann=+36.4% Sharpe=5.62 MDD=5.2% 53t 66%WR")
    print("  Target: ann > 40%, Sharpe > 5, MDD < 10%")
    print("  NO LEVERAGE. CASH0=1M, COMM=0.0005.")
    print("=" * 70)
    ...
    st_weight_values = [0.55, 0.60, 0.65, 0.70, 0.75]
    ...
    # === 2. Fine-grained parameter sweep ===
    print("\n" + "=" * 70)
    print("  FINE-GRAINED PARAMETER SWEEP (2019-2026)")
    print("  Sweeping around V80's optimum in small increments")
    print("=" * 70)
    ...
    for st_w in st_weight_values:
        ...
        for mps in [2, 3, 4]:
            for mp in [2, 3, 4, 5]:
                for ww in [10, 12, 15]:
                    for wt in [0.60, 0.65, 0.70]:
                        for nt in [0.82, 0.83, 0.84, 0.85, 0.86, 0.87]:
                            for lt in [0.90, 0.92, 0.95, 0.98]:
                                if lt <= nt:
                                    continue
                                sweep_count += 1
                                trades, eq, dd = backtest_v83(
                                    C, O, H, L, NS, ND, dates, syms, sigs,
                                    sector_lookup=sector_lookup,
                                    st_weight=st_w,
                                    win_threshold=wt,
    ...
    # Show configs meeting target: ann > 40%, Sharpe > 5, MDD < 10%
    target_configs = [
        r for r in results
        if r["ann"] > 40 and r["sharpe"] > 5 and r["dd"] < 10
    ]
    target_configs.sort(key=lambda x: -x["ann"])
    print(f"\n  Configs meeting TARGET (ann>40%, Sh>5, DD<10%): "
          f"{len(target_configs)}")
    if target_configs:
        print(
            f"{'STw':>4} {'WT':>4} {'NT':>4} {'LT':>4} {'WW':>3} "
            f"{'MPS':>3} {'MP':>3} "
    ...
    # === 4. Top config: full 10-year backtest + walk-forward ===
    # Use best by ann that also passes quality filters
    best_by_ann = top_10_ann[0]
    best_by_sharpe = results[0]

    # Pick the one with best ann that also has Sharpe > 4
    best = best_by_ann
    for r in top_10_ann:
        if r["sharpe"] >= 4.0 and r["dd"] < 15:
            best = r
            break

    print("\n" + "=" * 70)
    print(
        f"  BEST (by ann): st_w={best_by_ann['st_w']:.2f} "
        f"wt={best_by_ann['wt']:.2f} nt={best_by_ann['nt']:.2f} "
        f"lt={best_by_ann['lt']:.2f} ww={best_by_ann['ww']} "
        f"mps={best_by_ann['mps']} mp={best_by_ann['mp']}"
    )
    print(
        f"  BEST (by sharpe): st_w={best_by_sharpe['st_w']:.2f} "
        f"wt={best_by_sharpe['wt']:.2f} nt={best_by_sharpe['nt']:.2f} "
        f"lt={best_by_sharpe['lt']:.2f} ww={best_by_sharpe['ww']} "
        f"mps={best_by_sharpe['mps']} mp={best_by_sharpe['mp']}"
    )
    print(
        f"  SELECTED for WF: st_w={best['st_w']:.2f} "
    ...
    # Full 10-year backtest for top 5 unique configs
    print("\n" + "=" * 70)
    ...
    seen = set()
    unique_top = []
    all_sorted = sorted(results, key=lambda x: (-x["ann"], -x["sharpe"]))
    for r in all_sorted:
    ...
    print("\n" + "=" * 70)
    print(
        f"  WALK-FORWARD for selected config:"
    )
    print(
        f"  st_w={best['st_w']:.2f} "
        f"wt={best['wt']:.2f} nt={best['nt']:.2f} "
    ...
    # Also run walk-forward for best_by_sharpe if different
    if (best_by_sharpe["st_w"] != best["st_w"]
            or best_by_sharpe["wt"] != best["wt"]
            or best_by_sharpe["nt"] != best["nt"]
            or best_by_sharpe["lt"] != best["lt"]
            or best_by_sharpe["ww"] != best["ww"]
            or best_by_sharpe["mps"] != best["mps"]
            or best_by_sharpe["mp"] != best["mp"]):
        print("\n" + "=" * 70)
        print(
            f"  WALK-FORWARD for best-by-Sharpe config:"
        )
        print(
            f"  st_w={best_by_sharpe['st_w']:.2f} "
            f"wt={best_by_sharpe['wt']:.2f} nt={best_by_sharpe['nt']:.2f} "
            f"lt={best_by_sharpe['lt']:.2f} ww={best_by_sharpe['ww']} "
            f"mps={best_by_sharpe['mps']} mp={best_by_sharpe['mp']}"
        )
        ...
        walk_forward(
            C, O, H, L, NS, ND, dates, syms,
            signal_cache[best_by_sharpe["st_w"]],
            sector_lookup=sector_lookup,
            st_weight=best_by_sharpe["st_w"],
            win_threshold=best_by_sharpe["wt"],
            normal_threshold=best_by_sharpe["nt"],
            lose_threshold=best_by_sharpe["lt"],
            win_rate_window=best_by_sharpe["ww"],
            atr_stop=3.0,
            max_positions=best_by_sharpe["mp"],
            max_per_sector=best_by_sharpe["mps"],
        )

    print(f"\n[V83] Done. {time.time() - t0:.1f}s")
